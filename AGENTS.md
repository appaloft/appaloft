...
- if a behavior is marked rebuild-required in `docs/BUSINESS_OPERATION_MAP.md`, do not implement it directly; first update the governing ADR/specs/test matrix/implementation plan, then expose it through `CORE_OPERATIONS.md` and `operation-catalog.ts` during Code Round
...
- flow-control semantics are spec-first: rollout ordering, verification gates, cleanup timing, safe replacement, rollback-candidate retention, retry behavior, readiness checks, and runtime lifecycle ownership must be described in the governing ADR/workflow/testing specs before or in the same change as adapter/application code
...
- executable local or remote shell scripts must use `@appaloft/ash` and cross execution seams as `AshScript`; dynamic values must use explicit ash interpolation helpers, and contributors must not add local shell quoting helpers, newline-joined command arrays, or plain-string shell command interfaces
- run `bun run check:ash` after changing command-owning modules and keep it active in `lint:ci`; AI contributors must also follow `docs/ai/ASH_COMMAND_CONSTRUCTION.md` and must not add new checker allowlist entries
