...
- `appaloft auth mcp login` writes a dedicated bearer `mcp` profile through the same browser
  handoff. Use it when a host needs a bearer MCP profile. Cursor and OpenCode host install should
  reuse an already-logged-in `appaloft login` product-session or bearer profile instead.
