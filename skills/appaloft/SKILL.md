...
- Occupancy session: after `appaloft login` and an enrolled Server, `appaloft code` occupies
  ...
  Before answering “what project am I in?”, read `AGENTS.md` / `GROK.md` / `.appaloft/project.md`
  and call Appaloft MCP (`projects_show`, `resources_list`) or `appaloft project show` / `appaloft resource list`.
  Do not answer from git remotes or package.json alone. Occupy refreshes those files every time.
...
- Manage backing services: use dependency-resource provision/import/rotate-connection/inspect/query/
  backup/restore and resource dependency binding operations; do not inject raw connection strings
  into chat output. Pass imported connection replacements through `--connection-url-stdin` and do
  not delete/recreate a dependency merely to change its Appaloft-stored credential. Dependency query
  is allowlisted read-only inspection, not a database mutation escape hatch.
