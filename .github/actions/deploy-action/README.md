...
| `version` | `latest` | Appaloft release tag such as `v0.9.0`, `latest`, or `source`. `source` builds the CLI from the checked-out Appaloft source tree and requires Bun on `PATH`. Release values are also used for self-hosted console install. |
