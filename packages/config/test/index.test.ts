...
describe("resolveConfig", () => {
  ...
  test("defaults to embedded pglite with user-level storage", () => {
    ...
    expect(config.remotePgliteSyncBackupMaxCount).toBe(20);
  });
  ...
  test("[CONFIG-FILE-STATE-013] allows overriding the remote PGlite sync backup recovery window", () => {
    const config = resolveConfig({
      env: {
        ...
        APPALOFT_REMOTE_PGLITE_SYNC_BACKUP_MAX_COUNT: "5",
      },
    });

    expect(config.remotePgliteSyncBackupRetentionDays).toBe(14);
    expect(config.remotePgliteSyncBackupMaxCount).toBe(5);
  });
  ...
  test("reads the installer first-admin bootstrap settings from runtime configuration", () => {
    const config = resolveConfig({
      env: {
        ...
        APPALOFT_BETTER_AUTH_MIN_PASSWORD_LENGTH: "4",
      },
    });
