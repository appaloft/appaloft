...
export function registerRuntimeDependencies(
  ...
  container.register(tokens.occupancyAgentRepository, {
    useFactory: instanceCachingFactory(
      () =>
        new PgOccupancyAgentRepository(input.database.db, (prefix) =>
  ...
  container.register(tokens.githubAgentAutomationStore, {
    useFactory: instanceCachingFactory(() => new PgGitHubAgentAutomationStore(input.database.db)),
  });
