...
describe("CLI project commands", () => {
  ...
  test("[PROJ-LIFE-ENTRY-CLI-004] project restore dispatches the application command", async () => {
    ensureReflectMetadata();
    const { RestoreProjectCommand, createExecutionContext } = await import("@appaloft/application");
    const { createCliProgram } = await import("../src");
    const commands: AppCommand<unknown>[] = [];
    const commandBus = {
      execute: async <T>(_context: unknown, command: AppCommand<T>) => {
    ...
    const executionContextFactory: ExecutionContextFactory = {
      create: (input) =>
        createExecutionContext({
          ...input,
          requestId: "req_cli_project_restore_test",
        }),
    };
    const program = createCliProgram({
      version: "0.1.0-test",
    ...
    try {
      process.stdout.write = (() => true) as typeof process.stdout.write;
      await program.parseAsync(["node", "appaloft", "project", "restore", "prj_demo"]);
    } finally {
      process.stdout.write = writeStdout;
    }

    expect(commands).toHaveLength(1);
    expect(commands[0]).toBeInstanceOf(RestoreProjectCommand);
    expect(commands[0]).toMatchObject({
      projectId: "prj_demo",
    });
  });

  test("[PROJ-LIFE-ENTRY-CLI-005] project delete-check dispatches the application query", async () => {
    ensureReflectMetadata();
    const { CheckProjectDeleteSafetyQuery, createExecutionContext } = await import(
      "@appaloft/application"
    );
    const { createCliProgram } = await import("../src");
    const queries: AppQuery<unknown>[] = [];
    ...
    const executionContextFactory: ExecutionContextFactory = {
      create: (input) =>
        createExecutionContext({
          ...input,
          requestId: "req_cli_project_delete_check_test",
        }),
    };
    const program = createCliProgram({
      version: "0.1.0-test",
    ...
    try {
      process.stdout.write = (() => true) as typeof process.stdout.write;
      await program.parseAsync(["node", "appaloft", "project", "delete-check", "prj_demo"]);
    } finally {
      process.stdout.write = writeStdout;
    }

    expect(queries).toHaveLength(1);
    expect(queries[0]).toBeInstanceOf(CheckProjectDeleteSafetyQuery);
    expect(queries[0]).toMatchObject({
      projectId: "prj_demo",
    });
  });

  test("[PROJ-LIFE-ENTRY-CLI-006] project delete dispatches the application command", async () => {
    ensureReflectMetadata();
    const { DeleteProjectCommand, createExecutionContext } = await import("@appaloft/application");
    const { createCliProgram } = await import("../src");
    const commands: AppCommand<unknown>[] = [];
    const commandBus = {
      execute: async <T>(_context: unknown, command: AppCommand<T>) => {
    ...
    const executionContextFactory: ExecutionContextFactory = {
      create: (input) =>
        createExecutionContext({
          ...input,
          requestId: "req_cli_project_delete_test",
        }),
    };
    const program = createCliProgram({
      version: "0.1.0-test",
    ...
    expect(commands).toHaveLength(1);
    expect(commands[0]).toBeInstanceOf(DeleteProjectCommand);
    expect(commands[0]).toMatchObject({
      projectId: "prj_demo",
  ...
  test("[PROJ-LIFE-ENTRY-002] project set-description dispatches the application command", async () => {
    ensureReflectMetadata();
    const { SetProjectDescriptionCommand, createExecutionContext } = await import(
      "@appaloft/application"
    );
    const { createCliProgram } = await import("../src");
    const commands: AppCommand<unknown>[] = [];
    ...
    const executionContextFactory: ExecutionContextFactory = {
      create: (input) =>
        createExecutionContext({
          ...input,
          requestId: "req_cli_project_description_test",
        }),
    };
    const program = createCliProgram({
      version: "0.1.0-test",
    ...
    expect(commands).toHaveLength(1);
    expect(commands[0]).toBeInstanceOf(SetProjectDescriptionCommand);
    expect(commands[0]).toMatchObject({
      projectId: "prj_demo",
  ...
  test("[FOLDER-ONBOARD-004] project use persists the folder link for this cwd", async () => {
    ensureReflectMetadata();
    const { ShowProjectQuery, createExecutionContext } = await import("@appaloft/application");
    const { createCliProgram } = await import("../src");
    const { fileFolderProjectLinkStore, readFolderProjectLink } = await import(
      "../src/folder-project-link.js"
    );
    const home = await (await import("node:fs/promises")).mkdtemp(
      (await import("node:path")).join((await import("node:os")).tmpdir(), "appaloft-project-use-"),
    );
    const previousHome = process.env.APPALOFT_HOME;
    process.env.APPALOFT_HOME = home;
    const queries: AppQuery<unknown>[] = [];
    const commandBus = {
      execute: async <T>(_context: unknown, _command: AppCommand<T>) => ok({} as T),
    } as unknown as CommandBus;
    ...
    const program = createCliProgram({
      ...
      executionContextFactory: {
        create: (input) =>
          createExecutionContext({
            ...input,
            requestId: "req_cli_project_use_test",
          }),
      },
    ...
    try {
      ...
      await program.parseAsync(["node", "appaloft", "project", "use", "prj_switched"]);
    } finally {
      process.stdout.write = writeStdout;
      process.stderr.write = writeStderr;
      if (previousHome === undefined) delete process.env.APPALOFT_HOME;
      else process.env.APPALOFT_HOME = previousHome;
    }
    expect(queries[0]).toBeInstanceOf(ShowProjectQuery);
    expect(queries[0]).toMatchObject({ projectId: "prj_switched" });
    const store = fileFolderProjectLinkStore({ APPALOFT_HOME: home });
    expect((await readFolderProjectLink(process.cwd(), store))?.projectId).toBe("prj_switched");
