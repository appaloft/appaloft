...
describe("CLI deployment log commands", () => {
  test("[DEP-LOG-PRUNE-004] deployments logs prune dispatches the application command", async () => {
    ...
    const executionContextFactory: ExecutionContextFactory = {
      create: (input) =>
        createExecutionContext({
          ...input,
          requestId: "req_cli_deployment_log_prune_test",
        }),
    };
    ...
    expect(commands[0]).toBeInstanceOf(PruneDeploymentLogsCommand);
