...
describe("public docs operation coverage", () => {
  ...
  test("[SRV-ROLE-ENTRY-005] workload role configuration maps to public role guidance", async () => {
    ...
    const englishPage = await repositoryFile(
      "apps/docs/src/content/docs/en/servers/register-connect.mdx",
    ).text();
    const chinesePage = await repositoryFile(
      "apps/docs/src/content/docs/servers/register-connect.mdx",
    ).text();
    ...
    for (const page of [englishPage, chinesePage]) {
      expect(page).toContain('id="server-workload-roles"');
  ...
  test("[AGENT-DEPLOY-SKILL-001][AGENT-DEPLOY-SKILL-002] deploy skill is safe and operation-backed", async () => {
    ...
    expect(skill).toContain("appaloft up ./dist --as static-site");
