...
import { type ForceRedeployDeploymentCommandInput } from "./force-redeploy-deployment.command";
...
@injectable()
export class ForceRedeployDeploymentUseCase {
  constructor(
  ...
  async execute(
    ...
    return safeTry(async function* () {
      ...
      if (sourceState && !isServerBackedDeploymentState(sourceState)) {
        return err(
          domainError.deploymentNotRedeployable(
            "Force redeploy source deployment must be server-backed",
            {
              commandName: "deployments.force-redeploy",
              phase: "force-redeploy-admission",
              deploymentId: sourceState.id.value,
              resourceId: sourceState.resourceId.value,
              causeCode: "serverless_static_force_redeploy_not_supported",
            },
          ),
        );
      }
      ...
      if (!sourceState && (!input.projectId || !input.environmentId || !input.serverId)) {
        return err(
          domainError.deploymentNotRedeployable(
            "Force redeploy requires project, environment, and server context or a source deployment",
            {
              commandName: "deployments.force-redeploy",
              phase: "force-redeploy-admission",
