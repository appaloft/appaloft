...
import { type DeactivateServerCommandInput } from "./deactivate-server.command";
...
@injectable()
export class DeactivateServerUseCase {
  constructor(
  ...
  async execute(
    ...
    input: DeactivateServerCommandInput,
    ...
    return safeTry(async function* () {
      ...
      const reason = yield* DeactivationReason.fromOptional(input.reason);
      ...
      if (state.lifecycleStatus.isInactive()) {
        return ok({ id: serverId.value });
      }

      const deactivatedAt = yield* DeactivatedAt.create(clock.now());
      const deactivateResult = yield* server.deactivate({
        deactivatedAt,
        ...(reason ? { reason } : {}),
      });
      if (!deactivateResult.changed) {
        return ok({ id: serverId.value });
      }
