...
import { type ResetResourceHealthCommandInput } from "./reset-resource-health.command";

@injectable()
export class ResetResourceHealthUseCase {
  constructor(
  ...
  async execute(
    ...
    input: ResetResourceHealthCommandInput,
    ...
    return safeTry(async function* () {
      ...
      const resetAt = yield* UpdatedAt.create(clock.now());
      yield* resource.resetHealthPolicy({ resetAt });
