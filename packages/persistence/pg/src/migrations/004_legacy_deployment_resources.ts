...
export const legacyDeploymentResourcesMigration = {
  async up(db: Kysely<Database>): Promise<void> {
    ...
    await sql`
      ...
      SELECT
        'res_legacy_' || deployments.project_id || '_' || deployments.environment_id,
        deployments.project_id,
        deployments.environment_id,
        MIN(deployments.destination_id),
        'legacy-resource',
        'legacy-resource',
        'application',
        'Backfilled resource for legacy deployments',
        '[]'::jsonb,
        MIN(deployments.created_at)
      FROM deployments
      WHERE deployments.resource_id IS NULL
      GROUP BY deployments.project_id, deployments.environment_id
      ...
      WHERE resources.destination_id IS NULL
        ...
        AND destinations.server_id = (
          ...
          WHERE deployments.resource_id = resources.id
             OR (
              deployments.project_id = resources.project_id
              AND deployments.environment_id = resources.environment_id
            )
    ...
    await sql`
      ...
      FROM resources
      WHERE deployments.resource_id IS NULL
        AND resources.project_id = deployments.project_id
        AND resources.environment_id = deployments.environment_id
    `.execute(db);
