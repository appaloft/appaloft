...
| Web project page | Must treat resource list and create-resource as primary. Quick Deploy from a project page must enter the Quick Deploy workflow or another entry workflow that selects or creates a resource before deployment admission. |
| Web resource detail page | Owns resource-scoped Quick Deploy, deployment history, source/runtime configuration, and domain/TLS affordances. Redeploy is absent until reintroduced under ADR-016. |
