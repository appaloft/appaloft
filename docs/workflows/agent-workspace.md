...
worktrees still fail closed. A non-git directory occupies from existing occupancy identity or a
git remote. `--profile` and `--new` on `code` keep remote-open meaning and do not
