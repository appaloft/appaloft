...
worktrees still fail closed. `workspace open` from a non-git directory occupies from a git remote
and does not resume an unrelated occupancy. Default `code` from a no-git folder resumes the live
occupancy; git is not a `code` gate. `--profile` and `--new` on `code` keep remote-open meaning and do not
...
explicitly names a colliding Profile that has more than one live occupancy; that error lists
both ids and `appaloft code --profile <id>`. A failed `--new` must not leave another enabled
same-name installation.
