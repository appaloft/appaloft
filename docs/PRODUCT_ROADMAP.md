...
| Multi-server / Swarm / Kubernetes | Multi-node scheduling is important only after single-server resource lifecycle is reliable. | Target kinds include future multi-server/K8s concepts, but current support is single-server oriented. | Multi-node scheduling is not available. | P3 | Optional | Defer until single-server resource lifecycle, storage, routing, and rollback are reliable. |
