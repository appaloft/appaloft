...
| Read Dashboard Project Environment overview | Product-session member query | `project-environments.overview` | `ProjectEnvironmentOverviewQuery` | `ProjectEnvironmentOverviewQueryInput` | Not applicable; compact selection uses existing Project/Environment/Resource list operations | `GET /api/projects/{projectId}/environments/{environmentId}/overview` |
...
| Configure Server Runtime Target Profile    | Command                      | `servers.configure-runtime-target-profile`   | `ConfigureServerRuntimeTargetProfileCommand`  | `ConfigureServerRuntimeTargetProfileCommandInput`  | `appaloft server configure-runtime-target-profile <serverId> --connection-reference <ref>`            | `POST /api/servers/{serverId}/runtime-target-profile`                                 |
...
| Inspect Server Runtime Readiness           | Query                        | `servers.runtime-readiness`                  | `InspectServerRuntimeReadinessQuery`          | `InspectServerRuntimeReadinessInput`               | `appaloft server readiness <serverId>`                                                                | `GET /api/servers/{serverId}/runtime-readiness`                                       |
...
| Plan environment profile duplication  | Query                        | `environments.plan-duplicate`       | `PlanDuplicateEnvironmentQuery`       | `PlanDuplicateEnvironmentQueryInput`       | `appaloft env copy <environmentId> <targetName> --dry-run`                             | `GET /api/environments/{environmentId}/duplicate-plan`                      |
| Apply environment profile duplication | Command                      | `environments.duplicate-profile`    | `DuplicateEnvironmentProfileCommand`  | `DuplicateEnvironmentProfileCommandInput`  | `appaloft env copy <environmentId> <targetName>`                                       | `POST /api/environments/{environmentId}/duplicate-profile`                  |
...
| Create resource secret reference        | Command                      | `resources.secrets.create`                 | `CreateResourceSecretReferenceCommand`     | `CreateResourceSecretReferenceCommandInput`     | `appaloft resource secrets create <resourceId> <key> [<value> \| --stdin]`                                         | `POST /api/resources/{resourceId}/secrets`                                                                    |
| Rotate resource secret reference        | Command                      | `resources.secrets.rotate`                 | `RotateResourceSecretReferenceCommand`     | `RotateResourceSecretReferenceCommandInput`     | `appaloft resource secrets rotate <resourceId> <key> [<value> \| --stdin]`                                         | `POST /api/resources/{resourceId}/secrets/{key}`                                                              |
...
| Read Dashboard Resource overview        | Product-session member query | `resources.overview`                       | `ResourceOverviewQuery`                    | `ResourceOverviewQueryInput`                    | Not applicable; `resources.show` and `deployments.list` remain the compact CLI surfaces                            | `GET /api/projects/{projectId}/environments/{environmentId}/resources/{resourceId}/overview`                  |
...
| Rotate imported dependency connection        | Command | `dependency-resources.rotate-connection`         | `RotateDependencyResourceConnectionCommand`       | `RotateDependencyResourceConnectionCommandInput`      | `appaloft dependency rotate-connection <dependencyResourceId> --connection-url-stdin`        | `POST /api/dependency-resources/{dependencyResourceId}/connection`                  |
...
| Restore dependency resource backup           | Command | `dependency-resources.restore-backup`            | `RestoreDependencyResourceBackupCommand`          | `RestoreDependencyResourceBackupCommandInput`         | `appaloft dependency backup restore <backupId> [--target-dependency <dependencyResourceId>]` | `POST /api/dependency-resources/backups/{backupId}/restore`                         |
...
| `sandboxes.agents.runs.events.stream` | Query/stream | `StreamSandboxAgentRunEventsQuery`     | Run id, after-sequence cursor, bounded page size, terminal-close preference                                          | `appaloft sandbox agent run events <runId> --follow`               | `GET /api/sandbox-agent-runs/{runId}/events/stream`                     |
...
| ---------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Open remote Agent door | none                                                                              | default `appaloft code` prints present-tense progress, occupies after login + Server, and native-attaches as soon as my occupancy exists; TTY first useful alt-screen is under 1s and is not blocked on composition; a no-git folder occupies this folder after onboarding (git is not a gate) instead of staying on `Resolving repository…` or resuming an unrelated occupancy; it create-or-links this folder's Project and occupies `folder.local` without cloning or materializing a missing remote; `--profile` is a fallback pin; `--no-attach` prints the same progress and stops; `--local` is Scratch |
...
| List stale deployment attempts          | Query                        | `deployments.stale-attempts`     | `ListStaleDeploymentAttemptsQuery` | `ListStaleDeploymentAttemptsQueryInput` | `appaloft deployments stale`                                                                                                                                                                                                                                                                                                                                                                                                                          | `GET /api/deployments/stale`                             |
| Reconcile stale deployment attempt      | Command                      | `deployments.reconcile-stale`    | `ReconcileStaleDeploymentCommand`  | `ReconcileStaleDeploymentCommandInput`  | `appaloft deployments reconcile-stale <deploymentId> --state-version <stateVersion> --confirm <deploymentId>`                                                                                                                                                                                                                                                                                                                                         | `POST /api/deployments/{deploymentId}/reconcile-stale`   |
...
| Export global audit events           | Query   | `audit-events.export-global`         | `ExportGlobalAuditEventsQuery`        | `ExportGlobalAuditEventsQueryInput`        | `appaloft audit-event export-global --from <iso> --to <iso> [--cursor <iso>] [--order asc\|desc]` | `GET /api/audit-events/export-global`                                                        |
...
| Create domain binding                              | Command | `domain-bindings.create`                       | `CreateDomainBindingCommand`                     | `CreateDomainBindingCommandInput`                     | `appaloft domain-binding create <domainName> [--target-service-name <service>] [--redirect-to <domain>] [--redirect-status 301\|302\|307\|308]`                                  | `POST /api/domain-bindings`                                           |
...
| List GitHub repositories            | Query   | `system.github-repositories.list`            | `ListGitHubRepositoriesQuery`            | `ListGitHubRepositoriesQueryInput`            | `appaloft github repositories [--search <text>]`                                                                                            | `GET /api/integrations/github/repositories`   |
| Show GitHub App connection          | Query   | `system.github-app-connection.show`          | `GitHubAppConnectionQuery`               | `GitHubAppConnectionQueryInput`               | `appaloft github status`                                                                                                                    | `GET /api/integrations/github/app-connection` |
...
| Plan control-plane secret rotation  | Query   | `system.control-plane-secret-rotation.plan`  | `ControlPlaneSecretRotationPlanQuery`    | none                                          | `appaloft db secret-rotation plan [--state-backend ssh-pglite --server-host <host>]`                                                        | none                                          |
| Apply control-plane secret rotation | Command | `system.control-plane-secret-rotation.apply` | `ControlPlaneSecretRotationApplyCommand` | `ControlPlaneSecretRotationApplyCommandInput` | `appaloft db secret-rotation apply --plan-digest <digest> --backup-reference <reference> [--state-backend ssh-pglite --server-host <host>]` | none                                          |
