...
| --- | --- | --- |
| Open remote Agent door | none | default `appaloft code` prints present-tense progress, occupies after login + Server, and native-attaches as soon as my occupancy exists; `--profile` is a fallback pin; `--no-attach` prints the same progress and stops; `--local` is Scratch |
