...
| [ADR-022: Operator Terminal Session Boundary](./ADR-022-operator-terminal-session-boundary.md) | Accepted | Ephemeral server/resource terminal sessions use an explicit command and runtime port, with resource scope resolving deployment workspaces by deployment metadata or retained runtime container targets. |
...
| [ADR-045: Self-Hosted Organization Team Operations](./ADR-045-self-hosted-organization-team-operations.md) | Accepted | Defines organization/team command/query ownership, current context, invitation, non-owner member role changes, ownership transfer, non-owner member removal, and the Better Auth adapter boundary for post-bootstrap product auth. |
