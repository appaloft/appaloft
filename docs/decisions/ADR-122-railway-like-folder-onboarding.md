...
5. Default `code` resumes only an occupancy that matches this folder's
   identity. It must not resume the latest occupancy of another repository.
   Explicit `code <git-remote>` still follows ADR-119.
...
- Spec 139 WS-REMOTE-RESUME-004 / WS-REMOTE-NO-UPLOAD-006 no longer resume an
  unrelated occupancy from a non-matching cwd.
