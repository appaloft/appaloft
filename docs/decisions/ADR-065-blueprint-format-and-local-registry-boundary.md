...
- Blueprint instantiation may also produce a neutral `BlueprintApplicationBundlePlan`: one application-level grouping that contains multiple Blueprint components, their generated Resource/deployment intents, and their dependency-resource binding intents. The bundle plan is a dry-run planning artifact, not a persisted installed application record and not a second deployment engine.
- Dependency resource requirements may declare protocol-level readiness surfaces such as `tcp`, `http`, `postgres`, `redis`, `mysql`, `mongodb`, and `clickhouse`. Readiness must describe the surface the application actually uses, so a ClickHouse native query on port 9000 does not satisfy an application that connects through ClickHouse HTTP `/ping` on port 8123.
...
- The neutral Application Bundle concept belongs in Community because local/self-hosted users also need to understand that one Blueprint may represent an application made of multiple services plus dependency resources. Hosted catalog curation, installed application persistence, acceptance commands, upgrade execution, billing, entitlement, and marketplace trust metadata remain outside this public boundary.
