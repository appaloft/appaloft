...
| `/resources/[resourceId]` and nested canonical route | Resource detail workspace | Operate one resource | Resource header plus tabs; `settings` tab contains many vertical sections | URL tab/section params, popover health, inline forms, runtime controls, logs, terminal, domain forms | The default tab is `settings`, not operational overview; resource archive/delete are in the header; settings section contains profile, source, config, storage, dependencies, domains, usage, health, proxy, diagnostics; domain/TLS flow is a form panel rather than a guided verification flow | Tabbed detail page with overloaded tab |
...
| Resource `deployments` tab | Resource deployment history | Inspect and create deployments | Header plus deployment table | New deployment button and table navigation | Good table choice, but empty state is a muted strip with little guidance | Tab |
...
| Manage resource settings | Update source/runtime/network/access/health | Medium | Medium to high | High | Stable detail section | Resource Configure tab with vertical section nav |
...
- Resource detail defaults to `settings`, so the most important operational questions are not first: what is live, what URL works, is it healthy, what deployed last, and what needs attention.
- The resource `settings` tab is overloaded with profile, source, auto-deploy, configuration, storage, dependencies, domains, usage, health, proxy, and diagnostics. These are stable sibling sections and need clearer grouping under `Overview`, `Deployments`, `Logs`, `Domains`, `Environment`, `Metrics/Health`, and `Settings`.
