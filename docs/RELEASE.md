...
- The GitHub Release body is generated by `scripts/release/generate-release-notes.ts`. The default
  body is developer-facing: `appaloft up`, `appaloft setup agent`, one gotcha, npm/Homebrew CLI
  install, and the Compare Railway FAQ. It must not lead with `install.sh | sudo sh`, must not use
  occupancy wording, and must not dump the conventional-commit `CHANGELOG.md` section as the
  default notes. Commit-level detail stays a link to `CHANGELOG.md`. Download links, container
  image tags, and known packaging gaps follow that summary.
