...
6. If the folder has `public/index.html`, run `appaloft deploy .` / `appaloft deploy . --yes`.
   ...
   Keep the real cwd folder; the static plan writes that folder, and SSH package reconstructs it
   from leaf/`displayName` even if locator and workingDirectory were already walked to the parent,
   the package host does not have that folder, and cwd is a runtimeDir. A missing-workdir error
   names the full folder.
