...
`resources.configure-health`, and `resources.archive`. `resources.delete` remains the next guarded
cleanup candidate. No entrypoint may expose a generic `resources.update`; each source, runtime,
network, health, archive, and delete action must dispatch its own command or query.
