...
| SSH credential lifecycle | UI/API/CLI workflow / active read slice | `credentials.show` reads one reusable SSH credential's masked detail and safe server usage summary. It is the required visibility step before rotate/update or delete-when-unused commands can become public. | `credentials.show` | [SSH Credential Lifecycle](./workflows/ssh-credential-lifecycle.md), [credentials.show](./queries/credentials.show.md), [SSH Credential Lifecycle Error Spec](./errors/credentials.lifecycle.md), [SSH Credential Lifecycle Test Matrix](./testing/ssh-credential-lifecycle-test-matrix.md), [SSH Credential Lifecycle Implementation Plan](./implementation/ssh-credential-lifecycle-plan.md) |
