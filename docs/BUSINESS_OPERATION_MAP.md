...
| Deployment detail and observation | UI/read workflow / active | Deployment history or a deployment deep link resolves one immutable deployment attempt detail through `deployments.show`, reads attempt logs through `deployments.logs`, and may reconnect to create-time progress transport without turning that transport into a standalone business query. | `deployments.show` plus `deployments.logs` | [Deployment Detail And Observation](./workflows/deployment-detail-and-observation.md), [deployments.show](./queries/deployments.show.md), [Deployment Detail Test Matrix](./testing/deployments.show-test-matrix.md), [ADR-013](./decisions/ADR-013-project-resource-navigation-and-deployment-ownership.md), [ADR-016](./decisions/ADR-016-deployment-command-surface-reset.md) |
...
- Should future `deployments.stream-events` remain a separate deployment query or stay a transport-
  specific reconnect/read model over the accepted `deployments.show` detail contract?
