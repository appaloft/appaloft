...
| Reattach deployment | `deployments.stream-events` or transport reconnect over the same query | Treat reconnect as read-only observation over the accepted deployment event stream boundary; do not reintroduce it as a write command. |
