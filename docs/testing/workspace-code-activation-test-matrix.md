...
| WS-CODE-PACKAGE-011 | packaging/CLI | Supported packaged artifact is executed | `appaloft code --help` / `-h` starts without persistence/runtime composition, prints a compact stdout option table, and documents `--profile`, `--new`, `--no-attach`, `--local`, `--yes`, `--open`, `--open-target`, and `--harness` with real-use copy. | `apps/shell/test/help-without-runtime.test.ts`; `packages/adapters/cli/test/code-help.test.ts`; host release-bundle smoke | passing |
