...
| WS-CODE-PACKAGE-011 | packaging/CLI | Supported packaged artifact is executed | `appaloft code --help` starts without persistence/runtime composition and documents `--profile`, `--new`, `--no-attach`, and `--local`. | `apps/shell/test/help-without-runtime.test.ts`; host release-bundle smoke | passing |
