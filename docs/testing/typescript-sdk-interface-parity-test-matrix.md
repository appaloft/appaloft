...
| TS-SDK-ERROR-002 | SDK runtime | Sanitized unexpected response handling | HTML and non-JSON responses return stable structured errors without response bodies; unstructured JSON that is not an Appaloft error contract still returns `sdk_unstructured_error` with HTTP status, an optional remote code, and a truncated body preview (not a raw dump); HTTP 502, 503, and 504 variants are retriable and other statuses are non-retriable. |
