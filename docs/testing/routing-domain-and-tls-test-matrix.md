...
| ROUTE-TLS-CMD-031 | application admission | Reject internal single-label host as durable public domain | `domainName` is an internal single-label DNS name accepted by `HostAddress` | `err` | `validation_error`, phase `command-validation` | None | No binding created; server host identity and public domain/TLS contracts remain distinct | No |
| ROUTE-TLS-CMD-032 | integration | Configure automatic certificate policy | Active TLS binding uses manual policy | `domain-bindings.configure-certificate-policy(auto)` | `ok` | policy-transition fact when changed | Binding records auto policy and pending reconciliation; current serving certificate is retained | No |
| ROUTE-TLS-CMD-033 | integration | Configure manual certificate policy | Active TLS binding uses auto policy | `domain-bindings.configure-certificate-policy(manual)` | `ok` | policy-transition fact when changed | Binding records manual policy and pending reconciliation; current serving certificate is retained | No |
| ROUTE-TLS-CMD-034 | integration | Reject certificate policy on TLS-disabled binding | Binding TLS is disabled | configure policy | `err` | `certificate_policy_not_allowed`, phase `certificate-policy-configuration` | Binding and route remain unchanged | No |
...
| ROUTE-TLS-EVT-008 | integration | Certificate issued requests reconciliation | `certificate-issued` | Domain bound | `ok` | reconciliation work/fact | Binding remains not ready until activation and proof | Conditional |
| ROUTE-TLS-EVT-009 | integration | Certificate imported requests reconciliation | `certificate-imported` | Domain bound; manual certificate policy | `ok` | reconciliation work/fact | Binding remains not ready until activation and proof | Conditional |
...
| ROUTE-TLS-EVT-017 | integration | Reconciliation succeeds | Selected certificate is stored and route gates pass | activation/reload/proof succeeds | `ok` | `domain-ready` | Binding is ready with safe proven fingerprint metadata | No |
| ROUTE-TLS-EVT-018 | integration | Activation or reload fails | Previous certificate serves and candidate is stored | activation/reload fails | safe retriable failure | no `domain-ready` | Previous serving configuration and existing readiness truth remain; the returned error is safe retry evidence | Yes |
...
| ROUTE-TLS-EVT-022 | integration | Delayed event after policy transition | Imported event exists but binding now selects auto policy | reconciliation replays | safe failure before activation | none | Existing route/readiness remains unchanged | No; event is obsolete |
| ROUTE-TLS-EVT-023 | integration | Delayed event after revocation | Certificate event exists but certificate is no longer active | reconciliation replays | safe failure before activation | none | Existing route/readiness remains unchanged | No; new active material is required |
| ROUTE-TLS-EVT-024 | unit | Incomplete served proof | Readiness receives only certificate id or only fingerprint | aggregate marks ready | invariant failure | none | Binding cannot persist a half-proof | No |
| CERT-RECON-CONC-001 | integration | Binding-scoped certificate mutation serialization | Policy configuration, issue/renew, import, provider issuance, revocation, and reconciliation target one binding | writers overlap or event handlers nest under one request | one owned lease serializes mutations; nested handlers reuse its guard | normal lifecycle events only | A stale aggregate cannot overwrite newer policy or certificate lifecycle state | Yes after bounded coordination timeout |
| CERT-RECON-CONC-002 | integration | Fenced writer loses lease during long validation/provider work | A certificate writer owns the binding lease, then ownership is replaced before persistence | writer resumes after the long call | safe coordination failure | no certificate lifecycle event | No certificate/secret/repository mutation is committed from the stale writer | Yes under the new owner |
| ROUTE-TLS-MIG-001 | integration | Legacy optimistic readiness migration | TLS auto/manual binding is ready without a proven certificate pair | migration 117 runs | migration succeeds | none | Binding becomes `certificate_pending`; proof columns remain paired | Yes after reconciliation |
...
| ROUTE-TLS-READMODEL-018 | integration | Redirect binding current-truth parity | A ready binding's current route is a redirect while the last Deployment snapshot still describes a serve route | `domain-bindings.show` and shared selected-route descriptors report the binding's redirect destination/status | No current-state surface silently downgrades the redirect to `serve`; the historical snapshot is available only through deployment-snapshot scope. |
...
| ROUTE-TLS-WORKFLOW-005 | e2e-preferred | Manual certificate policy reaches ready state through import | CLI/API creates a manual-policy binding, confirms ownership, imports a valid certificate, and then lists certificates/resources | `certificate-imported` is recorded, activation/reload and served-certificate proof complete, then `domain-ready` follows and read models show HTTPS readiness without any `certificate-issued` event |
| ROUTE-TLS-WORKFLOW-006 | e2e-preferred, opt-in Docker | Policy switch serves the replacement certificate | A real Traefik/Caddy fixture serves an existing certificate for the binding hostname | operator switches policy and supplies/obtains a replacement | direct TLS with hostname as SNI observes the independently expected replacement fingerprint before readiness; failed candidates leave the previous certificate serving |
...
| ROUTE-TLS-ENTRY-023 | e2e-preferred | Compose routes target different services | CLI/API/config creates served bindings for one compose-stack resource with `/` targeting `web` and `/api` targeting `api` | Both bindings are accepted; deployment planning preserves target service and internal port; Swarm attaches each Traefik route to the selected service | `validation_error`, phase `domain-binding-admission`, for an undeclared service or redirect target service | `domain-binding-requested` includes `targetServiceName` | Binding read models expose the target and one compose deployment realizes both routes |
