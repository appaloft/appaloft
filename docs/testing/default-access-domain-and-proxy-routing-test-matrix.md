...
| DEF-ACCESS-QRY-002 | integration | Persisted resource after deploy | Latest deployment snapshot contains route state and access summary may include durable, server-applied, and generated routes | Resource query succeeds | None | Separate realized route fields remain visible, and current-route consumers select durable ready, server-applied, latest generated, then planned generated only when no deployment history exists | Resource detail, Quick Deploy completion, health, diagnostics, and proxy preview use the same selected-route precedence while still labeling route source | Query retry only |
...
| DEF-ACCESS-QRY-006 | integration | Cleaned preview route not current | Preview environment resource keeps deployment history after `deployments.cleanup-preview` removed its preview source link | Resource query succeeds | None | Historical preview route snapshots are omitted from current access summary; active preview source links still allow their route to project | Console does not display a closed PR preview URL as healthy/current, while deployment history remains visible | Query retry only |
