...
- Scheduled history retention is the third durable worker binding. It covers accepted scheduled
  ...
  manual history prune commands, retry-scheduled failure visibility, safe persisted details,
  skipped unsupported-category visibility, and disabled-by-default shell runner dispatch. It does
...
- Preview cleanup is the fourth operator-visible process-attempt binding. It covers successful and
  ...
  remains governed by the product-grade preview cleanup attempt store and preview-lifecycle
  mutation-coordinator lease rather than process-attempt atomic claim/completion.
...
- Dependency resource backup/restore is the eleventh operator-visible process-attempt binding. It
  ...
  command use cases rather than process-attempt atomic claim/completion.
- Provider-native dependency resource realization/delete is the twelfth operator-visible
  ...
  provider-managed `dependency-resources.delete` projection into the process attempt journal with
