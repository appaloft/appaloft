...
| --- | --- | --- | --- |
| SCHED-HISTORY-RETENTION-001 | Enabled retention defaults compute category cutoffs and dispatch existing prune commands or governed direct retention stores as dry-run by default. | Application + shell | Automated in `packages/application/test/scheduled-history-retention.test.ts` and `apps/shell/test/scheduled-history-retention-runner.test.ts`; runtime monitoring sample prune-store dispatch is covered in `packages/application/test/scheduled-history-retention.test.ts`. |
...
| SCHED-HISTORY-RETENTION-006 | Public surfaces remain existing `retention-defaults.*`, manual prune commands, and `operator-work.*`; the internal shell runner remains disabled by default, has an explicit batch size, and no parallel public scheduled-prune command is required for the first worker slice. | Shell config + docs/operation catalog | Covered by `packages/config/test/index.test.ts`, `apps/shell/test/maintenance-worker-status-reader.test.ts`, `apps/shell/test/scheduled-history-retention-runner.test.ts`, and `packages/application/test/scheduled-history-retention.test.ts`; runtime env vars are documented at `reference/configuration#reference-scheduled-workers`; this Code Round adds no operation catalog entry, CLI command, HTTP/oRPC route, or public scheduled-prune command. |
