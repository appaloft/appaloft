...
| RES-ACCESS-DIAG-ROUTE-005 | contract | Docker Swarm served route with diagnostic renderer target | Docker Swarm runtime renders provider-native Traefik labels directly for a served route and the Appaloft backend exposes a safe renderer URL | The Swarm service labels attach a Traefik `errors` middleware to served routers and point it at `/.appaloft/resource-access-failure` | Gateway-generated 404, 502, 503, and 504 responses from Swarm-served workloads are routed to the Appaloft diagnostic renderer instead of returning a bare provider page. |
