...
- Resource pages own Quick Deploy and deployment history actions.
...
| Web resource page | Resource detail defaults to overview/profile, owns deployment actions, deployment history, current health, access URL display, and promoted Environment/Dependencies/Domains/Monitor tabs. |
...
| PROJECT-CONSOLE-PROJECT-003 | e2e-preferred | Project all deployments | Project has deployments across resources | Project deployment view is labeled as rollup/all deployments | Deployment query filters by project and displays resource context | No Quick Deploy command without resource selection |
| PROJECT-CONSOLE-PROJECT-004 | e2e-preferred | Project Quick Deploy shortcut | User clicks project-level shortcut | Opens Quick Deploy or resource selection | Workflow eventually selects/creates resource before `deployments.create` | No direct project-owned deployment command |
...
| PROJECT-CONSOLE-RESOURCE-002 | e2e-preferred | Resource detail default tab | Resource exists | Overview/profile is the first selected tab; deployment history and logs are later tabs | Resource read model drives basic profile state and access URL | Deployment history is not the default application page |
| PROJECT-CONSOLE-RESOURCE-003 | e2e-preferred | Overview section tabs | Resource detail overview tab is open | Left overview navigation selects source/runtime/network/access, auto-deploy, storage, health, proxy, diagnostics, or danger-zone-adjacent content and replaces the right-side content panel | Section state is encoded in nested route/query state, not a hash anchor | The page does not scroll through one long configuration document |
| PROJECT-CONSOLE-RESOURCE-003A | e2e-preferred | Removed Settings compatibility | Legacy `tab=settings` or promoted `section=configuration/dependencies/domains/usage` URLs are opened | The page replaces them with Overview, Environment, Dependencies, Domains, or Monitor canonical URLs | No removed top-level Settings tab is rendered | Old settings links do not keep users on a dead tab |
...
| PROJECT-CONSOLE-RESOURCE-005 | e2e-preferred | Compact header actions | Resource exists with access URL, project, deployments, and domain placement | Header shows compact health and primary Quick Deploy action only | No command/query dispatched for removed navigation actions | Header does not expose open project, view deployments, open access URL, bind-domain, or diagnostic-copy as primary buttons |
...
| PROJECT-CONSOLE-RESOURCE-009 | e2e-preferred | Quick Deploy from resource | Resource exists and can deploy | Quick Deploy action is resource-scoped | Dispatch `deployments.create` with `resourceId` | Deployment attempt belongs to resource |
...
| PROJECT-CONSOLE-RESOURCE-011 | e2e-preferred | Terminal from resource | Resource has observable deployment workspace or retained runtime container target, and terminal access is enabled | Terminal is offered from an operational tab/action, not the default overview | Dispatch `terminal-sessions.open` with resource scope and attach returned transport | Terminal starts in resolved deployment workspace or retained runtime container target, not a resource-name directory |
...
| PROJECT-CONSOLE-RESOURCE-014 | e2e-preferred | Active deployment | Latest deployment non-terminal | Quick Deploy is blocked or explains active state | No deployment command until guard can pass | Latest status remains read-model projection |
