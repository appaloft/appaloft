...
| SELF-AUTH-ACTION-004 | HTTP/application contract | Scope mismatch | Valid token lacks endpoint, project, environment, resource, server, repository, preview workflow, or command scope for the request | Request rejects before package, source-link, profile, route, preview, or deployment mutation | `action_auth_forbidden`, `403`, phase `action-authorization` | Auth verify -> scope reject -> no command dispatch |
| SELF-AUTH-ACTION-004A | application/HTTP contract | Scope participates in target resolution | Valid token scope uniquely identifies project/environment/resource/server and the Action request omits Appaloft ids | Source-link deploy or server-config deploy resolves the target from token scope, then dispatches only ids-only deployment admission; explicit ids or source-link targets outside scope still return 403 before mutation | None or `action_auth_forbidden`, `403`, phase `action-authorization` | Auth verify -> scope facts -> source-link target command -> ids-only deployment |
...
- `packages/application/test/action-source-link-deployment.test.ts` covers `SELF-AUTH-ACTION-004A`
  and the target-resolution side of `SELF-AUTH-ACTION-004`: complete token scope can resolve
  source-link/server-config targets without Action ids, while explicit ids outside scope fail before
  deployment mutation with `action_auth_forbidden`.
