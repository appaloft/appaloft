...
| Cleanup/process | Close-event, explicit Resource-scoped delete, expiry, runtime cleanup, provider metadata cleanup, feedback update, retry attempts, manual-review state, and retention. |
...
| PG-PREVIEW-ENV-001C | integration | Legacy preview Resource root-list filtering | A compatibility Resource lives inside a preview-kind Environment | Default Resource list queries run for project or global console views | The preview Resource is omitted unless an explicit include-preview query is used for recovery | None |
| PG-PREVIEW-ENV-001D | integration | Legacy preview Resource delete recovery | A compatibility preview Resource has retained preview deployment/audit state and an operator confirms deletion | Resource deletion safety is evaluated | Preview-kind Environment Resources do not treat retained preview deployment/audit rows as product Resource deletion blockers | None |
