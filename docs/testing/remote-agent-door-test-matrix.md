...
| WS-REMOTE-PROGRESS-205 | TUI | automated | Ctrl+C / Ctrl+R never quit the occupancy TUI; they pass through to the nested agent; `q` / `^]` do not exit; unfocused `^q` is the documented quit | `apps/workspace-control-tui/src/lib.rs` |
