...
| WS-REMOTE-PROGRESS-197 | shell | automated | TTY occupancy writes alt-screen before sidecar lookup and before `reflect-metadata` / `run.ts`; first useful alt-screen is under 1s | `apps/shell/test/occupancy-cli-progress.test.ts`; `apps/shell/test/occupancy-tui-first-frame.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
...
| WS-REMOTE-PROGRESS-203 | shell | automated | parent occupancy first frame is `ESC[?1049h` + preparing the agent and happens before launch/sidecar resolve | `apps/shell/test/occupancy-tui-first-frame.test.ts` |
