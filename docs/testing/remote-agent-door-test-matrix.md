...
| WS-REMOTE-PROGRESS-197 | shell | automated | TTY occupancy warms the slim launch module before `reflect-metadata` and `run.ts`; first useful alt-screen is under 1s | `apps/shell/test/occupancy-cli-progress.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
