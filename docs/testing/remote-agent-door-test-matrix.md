...
| WS-REMOTE-PROGRESS-219 | CLI / TUI | automated | missing / stale Occupancy-chrome `appaloft-workspace-tui` leaves alt-screen before rustc/cargo or any error write; one human paragraph after restore names cargo build and does not require rustup; no glue onto `preparing the agent`; no Occupancy; no rustup cargo-chooser dump; JS warmup `^c` emits `1049l`; real TUI `^c` does not dump EPIPE | `packages/adapters/cli/test/workspace-tui-launch.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/shell/test/occupancy-tui-first-frame.test.ts`; `packages/adapters/cli/test/workspace-control-renderer.test.ts`; `packages/adapters/cli/test/workspace-control-real-renderer.test.ts` |
