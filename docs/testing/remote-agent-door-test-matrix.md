...
| WS-REMOTE-RESUME-004 | application | this slice | same subject resumes preferred Sandbox when origin matches; a no-git folder resumes the live occupancy; with no occupancy, Spec 142 occupies this folder |
| WS-REMOTE-OCCUPY-005 | application / persistence | this slice | second subject gets another preferred Sandbox |
| WS-REMOTE-NO-UPLOAD-006 | CLI | automated | local Git fail-closed not used on `code`; no-git folder resumes live occupancy; missing origin with no occupancy occupies this folder after onboarding |
...
| WS-REMOTE-PROGRESS-201 | CLI / TUI | automated | no-git folder resumes live occupancy without ancestor `ls-remote`; matching occupancy skips `ls-remote`; `--new` without a locator still fail-closes | `packages/adapters/cli/test/remote-code-session.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `packages/adapters/cli/test/workspace-control-presentation.test.ts` |
