...
| WS-REMOTE-PROGRESS-197 | shell | automated | TTY `workspace` writes alt-screen before sidecar lookup and before `reflect-metadata` / `run.ts`; TTY `code` does not enter alt-screen until after pre-TUI inquire; first useful alt-screen is under 1s | `apps/shell/test/occupancy-cli-progress.test.ts`; `apps/shell/test/occupancy-tui-first-frame.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
...
| WS-REMOTE-PROGRESS-219 | CLI / TUI | automated | missing or stale `appaloft-workspace-tui` is not launched for `code`; no Occupancy split; TTY is restored with one human message and no rustup dump | `packages/adapters/cli/test/workspace-tui-launch.test.ts` |
| WS-REMOTE-PROGRESS-203 | shell | automated | `workspace` parent first frame is `ESC[?1049h` + Appaloft Cloud Agents + preparing the agent before sidecar resolve; TTY `code` does not enter alt-screen at process start so inquire can run first | `apps/shell/test/occupancy-tui-first-frame.test.ts` |
