...
| WS-REMOTE-OPEN-BYOS-181 | CLI | this slice | `code` / `workspace open` / `create` pass enrolled BYOS or `--server` (same pin semantics); do not demand managed | `packages/adapters/cli/test/remote-code-session.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/shell/test/help-without-runtime.test.ts` |
