...
| WS-REMOTE-PROGRESS-194 | TUI / CLI | automated | TTY `code` opens collapsed occupancy TUI with Appaloft Cloud Agents + project and centered preparing-the-agent steps; first chrome is not Occupancy or the workspace-list copy; tree is hidden until restore the tree; attach replaces the wait; workspaces do not show the tree during the wait; first screen is one Checking login / Preparing skills / Preparing disk column (no duplicate Preparing skills); title is the full `Appaloft Cloud Agents` / `Appaloft Cloud Agents · <project>` string (not clipped to `Agen`); no Occupancy/sbx_ | `packages/adapters/cli/test/workspace-control-presentation.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
