...
| WS-REMOTE-VENDOR-204 | CLI | automated | `--claude/--codex/--grok` map onto existing harnesses; `--harness` remains the runtime override | `packages/adapters/cli/test/occupancy-vendor.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/shell/test/help-without-runtime.test.ts` |
| WS-REMOTE-VENDOR-205 | CLI | automated | default vendor follows laptop login then installed binary | `packages/adapters/cli/test/occupancy-vendor.test.ts` |
| WS-REMOTE-VENDOR-206 | CLI | automated | two vendor flags fail-closed | `packages/adapters/cli/test/occupancy-vendor.test.ts` |
| WS-REMOTE-VENDOR-207 | CLI | automated | `--harness` overrides runtime while vendor still selects the credential | `packages/adapters/cli/test/occupancy-vendor.test.ts` |
