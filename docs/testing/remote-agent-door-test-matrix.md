...
| WS-REMOTE-RESUME-004 | application | this slice | same subject resumes preferred Sandbox when origin matches; a no-git folder occupies this folder after Spec 142 onboarding and does not resume an unrelated occupancy |
| WS-REMOTE-OCCUPY-005 | application / persistence | this slice | second subject gets another preferred Sandbox |
| WS-REMOTE-NO-UPLOAD-006 | CLI | automated | local Git fail-closed not used on `code`; no-git folder occupies this folder after onboarding and does not resume an unrelated occupancy |
...
| WS-REMOTE-OPEN-COMPAT-011 | CLI/unit | `workspace open` locators | dirty worktree still `workspace_git_*`; non-git occupies from a git remote and does not resume an unrelated occupancy | `packages/adapters/cli/test/workspace-open-source.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts` |
...
| WS-REMOTE-CA-033 | CLI / unit + `appaloftdev` | this slice | headless `workspace --json` prints occupancy tree; unauthenticated no-profile + empty local backend is `login-required` before queries | `packages/adapters/cli/test/agent-workspace-command.test.ts`; live `appaloftdev workspace --json` |
...
| WS-REMOTE-CA-069 | TUI / unit | this slice | TUI list/detail show persisted display name and never paint `sbx_` | `workspace-control-tui` `ws_remote_ca_069_*`; `packages/core/test/sandbox-display-name.test.ts` |
...
| WS-REMOTE-HINT-140 | CLI / unit | this slice | occupancy GitHub PR hint follows the model hint | `remote-code-session.test.ts`; `agent-workspace-command.test.ts` |
...
| WS-REMOTE-COMPAT-221 | CLI / unit | this slice | Creating project / `deploy .` `operationCheckDenied` is a human next step (operation + reason + login/enroll/retry/Cloud update); logged-in create-project keeps handshake org when profile details were dropped; no Occupancy/`sbx_` | `packages/adapters/cli/test/runtime-error-format.test.ts`; `packages/adapters/cli/test/folder-project-onboarding.test.ts`; `packages/adapters/cli/test/control-plane-client.test.ts`; `packages/application/test/operation-guard.test.ts` |
| WS-REMOTE-COMPAT-222 | CLI / unit | this slice | occupy / Preparing disk HTTP 502/503 or Cloudflare bad-gateway / origin-incomplete humanizes to Cloud is temporarily unreachable (HTTP status kept); not the error-contract sentence, not Occupancy/`sbx_`, not a Cloudflare body dump, not `Opening folder.local/...`; `--omp` and `--pi` stay Occupancy-free. On the Cloud Agents wait panel this sentence may appear while retrying; it is not a terminal exhaust that leaves alt-screen | `packages/adapters/cli/test/remote-code-session.test.ts`; `packages/adapters/cli/test/runtime-error-format.test.ts`; `packages/adapters/cli/test/workspace-control-presentation.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `packages/adapters/cli/test/user-visible-occupancy-forbidden.test.ts` |
| WS-REMOTE-PROGRESS-223 | CLI / TUI | this slice | Preparing disk origin HTTP 502/503 / Cloudflare bad gateway / incomplete origin keeps the Cloud Agents wait panel up, marks the disk step retrying, and keeps retrying folder.local occupy (`OpenAgentWorkspaceCommand` → `sandboxes.create` on the enrolled hostinger Server) past a 4-attempt helper burst until attach + terminal-ready or the occupy deadline; no contract sentence, no `Opening folder.local/...`, and no alt-screen leave while retrying; occupy without a live session is not success; Occupancy/`sbx_` stay off; first paint stays Cloud Agents + preparing the agent | `packages/adapters/cli/test/remote-code-session.test.ts`; `packages/adapters/cli/test/workspace-control-presentation.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/workspace-control-tui` rust lib |
| WS-REMOTE-PROGRESS-224 | CLI / TUI | this slice | perpetual Preparing-disk 502/503 does not infinite-silent-spin; after the occupy deadline fail-closes with Cloud-unreachable / disk-prep, no `Opening folder.local/...`, non-zero exit | `packages/adapters/cli/test/remote-code-session.test.ts`; `packages/adapters/cli/test/workspace-control-presentation.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts` |
| WS-REMOTE-PROGRESS-225 | CLI / unit | this slice | in-flight occupy `open()` hang is abortable by signal / deadline, not only between attempts | `packages/adapters/cli/test/remote-code-session.test.ts`; `packages/adapters/cli/test/workspace-control-presentation.test.ts` |
| WS-REMOTE-PROGRESS-226 | CLI / TUI | this slice | Ctrl-C / quit on Preparing disk while never attached aborts the in-flight open and is not exit 0 success | `packages/adapters/cli/test/workspace-control-presentation.test.ts` |
...
| WS-REMOTE-OPEN-BYOS-181 | CLI | this slice | `code` / `workspace open` / `create` pass enrolled BYOS or `--server` (same pin semantics); folder.local `--pi --server` does not POST `/workspaces/open` and does not demand managed | `packages/adapters/cli/test/remote-code-session.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `packages/adapters/cli/test/folder-local-remote-open.test.ts`; `apps/shell/test/help-without-runtime.test.ts` |
...
| WS-REMOTE-PROGRESS-192 | CLI | automated | `--no-attach` prints Remote banner before skill copy; hung skill copy is fail-soft and does not block occupy; connecting steps still report actual copied `N` | `packages/adapters/cli/test/agent-workspace-command.test.ts`; `packages/adapters/cli/test/occupancy-code-progress.test.ts` |
| WS-REMOTE-PROGRESS-193 | TUI / CLI | automated | Linked TTY `code` / `code --pi` / `workspace` skip streamed line progress and enter Cloud Agents TUI; unlinked TTY `code` inquires before any alt-screen; TUI occupy bootstrap sends progress then attaches; `--pi` does not change chrome or TUI mode | `apps/shell/test/occupancy-cli-progress.test.ts`; `packages/adapters/cli/test/workspace-control-presentation.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-194 | TUI / CLI | automated | TTY `code` opens collapsed occupancy TUI with Appaloft Cloud Agents + project and centered preparing-the-agent steps; first chrome is not Occupancy or the workspace-list copy; tree is hidden until restore the tree; attach replaces the wait; workspaces do not show the tree during the wait; first screen is one Checking login / Preparing skills / Preparing disk column (no duplicate Preparing skills); product title is one full-width top-row paint of `Appaloft Cloud Agents` / `Appaloft Cloud Agents · <project>` (not clipped to `Agen` or `appaloft-clo`, not stacked over the wait panel); parent warmup uses that same top row; no Occupancy/sbx_ | `packages/adapters/cli/test/workspace-control-presentation.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/shell/test/occupancy-tui-first-frame.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-195 | CLI / shell | automated | source checkout finds or cargo-builds `appaloft-workspace-tui` | `packages/adapters/cli/test/workspace-control-renderer.test.ts` |
...
| WS-REMOTE-PROGRESS-197 | shell | automated | TTY `code` / `workspace` write alt-screen before sidecar lookup and before `reflect-metadata` / `run.ts`; first useful alt-screen is under 1s | `apps/shell/test/occupancy-cli-progress.test.ts`; `apps/shell/test/occupancy-tui-first-frame.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-202 | shell / CLI / TUI | automated | launch module has no presentation/application/core imports; existing binary skips cargo; default occupancy frame is collapsed preparing-the-agent | `apps/shell/test/occupancy-cli-progress.test.ts`; `packages/adapters/cli/test/workspace-tui-launch.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-219 | CLI / TUI | automated | missing / stale Occupancy-chrome `appaloft-workspace-tui` **or** a later occupy/renderer/control-plane failure after alt-screen leaves alt-screen (`ESC[?25h` `ESC[?1049l` + mouse off) before rustc/cargo or any `error:` write; occupy failure after the Cloud Agents wait is drawn restores then prints on the normal screen and does not write `error:` into the alt-screen; one human paragraph after restore names cargo build and does not require rustup; unstructured control-plane JSON includes HTTP status / remote code / truncated body; tiny-pty loading panel does not panic; no glue onto `preparing the agent`; no Occupancy; no rustup cargo-chooser dump; JS warmup `^c` emits `1049l`; real TUI `^c` does not dump EPIPE; warmup catch does not write stderr | `packages/adapters/cli/test/workspace-tui-launch.test.ts`; `packages/adapters/cli/test/workspace-control-presentation.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `packages/adapters/cli/test/control-plane-client.test.ts`; `packages/sdk/test/client-auth-error.test.ts`; `apps/shell/test/occupancy-tui-first-frame.test.ts`; `apps/shell/test/run-error-format.test.ts`; `packages/adapters/cli/test/workspace-control-renderer.test.ts`; `packages/adapters/cli/test/workspace-control-real-renderer.test.ts`; `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-203 | shell | automated | TTY `code` / `workspace` parent first frame is `ESC[?1049h` + Appaloft Cloud Agents + preparing the agent before sidecar resolve | `apps/shell/test/occupancy-tui-first-frame.test.ts` |
| WS-REMOTE-PROGRESS-204 | TUI | automated | nested OSC 52 is stripped from vt100, reassembled across frames, written to the host tty, and write failure names OSC 52 disabled without faking success or `pbcopy` | `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-205 | TUI | automated | In-session Ctrl+C / Ctrl+R pass through; `^c` quits only from the list/menu; `q` / `^]` / invented `^q` do not quit | `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-206 | TUI | automated | In-session `x` returns to Appaloft Cloud Agents + project, keeps the session, does not Quit, and has no user-visible Occupancy | `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-207 | TUI | automated | `?` shows the Railway `ca` key table plus source extras; n/c/t/setup/`⌥n`/`⌥p`/`^o` say unavailable; s/w/d map to pause/resume/terminate; `q` is not quit | `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-208 | TUI | automated | `j`/`k` move; `h`/`l` open/close unavailable; `⌥r` refresh from anywhere; `⌥enter` connect; `⌥[`/`⌥]` cycle already-open sessions with wrap and no wake; bare `f` fullscreen on the manage tree; `y`/`Y` only confirm delete | `apps/workspace-control-tui/src/lib.rs` |
| WS-REMOTE-PROGRESS-198 | CLI / shell | automated | missing `appaloft-workspace-tui` names the binary and `cargo build --locked` (Homebrew rustc/cargo; no `rustup default`); rustc 1.85 is called too old | `packages/adapters/cli/test/workspace-control-renderer.test.ts` |
...
| WS-REMOTE-PROGRESS-201 | CLI / TUI / application | automated | no-git folder occupies this folder without ancestor `ls-remote` and without resuming leftover `examples` / `truefile`; `--no-attach` from cwd (no path) shares the rule; matching occupancy skips `ls-remote`; folder occupancy does not `git init` / clone / materialize a missing remote and does not exit 1 for `workspace_open_source_materialization_failed`; leftover partial folder occupancy is repaired or replaced without `workspace_open_partial_recovery_required`; `code --no-attach` against a partial preferred workspace does not emit that error; `--new` without a locator still fail-closes; chrome never says Occupancy | `packages/adapters/cli/test/remote-code-session.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `packages/adapters/cli/test/folder-project-onboarding.test.ts`; `packages/adapters/cli/test/occupancy-code-progress.test.ts`; `packages/application/test/agent-workspace-open.test.ts` |
| WS-REMOTE-VENDOR-204 | CLI | automated | `--opencode/--pi/--omp/--claude/--codex/--grok` map onto existing harnesses; `--help` leads with aliases | `packages/adapters/cli/test/occupancy-vendor.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/shell/test/help-without-runtime.test.ts` |
| WS-REMOTE-VENDOR-205 | CLI | automated | default alias follows saved preference then laptop login then installed binary | `packages/adapters/cli/test/occupancy-vendor.test.ts` |
| WS-REMOTE-VENDOR-206 | CLI | automated | two agent aliases, or alias plus a different `--harness`, fail-closed | `packages/adapters/cli/test/occupancy-vendor.test.ts` |
| WS-REMOTE-VENDOR-207 | CLI | automated | `--harness` is compatibility for the matching harness alias; `--pi` occupies Pi | `packages/adapters/cli/test/occupancy-vendor.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts` |
| WS-REMOTE-CRED-208 | CLI | automated | Grok `auth.json` is written to occupancy HOME via `WriteSandboxFile` | `packages/adapters/cli/test/occupancy-credential-offer.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts` |
| WS-REMOTE-CRED-209 | CLI | automated | Codex `auth.json` is written to occupancy HOME | `packages/adapters/cli/test/occupancy-credential-offer.test.ts` |
| WS-REMOTE-CRED-210 | CLI | automated | Claude copies setup-token and skips `.credentials.json` | `packages/adapters/cli/test/occupancy-vendor.test.ts`; `packages/adapters/cli/test/occupancy-credential-offer.test.ts` |
| WS-REMOTE-CRED-211 | CLI | automated | credential is a HOME file; occupancy MCP/env carry no token | `packages/adapters/cli/test/occupancy-credential-offer.test.ts`; `packages/adapters/cli/test/occupancy-connecting-telemetry.test.ts` |
| WS-REMOTE-CRED-212 | CLI | automated | token values are not printed | `packages/adapters/cli/test/occupancy-connecting-telemetry.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts` |
| WS-REMOTE-CRED-213 | CLI | automated | explicit vendor without login fail-closes | `packages/adapters/cli/test/occupancy-vendor.test.ts` |
