...
| WS-REMOTE-PROGRESS-219 | CLI / TUI | automated | missing / stale / rustup-missing `appaloft-workspace-tui` is not launched for `code`; TTY is restored (`ESC[?25h` `ESC[?1049l` + mouse off) before the human message; no glue onto `preparing the agent`; no Occupancy; no rustup cargo-chooser dump | `packages/adapters/cli/test/workspace-tui-launch.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts` |
