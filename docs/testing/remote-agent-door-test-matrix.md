...
| WS-REMOTE-PROFILE-008 | application / runtime | this slice | missing default Profile installs `appaloft-remote`; OpenCode occupancy starts without a required model-api binding |
| WS-REMOTE-AUTH-009 | runtime | this slice | no model binding starts vendor-login OpenCode; teammate OAuth is not copied |
...
| WS-REMOTE-SKILL-017 | CLI / runtime | this slice | occupancy OpenCode serve config includes workspace skill paths; native attach uses `mcp remote-stdio` against the selected control plane |
