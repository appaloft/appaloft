...
| WS-REMOTE-PROGRESS-219 | CLI / TUI | automated | missing / stale / rustup-missing `appaloft-workspace-tui` leaves alt-screen before rustc/cargo or any error write; one human paragraph after restore; no glue onto `preparing the agent`; no Occupancy; no rustup cargo-chooser dump; warmup catch does not write stderr | `packages/adapters/cli/test/workspace-tui-launch.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts`; `apps/shell/test/occupancy-tui-first-frame.test.ts` |
...
| WS-REMOTE-PROGRESS-198 | CLI / shell | automated | missing `appaloft-workspace-tui` names the binary, `rustup default stable`, and `cargo build --locked` next command; rustc 1.85 is called too old | `packages/adapters/cli/test/workspace-control-renderer.test.ts` |
