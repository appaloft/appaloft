...
| WS-REMOTE-OPEN-COMPAT-011 | CLI/unit | `workspace open` locators | dirty worktree still `workspace_git_*`; non-git occupies from occupancy or git remote | `packages/adapters/cli/test/workspace-open-source.test.ts`; `packages/adapters/cli/test/agent-workspace-command.test.ts` |
