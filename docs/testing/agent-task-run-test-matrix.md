...
| AGENT-TASK-RESUME-002 | application/SDK/provider | resume finalization | Same Run and protected task state continue idempotently; replacement is atomic for concurrent observers, and a failed finalization stores a safe retryable reason. | `packages/application/test/agent-task-run.test.ts`; `packages/adapters/runtime/test/docker-sandbox-provider.test.ts`; `packages/sdk/test/agent-workspace-handles.test.ts` | automated |
