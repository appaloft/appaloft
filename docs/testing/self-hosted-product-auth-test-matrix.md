...
| FIRST-ADMIN-BOOTSTRAP-004 | auth-adapter | Better Auth implementation boundary | `@appaloft/auth-better` implements the Appaloft bootstrap port | Better Auth creates an email/password user and owner membership in the stable self-hosted organization tenant without Better Auth types leaking into application | None | Appaloft port -> Better Auth API |
...
| PRODUCT-AUTH-SIGNUP-001 | HTTP/Web contract | Product signup remains public | Bootstrap is complete and a browser has no product session | `/sign-up` is served as an ordinary account surface, `/api/auth/sign-up/email` is handled by the auth runtime without product-session auth, and the signup UI starts organization setup without first-admin bootstrap controls | None or auth runtime validation errors | Signup document request -> static route; signup API request -> auth runtime; organization setup request -> auth runtime |
...
| ORG-TEAM-ROLE-001 | core/application | Change member role | An owner updates a non-owner member role | Non-owner roles change through the generic role command; assigning `owner` or changing an existing owner is rejected and requires ownership transfer | None, `validation_error`, `invariant`, or `product_auth_forbidden` | Session verify -> member readback -> aggregate/application rule -> auth adapter |
| ORG-TEAM-REMOVE-001 | core/application | Remove member | An owner removes a non-owner member | Non-owner members are removed/deactivated through the generic remove command; removing an owner is rejected until ownership is transferred | None, `invariant`, or `product_auth_forbidden` | Session verify -> member readback -> aggregate/application rule -> auth adapter |
| ORG-TEAM-OWNER-TRANSFER-001 | core/application/HTTP/CLI/Web | Transfer owner | An owner selects another active member as the new owner | Ownership changes only through the dedicated transfer-owner command/API/CLI/Web control; the target becomes owner and the previous owner becomes admin | None, `validation_error`, `invariant`, or `product_auth_forbidden` | Session verify -> member readback -> ownership transfer command -> auth adapter |
...
| ORG-TEAM-DOCS-001 | docs/help | Organization/team docs | Team management is user-visible | Docs/help explain current context, invite, non-owner role update, ownership transfer, non-owner removal, and 401/403 recovery | None | Docs registry/help lookup |
| ORG-TEAM-WEB-001 | Web/source | Organization/team Web surface | A signed-in owner/admin opens `/organization` | Web reads current context, members, invitations, invite, non-owner role update, ownership transfer, and non-owner remove through shared oRPC contracts and i18n keys; owner rows do not show generic update/remove controls | None or product auth errors from the shared API | Web -> oRPC client -> application command/query |
...
| ACCOUNT-SETTINGS-PROFILE-001 | application/HTTP/Web | Account profile read/change | A signed-in product user opens `/account/profile` and changes display name or avatar URL | Safe account metadata is read and changed through `AccountSettingsPort`; email, password, sessions, organization state, provider payloads, and token material are unchanged and not exposed | `product_auth_missing`, `validation_error`, or adapter failure | Session verify -> account profile query/command -> auth adapter -> safe readback |
| ACCOUNT-SETTINGS-SESSION-001 | application/HTTP/Web | Account session list/revoke | A signed-in product user opens `/account/sessions` and revokes a session id visible to that user | Safe active session metadata is listed, Web/CLI/unknown client kind is exposed when known, current session is marked when detectable, and single-session revoke returns only safe revoked metadata without exposing session tokens | `product_auth_missing`, `product_auth_forbidden`, or adapter failure | Session verify -> sessions query -> revoke command -> auth adapter |
| ACCOUNT-SETTINGS-DANGER-001 | application/HTTP/Web | Account danger-zone deletion | A signed-in product user enters exact user-id confirmation in `/account/danger-zone` | The signed-in account is deleted through the auth adapter only when confirmation matches; organizations, projects, resources, deployments, deploy tokens, audit state, runtime state, and retained history are not cascaded | `product_auth_missing`, `product_auth_forbidden`, or adapter failure | Session verify -> profile readback -> exact confirmation -> delete account command |
| ORG-SETTINGS-PROFILE-001 | application/HTTP/Web | Organization profile read/change | A signed-in member opens organization profile settings and an owner/admin changes name, slug, or logo URL | Safe organization profile metadata is read; owner/admin changes profile metadata through Appaloft-owned organization settings methods without mutating members, invitations, deploy tokens, resources, or deployments | `product_auth_missing`, `product_auth_forbidden`, `validation_error`, or adapter failure | Session verify -> profile query -> admin profile command -> auth adapter |
| ORG-SETTINGS-DANGER-001 | application/HTTP/Web | Organization danger-zone deletion | A signed-in owner enters exact organization-id confirmation in `/organization/danger-zone` | Organization identity membership/profile data is deleted through the auth adapter only for owners; Appaloft project, resource, deployment, deploy-token, audit, runtime, and retained history state is not cascaded | `product_auth_missing`, `product_auth_forbidden`, or adapter failure | Session verify -> owner check -> exact confirmation -> delete organization command |
| SETTINGS-WEB-001 | Web/source | Account and organization settings sidebar | A signed-in user opens account or organization settings routes | Account and organization settings use a shared console-style settings sidebar with distinct localized item sets, shared oRPC contracts, account security endpoints where configured, organization profile summary metrics only on the profile route, route-scoped loading for organization members/invitations/deploy tokens, invitation and deploy-token collection pages show one empty state instead of inline empty lists/forms, create actions live in the page header and open URL-synchronized dialogs, and no Better Auth coupling in Web source | None or shared API errors rendered by pages | Web route -> SettingsShell -> settings nav -> route-scoped oRPC/auth-runtime calls |
...
- Product signup stays an ordinary account surface after bootstrap. `/sign-up` and
  `/api/auth/sign-up/email` are not product-session protected by the console document navigation
  gate; the Web signup page creates the account and starts organization setup through auth-runtime
  endpoints without linking to first-admin bootstrap, covering `PRODUCT-AUTH-SIGNUP-001`.
...
- ADR-045 and `docs/specs/054-self-hosted-organization-team-operations` now govern
  organization/team current context, member list, invitation, non-owner role update, ownership
  transfer, non-owner member removal, and the Better Auth adapter boundary.
...
- Public HTTP/oRPC routes for current context, current organization switching, member list,
  invitation list, invite, non-owner role update, ownership transfer, and non-owner removal are active behind `ProductSessionAuthorizationPort`, dispatch through
...
- Public docs coverage is active under `self-hosting.organization-team-management` for current
  context, current organization switching, member and invitation lists, invitation, non-owner role
  updates, ownership transfer, non-owner removal, CLI session handoff, safe outputs, owner recovery,
  and `401`/`403` recovery. It is verified by
