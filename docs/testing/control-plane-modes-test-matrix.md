...
| CONTROL-PLANE-INSTALL-007 | workflow contract | Repository console preview workflow deploys a PR-scoped console backend | A same-repository pull request changes Web console, shared frontend contracts, shell/backend code, or repository deploy plumbing | `.github/workflows/deploy-console-preview.yml` classifies the change, deploys the PR source with the repository `Dockerfile` as an application Resource through the CLI to an SSH target with `ssh-pglite`, serves Web assets and `/api` from the same preview origin, configures preview-scoped auth origin values through trusted environment flags, publishes a PR-scoped preview URL, uses a backend-preview config fingerprint so it does not drift against earlier static preview Resources, cleans both legacy static and backend preview fingerprints before redeploying a synchronized PR, uses a console-specific GitHub concurrency group so required docs previews do not cancel the console preview job, relies on SSH remote-state locks for the shared state root, and cleans up both backend and legacy static preview fingerprints when the PR closes. | Missing SSH variable/secret, runtime capacity/state preflight failure, image build failure, health timeout, or cleanup failure | Change classification -> trusted settings -> remote preflights -> legacy static preview cleanup -> backend preview cleanup -> backend Dockerfile CLI deploy -> same-origin preview URL output -> PR close cleanup |
