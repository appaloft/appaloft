...
| CONTROL-PLANE-HANDSHAKE-017 | integration | Server config deploy applies config through explicit operations | Source package and config are valid and trusted source context resolves existing or bootstrap-allowed identity | Server applies resource/environment/profile changes, plain env values, resolved `ci-env:` secret references, and managed domains through explicit commands and then dispatches ids-only `deployments.create` | Missing required `ci-env:` values or unsupported secret resolvers fail before mutation with `validation_error`, phase `config-secret-resolution` | Handshake -> package accepted -> config bootstrap -> source-link/context resolution -> profile/env/secret/route operations -> `deployments.create` |
