...
| CONTROL-PLANE-HANDSHAKE-012 | wrapper contract | Self-hosted Action preview deploy trigger | `control-plane-mode: self-hosted` is selected with URL, `preview=pull-request`, `preview-id`, and either trusted ids or an existing preview source link | Action derives a preview-scoped source fingerprint, performs `/api/version`, then `POST /api/action/deployments/from-source-link`, writes preview id, deployment id, and console URL outputs, and does not invoke CLI or SSH | None, or validation when direct preview route/profile inputs are supplied | Version check -> preview source-link deployment API request |
