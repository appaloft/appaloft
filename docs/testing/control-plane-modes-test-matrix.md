...
| CONTROL-PLANE-CLI-012 | CLI/unit | Login defaults to Appaloft Cloud browser auth exchange | No `--url` is supplied and no local env credential exists | `appaloft login` or `appaloft auth login` selects `https://app.appaloft.com`, creates a CLI auth session, prints `verificationUriComplete` and the user code with an `https` Cloud authorize URL, waits for explicit Enter before opening the browser when browser opening is enabled, polls pending then authorized, exchanges once, verifies current organization context, writes the active `cloud` profile, and does not print raw credential material. `login --help` and `login -h` print usage and exit 0 without starting OAuth. | Browser-open failure is non-fatal and prints the URL/code; profile write happens only after exchange and current-context verification; Cloud `http://app.appaloft.com` authorize URLs are rewritten to `https` while custom `--url` schemes are preserved | Default Cloud endpoint -> auth session create -> print/confirm/open -> poll -> exchange -> version/current-context -> profile store write |
...
| CONTROL-PLANE-CLI-021 | CLI/adapter integration | Bounded transient gateway retry | A catalog-classified query receives an HTML 502, 503, or 504 response from the selected control plane | The CLI replays the same authenticated typed query once and returns the successful structured result when the second response succeeds; commands are never automatically replayed | An exhausted query returns `control_plane_unexpected_html_response`, `retryable = true`, and safe request/status/content metadata without response body or credential material | Catalog kind check -> first typed query -> one bounded retry on transient HTML gateway response -> result or structured retryable error |
...
- mode/profile mismatch and explicit local-only deploy unsupported checks under
...
OS keychain credential storage, source-package quick deploy, other remote streaming/watch behavior,
MCP exposure, broader OIDC provider flows, and SSH PGlite adoption remain governed follow-ups.
