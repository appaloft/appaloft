...
| CONTROL-PLANE-INSTALL-007 | workflow contract | Repository console preview workflow deploys Web static assets | A same-repository pull request changes Web console, shared frontend contracts, or repository deploy plumbing | `.github/workflows/deploy-console-preview.yml` classifies the change, builds `apps/web` with an explicit `VITE_APPALOFT_API_BASE_URL`, deploys `apps/web/build` as a static-site Resource through the CLI to an SSH target with `ssh-pglite`, publishes a PR-scoped preview URL, and cleans up through the same preview fingerprint when the PR closes. | Missing SSH variable/secret, missing preview API base URL, runtime capacity/state preflight failure, static build failure, deploy failure, or cleanup failure | Change classification -> trusted settings -> remote preflights -> static CLI deploy -> preview URL output -> PR close cleanup |
...
`scripts/test/deploy-console-preview-workflow.test.ts` covers the repository static Web console
preview workflow (`CONTROL-PLANE-INSTALL-007`), including the dedicated preview config fingerprint,
explicit API base URL, CLI static deployment, and PR cleanup path.
