...
admission. The same row now covers plain `env` bootstrap through `environments.set-variable` and
managed `access.domains[]` bootstrap through `domain-bindings.create` before deployment admission,
including destination lookup from resource context and server proxy kind lookup. Unsupported
source/secret fields still fail before mutation with `profile-application`.
