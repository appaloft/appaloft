...
| New resource before first deploy | Web or CLI | Source plus new resource draft and runtime profile draft | Deployment request accepted | None | `resources.create` -> `deployments.create(resourceId)` | Resource exists before deployment admission with source/runtime profile | Retry follows resource/deployment rules |
...
| Source/runtime draft supplied | Web or CLI | Source locator/source descriptor plus runtime plan strategy hint | Deployment request accepted when compatible | None | Context commands -> `resources.create(source, runtimeProfile)` -> `deployments.create(resourceId)` | Resource owns source/runtime profile; deployment carries resolved runtime plan snapshot | Deployment retry creates new attempt |
...
| New resource | `resources.create` | `resources.create` | Explicit `resources.create` before deployment |
...
Current Web QuickDeploy orchestration lives in `QuickDeploySheet.svelte`, so initial Web tests may assert component-level dispatch behavior.

Current CLI interactive deploy orchestration lives in `deployment-interaction.ts`, so initial CLI tests may assert the resolved `CreateDeploymentCommandInput`.
...
Current Web and CLI Quick Deploy flows call `resources.create` with source/runtime profile before ids-only `deployments.create` when they create a new resource.
...
- Should Web and CLI share a reusable Quick Deploy workflow helper after `resources.create` exists, or remain separate entry implementations governed by the same spec?
