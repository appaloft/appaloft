...
| QUICK-DEPLOY-WF-067 | local explicit real Docker e2e | Config-only redeploy changes the running workload | CLI or automation | A successful local Docker deployment exists; the resource runtime variable snapshot changes while source stays unchanged | Redeploy succeeds only after a new deployment attempt starts a distinct container whose running process serves the updated runtime value | Structured deployment or runtime failure; an unchanged value must fail the smoke | `resources.set-variable` -> `deployments.redeploy(sourceDeploymentId)` -> container-local HTTP readback | First and second deployment ids and container ids differ; readback changes from `v1` to `v2`; both attempts report `succeeded` | Per redeploy |
