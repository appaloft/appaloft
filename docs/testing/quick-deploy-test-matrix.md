...
| Web workflow step progress | Web QuickDeploy | New context and resource draft | Each in-flight request shows loading and succeeded requests show complete | Failed command error shown on failed step | One typed oRPC request per yielded workflow step; final call is `deployments.create`, not `deployments.createStream` | Workflow progress stops at the failed step or stays visible after accepted deployment | Per command |
...
Current Web QuickDeploy renders per-step workflow progress and uses the normal `deployments.create` request for deployment admission. It does not use `deployments.createStream` as the workflow execution path.
