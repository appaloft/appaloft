...
| QUICK-DEPLOY-WF-045 | e2e-preferred | Repository config as non-interactive Quick Deploy profile | CLI, local Web agent, or headless binary | Source root has a valid Appaloft config profile with runtime/network/health fields and no self-hosted `controlPlane.deploymentContext` | Workflow auto-creates or selects project/resource outside the file, maps profile fields to the same resource-owned commands as interactive Quick Deploy, and accepts deployment | None | Config discovery -> identity resolution outside config -> shared Quick Deploy operation order -> `projects.create` if needed -> `resources.create(source/runtime/network/health)` -> `deployments.create(resourceId)` | Config changes cannot retarget project/resource in non-server config mode; deployment input stays ids-only | Per command |
| QUICK-DEPLOY-WF-046 | e2e-preferred | Repository config identity rejected | CLI or local Web agent | Config contains `project`, `projectId`, `resourceId`, target host, server id, destination id outside `controlPlane.deploymentContext`, or credential identity | Workflow fails before mutation | `validation_error`, phase `config-identity` | Config validation only | No project/resource/server/deployment mutation | No |
...
| QUICK-DEPLOY-WF-050 | e2e-preferred | Repository config named profile overlay | CLI or local Web agent | Config has a base profile plus `profiles.<key>`, and trusted entrypoint input selects `--config-profile <key>` | Overlay applies only after identity has been selected outside the file; final command remains ids-only | None | Config merge -> explicit operations -> `deployments.create` | Config overlay cannot move deployment to another environment by itself | Per command |
...
| QUICK-DEPLOY-WF-062 | shared workflow + CLI/Web focused tests | URL-first completion output | CLI, Web, or future tool entry | Deployment is accepted and access/read-model state can produce a URL or a structured unavailable reason | Result leads with URL/access state, Resource, Deployment, logs, diagnostics, and recovery hints; Project/Environment/Target details remain secondary | None or structured access-unavailable source error | Context commands -> `resources.create` if needed -> `deployments.create` -> event/access/log/diagnostic observation | No new business operation; read models remain the source for URL/log/diagnostic state | Per deployment/read-model refresh |
| QUICK-DEPLOY-WF-063 | shared workflow + CLI focused tests | Local static output as source | CLI or local Web agent | User supplies an already-built local static output directory such as `dist` or `build` | Workflow normalizes the local output into a static Resource profile and deploys it through the BYOS runtime target | Input validation error for missing/unsafe/empty output directory | Local path validation/materialization -> `resources.create(kind = static-site, runtimeProfile.strategy = static, publishDirectory)` -> `deployments.create(resourceId)` | Static output is packaged into Appaloft-owned Docker/OCI static-server artifact; no hosted artifact route is created | Per deployment |
| QUICK-DEPLOY-WF-064 | contract | No implicit hosted artifact serving | CLI, Web, or future tool entry | User chooses the default upload-like static output path | Deployment completes or fails using the selected local/SSH/cluster runtime target only | None unless selected target/runtime fails | Explicit operations -> BYOS runtime adapter execution | No Appaloft-owned cloud artifact storage, hosted gateway route, or deployment command source/runtime fields are used | No, unless a future hosted mode is explicitly selected |
| QUICK-DEPLOY-WF-065 | docs/skill contract | Agent-safe deployment protocol | Agent Deploy Skill, CLI docs, or public docs | Agent receives a request to deploy a source or static output | Guidance requires source inspection, secret/cache exclusion, explicit operation sequencing, progress observation, and URL plus diagnostics reporting before MCP is required | Stable validation/security errors when unsafe inputs are detected | Inspect -> select/create context -> resource profile command -> `deployments.create` -> observation queries | No agent-only business semantics or hidden repository/use-case access | Per command |
| QUICK-DEPLOY-WF-066 | contract | Durable monitoring references | Web, CLI, or automation | Workflow receives an accepted deployment result | Outcome includes `deploymentId` plus machine-readable `appaloft work list --deployment-id`, optional `appaloft work show <workId>`, and `deployments timeline <deploymentId> --follow` commands | None | Context commands -> `deployments.create` -> outcome packet | Later monitoring is anchored by deployment/durable work ids, not by the original HTTP request, modal, or process memory | Per deployment |
| QUICK-DEPLOY-WF-067 | contract | Reuse loopback local-shell Server | CLI non-TTY deploy | Enrolled `local-shell` Server uses `localhost`; headless deploy defaults to `127.0.0.1:22` | Workflow reuses the enrolled this-Mac Server and does not dispatch `servers.register` | `cloud-plan-limit-exceeded` must not appear only because loopback names differ | List servers -> reuse existing `local-shell` loopback Server -> `deployments.create` | No second this-Mac Server is created; occupancy/deploy stays on the enrolled Server | Per deployment |
...
| QUICK-DEPLOY-ENTRY-008B | CLI + unit | Auto static when `public/index.html` exists | Not applicable unless a local Web agent shells out to the CLI | `deploy . --yes` with no `--method` / `--publish-dir` auto-selects static and `publishDirectory` `public` when `public/index.html` exists. `--method static` / `--as static-site` without `--publish-dir` uses the same default (`public` if present, otherwise `.` / `/`). Source locator keeps the real cwd folder, including hyphenated names such as `nux-d73d53b6-static` (non-git) under a parent named `projects`. The required live case is that small no-git leaf; a git checkout named `appaloft-cloud` is `DEP-CREATE-PKG-008` and is not a substitute PASS. A prettier summary.Source is not a pass if the worker package path is still the parent. The create-resource send fields (`locator`, `originalLocator`, `cliResolvedSource`, `cliPackedSourceTarGz`) keep that leaf; a prettier summary.Source is not a pass if those fields are already the `projects` parent. CLI snapshots that leaf (not the parent) and packages it on the CLI host into `cliPackedSourceTarGz` before detached `deployments.create`. The static plan stamps that archive onto `execution.metadata`. The worker applies that archive before any `existsSync` of a Mac path and must not report `Source working directory does not exist: /Users/nichenqin/projects`. Reconstructing the leaf from `context.resourceName` then `existsSync` of the Mac folder is not a pass. Live shape: CLI cwd = existing hyphenated folder with `public/index.html`; locator/workdir already the parent; displayName `workspace`/`projects` or omitted; `source.metadata` empty; worker cwd=runtimeDir → archive contents come from the hyphenated folder. When the CLI-host folder exists, prepare/tar reads it and the deployment can report a live URL; failures stay non-zero with no fake URL. `recoverLocalSourceFolderFromCwd` must refuse generic parent basenames. Detector must not clobber a hyphenated displayName with `basename(parent)` and must retain the packed archive. Root `index.html` or explicit `--publish-dir .` stays a legal source-root value on the public validator (wire `/`). Must not silently reuse whoami. | API/automation callers send an explicit static profile |
...
| QUICK-DEPLOY-ENTRY-016 | e2e-preferred | URL-first result parity | Shows URL/access state first when available, with resource/deployment/logs/diagnostics/recovery links nearby | Prints URL/access state first when available, with equivalent resource/deployment/logs/diagnostics/recovery commands | API/automation receives or composes the same information from operation results and read queries; no Quick Deploy-only response shape is required |
| QUICK-DEPLOY-ENTRY-017 | contract + e2e-preferred | Local static output entry parity | May accept a local static output selection only through an agent/local-web capable path that can materialize files safely | May accept a local static output directory and normalize it into the static Resource profile path | Remote API callers must provide an already materialized source or explicit Resource profile; API must not accept arbitrary local filesystem paths |
| QUICK-DEPLOY-ENTRY-018 | unit + Web focused test | Ready GitHub connection defaults to repository browsing | When the configured user OAuth or hosted provider App connection can browse repositories, untouched Quick Deploy state with no entered locator opens repository browsing; an explicit URL-mode choice or existing locator remains unchanged | CLI remains source-locator/config driven and does not infer provider authorization | Integration queries and repository browsing stay explicit public operations; no hosted-provider-specific deploy command is added |
...
`QUICK-DEPLOY-WF-062` through `QUICK-DEPLOY-WF-065` and `QUICK-DEPLOY-ENTRY-016` through
`QUICK-DEPLOY-ENTRY-017` are future URL-first entry experience rows governed by
[URL-First Deployment Entry Experience](../specs/071-url-first-deployment-entry-experience/spec.md).
`QUICK-DEPLOY-WF-065` is also governed by
[Appaloft Agent Deploy Skill](../specs/072-appaloft-agent-deploy-skill/spec.md), because the v1
agent-friendly surface is a skill before MCP.
They are not current implementation claims.
...
UX, stored/external secret lookup/application, and profile drift Code Round coverage remain
follow-up. Profile drift behavior is specified in
