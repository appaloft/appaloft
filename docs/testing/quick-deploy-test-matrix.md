...
| QUICK-DEPLOY-WF-066 | contract | Durable monitoring references | Web, CLI, or automation | Workflow receives an accepted deployment result | Outcome includes `deploymentId` plus machine-readable `operator-work list --deployment-id`, optional `operator-work show <workId>`, and `deployments events <deploymentId> --follow` commands | None | Context commands -> `deployments.create` -> outcome packet | Later monitoring is anchored by deployment/durable work ids, not by the original HTTP request, modal, or process memory | Per deployment |
