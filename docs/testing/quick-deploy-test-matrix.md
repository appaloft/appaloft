...
| QUICK-DEPLOY-ENTRY-008B | CLI + unit | Auto static when `public/index.html` exists | Not applicable unless a local Web agent shells out to the CLI | `deploy . --yes` with no `--method` / `--publish-dir` auto-selects static and `publishDirectory` `public` when `public/index.html` exists. `--method static` without `--publish-dir` uses the same default (`public` if present, otherwise `.` / `/`). Source locator and later SSH workdir keep the real cwd folder, including hyphenated names; they must not dirname a missing folder to its parent, including when `execution.workingDirectory` is already that parent. CLI snapshots the resolved cwd into first-class `source.locator`, typed `source.originalLocator`, and `execution.workingDirectory`. Remote `deploy .` runs create-deployment detached, so do not depend on `metadata.cliResolvedSource`. Live shape: locator and workdir already the parent, displayName `workspace`/`projects` or omitted, metadata empty of cliResolvedSource → exists-check/tar use originalLocator or the snapshotted CLI cwd. `recoverLocalSourceFolderFromCwd` must refuse generic parent basenames (`projects`, `workspace`, `src`, `$HOME` leaf, `.`). Detector must not clobber a hyphenated displayName with `basename(parent)`. When the hyphenated cwd exists on the package host, tar packages that folder. A missing-workdir error must name the leaf, not only the parent. Root `index.html` or explicit `--publish-dir .` stays a legal source-root value on the public validator (wire `/`). Must not silently reuse whoami. | API/automation callers send an explicit static profile |
