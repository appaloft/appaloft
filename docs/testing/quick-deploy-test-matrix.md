...
| QUICK-DEPLOY-ENTRY-008B | CLI + unit | Auto static when `public/index.html` exists | Not applicable unless a local Web agent shells out to the CLI | `deploy . --yes` with no `--method` / `--publish-dir` auto-selects static and `publishDirectory` `public` when `public/index.html` exists. `--method static` without `--publish-dir` uses the same default (`public` if present, otherwise `.` / `/`). Source locator and later SSH workdir keep the real cwd folder, including hyphenated names; they must not dirname a missing folder to its parent, including when `execution.workingDirectory` is already that parent. When locator and workdir are both already the parent and `displayName` is omitted, the static plan and SSH package exists-check/tar use the persisted CLI-resolved `deploy .` path (the same path printed as summary.Source), not the locator and not a runtimeDir cwd. Locator is not a substitute for that path. Reused static resources refresh source when that metadata is missing. When the hyphenated cwd exists on the package host, tar packages that folder. Root `index.html` or explicit `--publish-dir .` stays a legal source-root value on the public validator (wire `/`). Must not silently reuse whoami. | API/automation callers send an explicit static profile |
