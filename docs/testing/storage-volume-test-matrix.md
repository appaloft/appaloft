...
| STOR-BACKUP-OFFSITE-PLAN-001 | S3-compatible storage backup plan | Application/runtime unit | A Docker volume source adapter and registered `s3-compatible` target are available. | Source selection depends on data format and consistency rather than target provider, and the plan reports `localOnly = false`. | `packages/application/test/storage-volume-backup-contract.test.ts`; `packages/adapters/runtime/test/storage-volume-backup-provider.test.ts` |
| STOR-BACKUP-OFFSITE-STORE-001 | S3-compatible storage backup create | Runtime unit | A source artifact exists on a local-shell or generic-SSH runtime target and a broker grants short-lived upload authorization. | Runtime verifies size/checksum, uploads the artifact, returns only the safe artifact handle, and removes the source working file. | `packages/adapters/runtime/test/storage-volume-backup-provider.test.ts` |
| STOR-BACKUP-OFFSITE-RESTORE-001 | S3-compatible storage backup restore | Application/runtime unit | A ready offsite backup has a safe artifact handle and checksum. | Runtime downloads through short-lived authorization, verifies checksum, restores to a new Docker volume, and removes the downloaded working file. | `packages/application/test/storage-volume-backup-restore.test.ts`; `packages/adapters/runtime/test/storage-volume-backup-provider.test.ts` |
| STOR-BACKUP-OFFSITE-PRUNE-001 | S3-compatible storage backup prune | Application/runtime unit | A retained offsite backup is pruned. | The exact provider object is deleted before the backup aggregate is marked pruned; cleanup failure leaves state unpruned. | `packages/application/test/storage-volume-backup-restore.test.ts`; `packages/adapters/runtime/test/storage-volume-backup-provider.test.ts` |
| STOR-BACKUP-OFFSITE-SECRET-001 | S3-compatible transfer authorization safety | Runtime unit | The broker issues signed upload/download URLs and required headers. | Signed URLs, credentials, secret headers, cookies, and private keys are not persisted or emitted and are redacted from command errors. | `packages/adapters/runtime/test/storage-volume-backup-provider.test.ts` |
