...
| CI-LIGHTWEIGHT-004 | `workflow_dispatch` classifies against the default branch the same way as pull requests, so a version-bump dispatch cannot force full CI. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts` and `scripts/check-ci-test-boundaries.ts`. |
| CI-LIGHTWEIGHT-005 | `ci.yml` and `e2e.yml` share `scripts/ci/classify-changed-files.ts`. Lightweight runs still publish required checks named `ci`, `e2e (1, 2)`, and `e2e (2, 2)`. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts` and `scripts/test/check-ci-test-boundaries.test.ts`. |
