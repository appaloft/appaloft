...
| RELEASE-HARDENING-009 | Every pull request publishes both stable E2E shard checks so branch protection can require them without leaving workflow-only, documentation-only, or release-please version-bump changes permanently pending. | GitHub Actions merge-gate contract | Automated in `scripts/test/select-shell-e2e-shard.test.ts` and `scripts/test/classify-changed-files.test.ts`; `.github/workflows/e2e.yml` must keep the pull-request trigger free of path filters, retain the two-shard matrix, and still report `e2e (1, 2)` and `e2e (2, 2)` as success when real shards are skipped. |
...
| CI-LIGHTWEIGHT-003 | PRs that touch product source under `apps/`, `packages/`, `skills/` (non-markdown), lockfiles, or Rust besides the two desktop version files stay on full CI (`lightweight_only=false`). Isolated web or shell E2E surfaces may still scope E2E; anything that is not proven isolated stays `e2e_full`. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
...
| CI-E2E-SCOPE-003 | Packages, shell/server composition, lockfiles, shared config, desktop, CI scripts, e2e harness/support files, mixed web+shell surfaces, or an empty diff classify as `e2e_full` and run the current WebView plus both shell shards. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-E2E-SCOPE-004 | `e2e.yml` keeps job `e2e` and matrix `shard: [1, 2]` / `shard_total: [2]` so required names stay `e2e (1, 2)` and `e2e (2, 2)`. Real work is skipped inside those jobs. Postgres is a step, not a job-level `services:` block. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`, `scripts/test/select-shell-e2e-shard.test.ts`, and `scripts/test/check-ci-test-boundaries.test.ts`. |
