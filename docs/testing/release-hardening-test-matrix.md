...
| RELEASE-HARDENING-009 | Every pull request publishes both stable E2E shard checks so branch protection can require them without leaving workflow-only or documentation-only changes permanently pending. | GitHub Actions merge-gate contract | Automated in `scripts/test/select-shell-e2e-shard.test.ts`; `.github/workflows/e2e.yml` must keep the pull-request trigger free of path filters and retain the two-shard matrix. |
