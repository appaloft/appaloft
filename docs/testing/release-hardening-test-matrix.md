...
| RELEASE-HARDENING-006 | Local, Docker, Compose, prebuilt image, static, full framework fixture Docker, full framework fixture SSH, aggregate framework, scheduled-task Docker, scheduled-task SSH, aggregate scheduled-task, storage-cleanup Docker, storage-cleanup SSH, aggregate storage-cleanup, storage-backup Docker, storage-backup SSH, aggregate storage-backup, runtime-usage Docker, runtime-usage SSH, aggregate runtime-usage, capacity-prune local, capacity-prune SSH, aggregate capacity-prune, GitHub preview provider smoke, dependency Redis backup smoke, Swarm, install-auth, SSH preflight, aggregate SSH, public launch basic Docker, public launch GitHub repository, public launch scheduled task, and redacted SSH evidence-capture smoke commands remain first-class package scripts and release workflow gates for release readiness. Publishing Release calls explicitly confirm all three live public-launch reusable workflows, while direct manual smoke dispatch remains fail-closed without confirmation. | Smoke command contract | Automated in `scripts/test/release-build-workflow.test.ts` by checking package scripts, public launch smoke entrypoints and reusable confirmation inputs, Release caller confirmation, framework fixture smoke entrypoints, scheduled-task real runtime smoke entrypoints, storage cleanup real runtime smoke entrypoints, storage backup real runtime smoke entrypoints, runtime usage real target smoke entrypoints, capacity prune real target smoke entrypoints, GitHub preview provider smoke entrypoints, dependency Redis backup smoke entrypoints, SSH preflight fail-closed behavior, evidence redaction, and workflow references including `public-launch-basic-docker-smoke.yml`, `public-launch-github-repo-smoke.yml`, `public-launch-cron-smoke.yml`, `storage-cleanup-e2e.yml`, `storage-backup-e2e.yml`, `runtime-usage-e2e.yml`, `capacity-prune-e2e.yml`, `preview-provider-e2e.yml`, and `dependency-redis-backup-e2e.yml`; real execution remains environment-gated where Docker/SSH/GitHub provider credentials or service containers are required. |
...
| RELEASE-HARDENING-008 | After the governed GA phase is complete, stable releases can continue within the same major line without inventing a roadmap phase for every minor or patch, while a new major still requires an explicit roadmap target. | Post-GA roadmap release gate | Automated in `scripts/test/release-build-workflow.test.ts` by accepting `1.1.0` against the completed `1.0.0` GA gate and rejecting `2.0.0` when no matching roadmap phase exists. |
| RELEASE-HARDENING-009 | Every pull request, including drafts, publishes both stable E2E shard checks so branch protection can require them without leaving workflow-only, documentation-only, release-please version-bump, or draft changes permanently pending. | GitHub Actions merge-gate contract | Automated in `scripts/test/select-shell-e2e-shard.test.ts` and `scripts/test/classify-changed-files.test.ts`; `.github/workflows/e2e.yml` must keep the pull-request trigger free of path filters, must not job-level skip drafts, retain the two-shard matrix, and still report `e2e (1, 2)` and `e2e (2, 2)` as success when real shards are skipped. |
| CI-LIGHTWEIGHT-001 | Docs-only, markdown, LICENSE, and `.github/workflows/*` PRs classify as `docs_only` and set `lightweight_only=true`. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-LIGHTWEIGHT-002 | Release-please / version-bump PRs whose files are only the docs set plus `.github/.release-please-manifest.json`, `package.json`, `apps/desktop/src-tauri/Cargo.toml`, and `apps/desktop/src-tauri/tauri.conf.json` classify as `release_bump` and set `lightweight_only=true`. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-LIGHTWEIGHT-003 | PRs that touch product source under `apps/`, `packages/`, `skills/` (non-markdown), lockfiles, or Rust besides the two desktop version files stay on full CI (`lightweight_only=false`). Isolated `apps/web/**` is `e2e_web`. Product files that do not touch `apps/web/**` classify as `e2e_shell`. Mixed web+product or empty diffs stay `e2e_full`. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-LIGHTWEIGHT-004 | `workflow_dispatch` classifies against the default branch the same way as pull requests. `release.yml` does not dispatch `ci.yml`; the align commit is pushed with `RELEASE_PLEASE_TOKEN` so `pull_request` checks run on HEAD. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`, `scripts/test/release-build-workflow.test.ts`, and `scripts/check-ci-test-boundaries.ts`. |
| CI-LIGHTWEIGHT-005 | `ci.yml` and `e2e.yml` share `scripts/ci/classify-changed-files.ts`. Lightweight runs still publish required checks named `ci`, `e2e (1, 2)`, and `e2e (2, 2)` by skipping inside the E2E matrix jobs and treating `skipped` CI jobs as success. Workspace TUI jobs are not skipped at the job level. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts` and `scripts/test/check-ci-test-boundaries.test.ts`. |
| CI-E2E-SCOPE-001 | Isolated `apps/web/**` changes classify as `e2e_web`, run mocked WebView on shard 1, skip shell E2E and Postgres, and still publish `e2e (1, 2)` and `e2e (2, 2)`. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-E2E-SCOPE-002 | Isolated `apps/shell/test/e2e/*.e2e.ts` changes classify as `e2e_shell`, run both shell shards, skip WebView, and still publish `e2e (1, 2)` and `e2e (2, 2)`. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-E2E-SCOPE-003 | Mixed `apps/web/**` plus other product files, an empty diff, `workflow_dispatch`, or an `e2e` label classify as `e2e_full` and run the current WebView plus both shell shards. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-E2E-SCOPE-004 | `e2e.yml` keeps job `e2e` and matrix `shard: [1, 2]` / `shard_total: [2]` so required names stay `e2e (1, 2)` and `e2e (2, 2)`. The job has no job-level `if:` (drafts included). Real work is skipped inside those jobs. Postgres is a step, not a job-level `services:` block. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`, `scripts/test/select-shell-e2e-shard.test.ts`, and `scripts/test/check-ci-test-boundaries.test.ts`. |
| CI-E2E-SCOPE-005 | CLI / adapter-cli / setup-help / inquire and other product file lists that do not touch `apps/web/**` classify as `e2e_shell`: no Chromium, no Web WebView Smoke, shell HTTP e2e only. Workspace TUI classification stays on its existing path filter. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-UNIT-SCOPE-001 | Product PRs, including CLI-only file sets, still publish the required check named `Unit Tests`. That job runs the canonical `bun run test` graph (packages, PGlite unit suites, web vitest) and must not invoke `@appaloft/web` `test:e2e` / `test:e2e:webview`. WebView stays in `e2e.yml`. Docs/release-bump PRs still echo-skip inside the same job name. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts` and `scripts/test/check-ci-test-boundaries.test.ts`. |
| CI-TUI-SCOPE-001 | Docs-only, release-bump, isolated `apps/web/**`, isolated shell CLI e2e files, most TypeScript packages, and unrelated CLI sources classify as `tui_skip` and echo-skip cargo/docker/bridge work inside every Workspace TUI matrix job. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-TUI-SCOPE-002 | `apps/workspace-control-tui/**`, JS bridge files the TUI job tests, packaging that copies `appaloft-workspace-tui`, shared rust workspace/toolchain files, or an empty diff classify as `tui_full` and run the current six-target graph. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
| CI-TUI-SCOPE-003 | `ci.yml` keeps job `workspace-tui` and all six `Workspace TUI (*)` matrix names. Real work is skipped inside those jobs. The job has no job-level `if:`. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts` and `scripts/test/check-ci-test-boundaries.test.ts`. |
...
- 2026-05-12 GitHub Actions/local explicit SSH workflow rerun: `bun test
  ...
  evidence.
- 2026-06-29 current-state wiring update: `smoke:ssh` is the aggregate local explicit SSH
  release-readiness command over `smoke:public-launch:basic-docker`,
  `smoke:public-launch:github-repo`, and `smoke:public-launch:scheduled-task-cron`.
  `public-launch-basic-docker-smoke`, `public-launch-github-repo-smoke`, and
  `public-launch-cron-smoke` are reusable GitHub workflows, nightly invokes all three as optional
  environment-gated smokes, and release can require them through
  `require_public_launch_basic_docker_smoke`, `require_public_launch_github_repo_smoke`, and
  `require_public_launch_cron_smoke`. Local preparation may skip SSH execution when no target
  server exists, but a release-readiness run that needs public launch confidence sets the required
  inputs so missing SSH secrets fail the reusable workflows.
...
- 2026-06-09 storage backup runtime smoke wiring update: `smoke:storage-backup:docker`,
  `smoke:storage-backup:ssh`, and `smoke:storage-backup` are first-class package scripts for
  local explicit real storage backup/restore execution. The Docker script enables
  `APPALOFT_E2E_STORAGE_BACKUP_DOCKER=true` and proves PocketBase-style SQLite backup, local
  filesystem artifact storage, restore-to-new-Docker-volume, and restored content verification
  against a real Appaloft-named Docker volume. The SSH script runs `smoke:ssh:preflight` before
  enabling `APPALOFT_E2E_SSH_STORAGE_BACKUP_DOCKER=true`, so generic-SSH storage backup cannot be
  claimed without a configured SSH Docker target. `storage-backup-e2e.yml` is the reusable GitHub
  Actions gate for the same probes; nightly invokes it as an optional environment-gated smoke, and
  release can require the SSH storage-backup gate with `require_storage_backup_e2e=true`.
...
- 2026-06-29 fail-closed SSH smoke script check: `smoke:public-launch:basic-docker` and
  `smoke:public-launch:github-repo` now route through `scripts/smoke/public-launch-smoke.ts`. With
  live-smoke confirmation intentionally absent, both scripts fail at the explicit
  `APPALOFT_PUBLIC_LAUNCH_SMOKE_CONFIRM=true` requirement, which proves the scripts do not silently
  pass without real SSH release-readiness evidence.
- 2026-05-12 SSH preflight script check: `smoke:ssh:preflight` verifies
  ...
  `public-launch-basic-docker-smoke`, `public-launch-github-repo-smoke`, and
  `public-launch-cron-smoke` workflows also run the same preflight after writing the SSH key to the
  runner temp directory and before starting the real SSH suites, passing the optional SSH port and
  username secrets into preflight before the execution step applies its defaults of `22` and `root`.
  The preflight prints only variable names and status messages, not the configured private-key path,
  username, or secret material.
- 2026-06-29 SSH evidence capture check: `smoke:ssh:evidence` runs the aggregate `smoke:ssh` suite
  and writes `dist/release/ssh-smoke-evidence.json` only after the public launch SSH suites pass.
  The evidence record uses schema `appaloft.ssh-smoke-evidence/v1`, binds the result to
  `RELEASE-HARDENING-006`, lists the three public launch SSH smoke suites, and stores only redacted
  configuration booleans for host, key, port, username, and public-route host presence. It does not
  record host, username, private-key path, public-route host, or secret material.
  ...
  `smoke:public-launch:basic-docker:evidence`, `smoke:public-launch:github-repo:evidence`, and
  `smoke:public-launch:scheduled-task-cron:evidence` write the per-workflow
  `dist/release/public-launch-basic-docker-evidence.json`,
  `dist/release/public-launch-github-repo-evidence.json`, and
  `dist/release/public-launch-cron-evidence.json` records used by GitHub Actions reusable
  workflows, which upload those files as `public-launch-basic-docker-evidence`,
  `public-launch-github-repo-evidence`, and `public-launch-cron-evidence` artifacts. Focused test
  coverage verifies successful redacted evidence capture with a fake smoke command, per-suite suite
  lists, workflow artifact wiring, evidence verification success, target mismatch rejection, leaked
  configured-value rejection, and verifies failed smoke commands do not write an evidence file.
...
- SSH workflow smokes require a target host, matching private key, and required public launch SSH
  variables or `APPALOFT_E2E_SSH_*` fallbacks. Local runs can be skipped when no target exists;
  GitHub Actions release-readiness runs can set the matching `require_public_launch_*_smoke=true`
  input and fail closed if secrets are absent.
