...
| CI-LIGHTWEIGHT-003 | PRs that touch product source under `apps/`, `packages/`, `skills/` (non-markdown), lockfiles, or Rust besides the two desktop version files stay on full CI and real E2E. | GitHub Actions merge-gate contract | Automated in `scripts/test/classify-changed-files.test.ts`. |
