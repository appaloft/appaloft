...
| `SRC-AUTO-POLICY-003` | Source binding changes after policy creation. | Policy is disabled or blocked according to accepted Code Round rule. | planned | Deferred gap |
...
| `SRC-AUTO-EVENT-002` | Provider redelivers same event. | Dedupe prevents duplicate deployment and read model reports deduped. | planned | Deferred gap |
