...
| `SRC-AUTO-ROUNDTRIP-001` | User saves every supported auto-deploy policy field and reloads the Resource settings. | HTTP dispatch preserves the shared command schema; Resource persistence and `resources.show` return the same supported fields; Web rehydrates the form from readback after cache invalidation. No tag-trigger capability is inferred. | `packages/orpc/test/resource-source-profile.http.test.ts`; `packages/persistence/pg/test/resource-auto-deploy-policy.pglite.test.ts`; `packages/application/test/show-resource.test.ts`; `apps/web/src/lib/console/auto-deploy.test.ts` | Passing |
