...
`cliResolvedSource`, and the full cwd is first-class `originalLocator`. `recoverLocalSourceFolderFromCwd`
must not treat `projects` as the source leaf. A hyphenated leaf under a parent named `projects`
(`appaloft-cloud` or `nux-*-static`) stays that leaf for CLI-host package and for any
missing-workdir message. When that folder exists on the package host,
