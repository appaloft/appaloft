...
| DEP-CREATE-PKG-007 | unit | Local source workdir keeps the real folder | Locator is a hyphenated folder such as `/Users/nichenqin/projects/nux-c79876d8-static` that does not exist on this host. CLI summary.Source is that full cwd. `execution.workingDirectory` may already be the parent (`/Users/nichenqin/projects`). Locator and workdir may both already be that parent after an upstream dirname, and `displayName` may be `workspace`, `projects`, or omitted after persist/rehydrate. `metadata.cliResolvedSource` is empty on the detached worker. The package host may not have that folder, and `process.cwd()` may be a runtimeDir rather than the source folder. When the same hyphenated cwd exists on the package host, the existsSync + tar path must package that folder. | Runtime still uses that folder path | None | CLI `deploy .` snapshots `resolve(process.cwd(), ".")` into first-class `source.locator`, typed `source.originalLocator` (local-folder allows absolute host paths), and `execution.workingDirectory`. Do not depend on `metadata.cliResolvedSource`; create-deployment for a remote target is detached. `resolveSshPackageLocalWorkdir` / `prepareSshSource` prefer originalLocator, then workingDirectory, then locator. `recoverLocalSourceFolderFromCwd` must refuse generic parent basenames (`projects`, `workspace`, `src`, `$HOME` leaf, `.`) and must not treat `projects` as the source leaf. Detector must not replace a local-folder locator with the parent and must not clobber a hyphenated displayName with `basename(parent)`. Live shape: locator=parent, workdir=parent, no cliResolvedSource, displayName=`workspace`/`projects`, originalLocator or snapshotted CLI cwd is the hyphenated folder → existsSync+tar read that folder; a missing-workdir error must name the leaf, not only the parent. Reused static resources refresh source when originalLocator is missing. `baseDirectory` `/` or `..` must not walk to the parent. Only an existing file locator may use its parent. |
...
must not treat `projects` as the source leaf. When that folder exists on the package host,
