...
| DEP-CREATE-PKG-009 | unit | Skip-pack git dockerfile checkout ignores leftover packed archives | Hyphenated **git** leaf named `appaloft-cloud` (not `nux-*-static`) has a real root `Dockerfile` (~4kB). Skip-pack sends `{ folder }` with no `packedSourceArchiveTarGz`. Resource-binding / execution metadata still has leftover empty or stale `cliPackedSourceTarGz` from a previous apply. | Later terminal result depends on runtime execution | `build-requested` when package/build work is required | `prepareSshSource` prefers the live git folder and git-aware tar. The uploaded archive contains that root `Dockerfile` at approximately the on-disk size, not a 2B / missing BuildKit dockerfile. Leftover binding archives must not be materialized as `cli-source.tgz`. Do not pack the tree as `nux-*-static`. Do not raise the 16MiB bound. Tiny no-git `nux-*-static` remains `DEP-CREATE-PKG-007`. |
| DEP-CREATE-PKG-010 | unit | Empty git-aware dockerfile upload must not report ready | `git ls-files` is empty, or leftover `cliPackedSourceTarGz` is a 45-byte gzip with 0 files, and the plan is `buildStrategy = dockerfile` / `dockerfilePath = Dockerfile` | Accepted attempt reaches terminal `failed` | `deployment-failed` with `ssh_source_upload_failed` | Empty `git ls-files \| tar --null --files-from -` may exit 0, but `prepareSshSource` must not report `Remote source workspace is ready`. Missing or <=2B Dockerfile in the live git list or packed archive fails closed before docker build. Remote extract may additionally `test -s` the Dockerfile. |
...
the remote workspace ready. The
