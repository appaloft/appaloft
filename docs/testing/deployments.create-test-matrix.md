...
| DEP-CREATE-ENTRY-010 | CLI adapter | Top-level deploy waits for terminal status | `appaloft deploy` (local or remote/detached) polls until a terminal deployment status, exits non-zero on `failed` / `ssh_docker_build_failed`, and prints a live URL only after `succeeded`. |
