...
| DEP-CREATE-PKG-007 | unit | Local source workdir keeps the real folder | Locator is a hyphenated folder such as `/Users/nichenqin/projects/nux-c79876d8-static` that does not exist on this host. CLI summary.Source is that full cwd. `execution.workingDirectory` may already be the parent (`/Users/nichenqin/projects`). Locator and workdir may both already be that parent after an upstream dirname, and `displayName` may be omitted after persist/rehydrate. The package host may not have that folder, and `process.cwd()` may be a runtimeDir rather than the source folder. When the same hyphenated cwd exists on the package host, the existsSync + tar path must package that folder. | Runtime still uses that folder path | None | CLI `deploy .` persists the exact resolved cwd as `source.metadata.cliResolvedSource` (the same path printed as summary.Source). Locator is not a substitute for that path. Reused static resources refresh source when that metadata is missing. SSH `prepareSshSource` exists-check and tar (`resolveSshPackageLocalWorkdir`) must use that CLI-resolved path even when locator/workdir are already the parent, `displayName` is omitted, and cwd is a runtimeDir. Do not pass runtimeDir as the source cwd and do not add another recover/dirname helper. When that folder exists on the package host, existsSync succeeds and tar `-C` that folder includes `public/index.html`; the missing-workdir error must not name the parent. The static plan writes `execution.workingDirectory` from that same path, not the parent. A dirname'd `workingDirectory` must not win over the CLI-resolved source. When the CLI-resolved source is absent, reconstruct the hyphenated `deploy .` cwd by leaf/`displayName` even if the folder is missing here. `baseDirectory` `/` or `..` must not walk to the parent. Only an existing file locator may use its parent. Detector keeps the same folder and persists the CLI-resolved source only when it was explicit. |
...
both already the parent, `displayName` is the hyphenated leaf, and process.cwd() is a runtimeDir
that is not the source folder. The live package path also covers locator and workdir already the
parent, `displayName` omitted, cwd a runtimeDir, and the original `deploy .` path supplied as the
CLI-resolved source: `localWorkdir` must be that full path and a missing error must include the
leaf. Executable static smoke coverage
