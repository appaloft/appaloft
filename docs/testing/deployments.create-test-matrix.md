...
| DEP-CREATE-PKG-007 | unit | Local source workdir keeps the real folder | Locator is a hyphenated folder such as `/Users/nichenqin/projects/nux-fb4bd8c5-static` that does not exist on this host | Runtime still uses that folder path | None | `normalizeLocalSourceWorkingDirectory` must not dirname to the parent (`/Users/nichenqin/projects`); only an existing file locator may use its parent |
