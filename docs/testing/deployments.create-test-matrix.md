...
| DEP-CREATE-PKG-007 | unit | Local source workdir keeps the real folder | Locator is a hyphenated folder such as `/Users/nichenqin/projects/nux-9859a0e9-static` that does not exist on this host | Runtime still uses that folder path | None | SSH package uses `resolveLocalWorkspaceWorkdir(workingDirectory ?? locator, metadata)` and must not dirname to the parent (`/Users/nichenqin/projects`). `baseDirectory` `/` or `..` must not walk to the parent. Only an existing file locator may use its parent. Detector and static plan keep the same folder. |
...
parent. Executable static smoke coverage
