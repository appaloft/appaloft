...
| DEP-CREATE-PKG-007 | unit | Local source workdir keeps the real folder | Locator is a hyphenated folder such as `/Users/nichenqin/projects/nux-c79876d8-static` that does not exist on this host. CLI summary.Source is that full cwd. `execution.workingDirectory` may already be the parent (`/Users/nichenqin/projects`). Locator and workdir may both already be that parent after an upstream dirname. The package host may not have that folder, and `process.cwd()` may be a runtimeDir rather than the source folder. | Runtime still uses that folder path | None | The static plan writes `execution.workingDirectory` from the real folder, not the parent. SSH `prepareSshSource` exists-check must package the same full cwd as summary.Source. A dirname'd `workingDirectory` must not win over the locator. When locator and workdir are both the parent, reconstruct the hyphenated `deploy .` cwd by leaf/`displayName` even if the folder is missing here and cwd is a runtimeDir. Do not require existsSync or a matching process.cwd(). `baseDirectory` `/` or `..` must not walk to the parent. Missing-workdir error must contain the leaf folder and must not be the parent. Only an existing file locator may use its parent. Detector keeps the same folder. |
...
that is not the source folder. Executable static smoke coverage
