...
parent, including the live `prepareSshSource` exists-check when `workingDirectory` is already the
parent while summary.Source / `source.locator` is the full cwd. Executable static smoke coverage
