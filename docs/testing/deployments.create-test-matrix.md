...
| DEP-CREATE-PKG-007 | unit | Local source workdir keeps the real folder | Locator is a hyphenated folder such as `/Users/nichenqin/projects/nux-c79876d8-static` that does not exist on this host. CLI summary.Source is that full cwd. `execution.workingDirectory` may already be the parent (`/Users/nichenqin/projects`). Locator and workdir may both already be that parent after an upstream dirname, and `displayName` may be omitted after persist/rehydrate. The package host may not have that folder, and `process.cwd()` may be a runtimeDir rather than the source folder. | Runtime still uses that folder path | None | CLI `deploy .` persists the exact resolved cwd as `source.metadata.cliResolvedSource` (the same path printed as summary.Source). SSH `prepareSshSource` exists-check and tar must use that CLI-resolved path even when locator/workdir are already the parent, `displayName` is omitted, cwd is a runtimeDir, and the folder is missing on the worker. Do not add another recover heuristic. The static plan writes `execution.workingDirectory` from that same path, not the parent. A dirname'd `workingDirectory` must not win over the CLI-resolved source or locator. When the CLI-resolved source is absent, reconstruct the hyphenated `deploy .` cwd by leaf/`displayName` even if the folder is missing here and cwd is a runtimeDir. Do not require existsSync or a matching process.cwd(). `baseDirectory` `/` or `..` must not walk to the parent. Missing-workdir error must contain the leaf folder and must not be the parent. Only an existing file locator may use its parent. Detector keeps the same folder and persists the CLI-resolved source. |
