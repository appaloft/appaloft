...
| PROJ-LIFE-DELETE-CHECK-002 | `projects.delete-check` | integration | Archived project has retained blockers and retained audit history. | Returns `eligible = false` with safe typed blockers for non-audit blockers only; audit history remains retained but is not a project delete blocker. |
...
| PROJ-LIFE-DELETE-001 | `projects.delete` | integration | Archived project has no blockers, may have retained audit history, and confirmation matches. | Persists deleted tombstone, publishes `project-deleted`, omits project from normal read models, and does not cascade child cleanup or prune audit history. |
