...
| PROJ-LIFE-ENTRY-005 | Web | e2e-preferred | Project detail/settings lifecycle controls. | Reads `projects.show`, dispatches `projects.rename` and `projects.archive`, and disables project-scoped creation affordances when archived. |
...
The first Code Round includes Web controls. Dedicated browser automation for
`PROJ-LIFE-ENTRY-005` remains a future coverage improvement; current closure relies on Svelte
typecheck and shared operation tests.
