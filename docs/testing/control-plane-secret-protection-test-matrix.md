...
| CPS-DIAG-018 | Rotation source read failures expose only a fixed source reason without database details. | PG/PGlite + CLI safe error | integration | `control-plane-secret-rotation.pglite.test.ts` required-source failure fixture; `CPS-SAFE-016` renderer | passing |
