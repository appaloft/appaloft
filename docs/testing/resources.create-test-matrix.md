...
| RES-CREATE-ADM-032 | integration | Duplicate slug | Existing resource same project/environment/slug | `err` | `resource_slug_conflict`, phase `resource-admission`, details include existing `resourceId` | None | Existing resource unchanged |
...
| RES-CREATE-ADM-035 | integration | Static site resource profile | `kind = static-site`, source binding, `runtimeProfile.strategy = static`, `runtimeProfile.publishDirectory = /dist` or `dist`, and `networkProfile.internalPort = 80` | `ok({ id })` | None | `resource-created` | Resource persists static runtime profile with source-root-relative publish directory `dist` and HTTP reverse-proxy network profile |
...
| RES-CREATE-ADM-037D | integration | Relative static publish directory `public` | Static runtime profile uses `publishDirectory = public` | `ok({ id })` | None | `resource-created` | Resource persists `public`, not `/public`; Dockerfile `COPY` uses context-relative `public/` |
...
| RES-CREATE-WF-009 | e2e-preferred | Repository config file creates first resource profile | CLI or local Web agent reads valid config profile with source/runtime/network/health fields | Resource creation persists profile values, but resource identity is selected, auto-created, or explicitly supplied by self-hosted `controlPlane.deploymentContext` outside `resources.create` | Resource state owns profile; config-origin metadata is safe; deployment uses resource id only | Config discovery -> identity resolution -> `resources.create(source, runtimeProfile, networkProfile)` or existing resource profile commands -> `deployments.create(resourceId)` |
| RES-CREATE-WF-010 | e2e-preferred | Repository config file rejected before resource create | Config contains project/resource/server identity outside `controlPlane.deploymentContext`, raw secret, or unsupported sizing field | Workflow fails before `resources.create` | No resource is persisted from invalid config | Config validation only |
| RES-CREATE-WF-010A | e2e-preferred | Preview resource first create uses preview runtime name seed | Trusted PR preview entry creates a new preview-scoped resource and no explicit runtime name is supplied | Resource create persists preview runtime naming intent before deployment admission | Preview resource stores `runtimeProfile.runtimeName = preview-{pr_number}` or equivalent trusted derived value; deployment uses the resource id only | Preview context resolution -> `resources.create(runtimeProfile.runtimeName)` -> `deployments.create(resourceId)` |
...
`RES-CREATE-ADM-037`, `RES-CREATE-ADM-037C`, and `RES-CREATE-ADM-037D` are covered by
`packages/application/test/create-resource.test.ts`, `packages/core/test/resource.test.ts`, and
runtime adapter static-server Dockerfile generation tests.
