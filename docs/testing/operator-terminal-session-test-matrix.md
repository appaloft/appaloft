...
| TERM-SESSION-ENTRY-011 | CLI/adapter integration | Remote control-plane terminal attach | A compatible Cloud or self-hosted profile is selected | Server/resource/Sandbox terminal open dispatches through the typed remote operation and `--attach` bridges the accepted WebSocket while avoiding local PGlite, SSH state sync, and target credential exposure. |
