...
| SBX-DOM-005 | core/application unit | Sandbox display name | Two Appaloft generates are kebab adjective-noun; a stored name is stable on rehydrate; folder.local may use the directory; git occupy generates kebab (not `repo@sha`); user-facing TUI list/detail never paint `sbx_`. | `packages/core/test/sandbox-display-name.test.ts`; `packages/application/test/execution-sandbox-operations.test.ts`; `apps/workspace-control-tui` `ws_remote_ca_069_*` | passed |
...
| SBX-CMD-005 | application/provider + local Docker | Background process lifecycle | Start/list/show/terminate use safe ids; a successful detached start returns only after the exact process is observed running or terminal, while a launch that remains unobservable fails within a bounded window and cleans up; every background command is isolated into one process group, exact termination performs bounded TERM-to-KILL escalation, no running descendant survives, and repeated termination remains safe; live attach/events are not claimed. | application + hermetic provider tests including delayed/unavailable start observation; local Alpine Docker descendant probe | passed; remote registered-Server regression tracked by #1055 |
...
| SBX-FILE-005 | application/provider | Missing confined file read | An explicit provider-neutral missing-file signal becomes non-retryable `sandbox_file_not_found`; transport, confinement and arbitrary provider failures remain retryable provider failures. | `packages/application/test/execution-sandbox-operations.test.ts` | passed |
| ADAPTER-RUNTIME-014 | runtime adapter | Docker missing-file distinction | A Docker read checks absence inside the managed container, preserves canonical-path/symlink confinement for existing paths and raises only the provider-neutral missing-file signal for an absent path. | `packages/adapters/runtime/test/docker-sandbox-provider.test.ts` | passed |
...
| SBX-RECONCILE-002 | application/provider + persistence | In-flight lifecycle reconciliation | Provider inventory for a Sandbox in `provisioning`, `pausing`, `resuming` or `terminating` is retained while the persisted lifecycle transition is in flight, even before its final provider handle is saved; stable mismatched handles remain removable. | `packages/application/test/execution-sandbox-reconciliation.test.ts`; `packages/persistence/pg/test/execution-sandbox-repository.test.ts` | passed |
...
| SBX-RUNTIME-006 | runtime adapter | Concurrent exact Docker termination | An exact owned-container removal that reports already in progress succeeds only after a bounded multi-second inspect/list readback proves absence; a still-present container through the full bound or an unrelated failure remains an error. Test timing is deterministic without sleeping for the production window. | `packages/adapters/runtime/test/docker-sandbox-provider.test.ts` | automated pass under reopened #959; downstream registered-server rerun pending after run `30754835742` exceeded the original three 100 ms probes |
...
| SBX-RUNTIME-005 | runtime adapter + Agent regression | Workspace-scoped process home under a read-only runtime root and compute-released resume | Docker foreground/background exec and managed terminal commands supply `HOME` and XDG data/config/state/cache below `/workspace`; the OpenCode harness uses the same environment for version, server and task processes, so pause/resume cannot reopen root/global Agent state. | `packages/adapters/runtime/test/docker-sandbox-provider.test.ts`; `packages/adapters/runtime/test/opencode-sandbox-agent-harness.test.ts`; `scripts/smoke/execution-sandbox-docker.ts`; opt-in registered-server GitHub Agent smoke | public focused tests and local Docker product probe passed; registered-server rerun pending after external run `30690194327` |
