...
| SBX-CMD-005 | application/provider + local Docker | Background process lifecycle | Start/list/show/terminate use safe ids; every background command is isolated into one process group, exact termination performs bounded TERM-to-KILL escalation, no running descendant survives, and repeated termination remains safe; live attach/events are not claimed. | application + hermetic provider tests; local Alpine Docker descendant probe | passed |
