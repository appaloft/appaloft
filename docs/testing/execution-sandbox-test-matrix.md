...
| SBX-SECRET-001 | security integration | Destination-bound credential grant | Only secret refs cross boundaries and plaintext never enters state/output/audit/snapshot. | credential broker suite | planned |
...
| SBX-RECONCILE-001 | application/provider | Restart/orphan reconciliation | Duplicate attempts converge and cleanup targets exact persisted handles. | reconciliation suite | planned |
...
| SBX-STREAM-001 | HTTP/SDK contract | Live process/event streaming | SSE/WebSocket is bounded, cancellable and exposes one terminal frame. | stream suite | planned |
