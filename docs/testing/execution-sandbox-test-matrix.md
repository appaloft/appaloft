...
| SBX-RUNTIME-006 | runtime adapter | Concurrent exact Docker termination | An exact owned-container removal that reports already in progress succeeds only after bounded inspect/list readback proves absence; a still-present container or unrelated failure remains an error. | `packages/adapters/runtime/test/docker-sandbox-provider.test.ts` | passed |
