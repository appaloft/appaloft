...
public preview URL, resolves control-plane mode on PR close, runs self-hosted server API cleanup
when configured or explicit CLI preview cleanup otherwise, and deletes matching GitHub preview
deployments/environment metadata. This workflow is not a substitute for the public action wrapper
tests because preview deploy still runs the source checkout directly instead of installing a
released `appaloft/deploy-action` wrapper.
