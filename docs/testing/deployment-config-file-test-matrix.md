...
| CONFIG-FILE-STATE-003 | integration | State-root coordination protects remote backend maintenance | Two config deploys target the same SSH-server state root during ensure/migrate/sync work | One workflow owns backend maintenance at a time; the second may wait for a bounded retry window, then succeeds after release or fails with retriable lock error according to policy. Current clients may cap older recorded stale windows to the configured maintenance stale window before stale-only recovery when heartbeat metadata is no longer fresh | `infra_error`, phase `remote-state-lock` when state-root coordination cannot be acquired within the retry window | Ensure/migrate/sync before identity/resource/env/deployment writes |
...
| CONFIG-FILE-STATE-006 | integration | Abandoned lock recovers safely | Previous workflow left a stale lock or recovery marker | Workflow records recovered lock metadata, takes over with a fresh owner/correlation/heartbeat, and proceeds; active locks still fail with a retriable lock error; lock-directory creation failures such as full disk are not misclassified as active locks | `infra_error`, phase `remote-state-lock` only when the existing lock is still active or recovery is unsafe; lock creation failures use the prepare/resolution phase with the underlying stderr | No write commands while the lock is actively owned by another workflow |
...
| CONFIG-FILE-STATE-019 | integration | Out-of-band stale lock recovery | SSH state-root `locks/mutation.lock` is older than its stale window, including an incomplete lock directory that never wrote `owner.json` | `appaloft remote-state lock recover-stale` atomically moves the lock directory under `locks/recovered`, writes recovered metadata, treats ownerless incomplete locks as stale after a short incomplete-lock window, allows an explicit shorter stale window to cap older owner metadata, and leaves active owned locks in place | SSH transport error only when the target cannot be reached | Direct SSH recovery -> recovered journal -> next deployment may acquire a fresh state-root lock |
...
| CONFIG-FILE-STATE-021 | integration | Immutable server-owned SSH PGlite backup | An operator authorizes repair of an existing SSH PGlite state root | `appaloft remote-state backup create` acquires state-root coordination, archives only the authoritative PGlite and bounded sidecar state, records safe digest/revision/major metadata, makes the backup tree read-only, and returns a bounded `remote-state-backup:immutable-*` reference | Active lock or SSH/filesystem failure; no live state mutation | Coordinate live state -> create immutable archive and manifest -> release coordination |
...
| CONFIG-FILE-STATE-023 | integration | Validated candidate promotion | Candidate migration and read-only validation produced a safe plan digest while the live write freeze remains unchanged | `appaloft remote-state backup promote-copy` verifies backup, candidate, plan digest, and live source-tree digest, then performs a staged swap and increments the live revision; digest drift fails before replacement | Invalid proof/reference, live digest drift, candidate validation failure, or staged-swap failure with automatic restoration | Coordinate live state -> verify freeze/candidate -> preserve rollback staging -> promote -> revision/marker -> release |
| CONFIG-FILE-STATE-024 | integration | Immutable backup rollback | A promoted candidate fails later smoke and the operator supplies its immutable backup reference with explicit confirmation | `appaloft remote-state backup rollback` verifies the immutable archive and restores it through the same coordinated staged-swap path, increments revision, and records a safe recovery marker | Invalid reference/digest, active lock, or staged-swap failure with previous live state restored | Coordinate live state -> verify backup -> staged restore -> revision/marker -> release |
...
- `CONFIG-FILE-STATE-018`, `CONFIG-FILE-STATE-019`, and `CONFIG-FILE-STATE-020` have CLI/static
  ...
  command construction, workflow-action remedy registration, and the repository maintenance
  workflow source.
