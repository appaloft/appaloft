...
| RUNTIME-CAPACITY-INSPECT-001 | unit/integration | Read-only SSH capacity diagnostic | `appaloft server capacity inspect <serverId>` targets a generic-SSH single-server Docker backend | Query dispatches `servers.capacity.inspect`, executes only read-only probes, and returns disk, inode, Docker image/build-cache, Appaloft runtime-root/state/source-workspace, safe reclaimable estimate, and warnings | None or `runtime_target_unsupported` for unsupported providers | `servers.capacity.inspect` only; no prune/delete/cleanup commands |
