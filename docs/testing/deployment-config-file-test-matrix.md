...
| CONFIG-FILE-STATE-019 | integration | Out-of-band stale lock recovery | SSH state-root `locks/mutation.lock` is older than its stale window | `appaloft remote-state lock recover-stale` atomically moves the lock directory under `locks/recovered`, writes recovered metadata, and leaves active locks in place | SSH transport error only when the target cannot be reached | Direct SSH recovery -> recovered journal -> next deployment may acquire a fresh state-root lock |
