...
| CONFIG-FILE-STATE-020 | workflow-contract | Shared docs remote-state maintenance workflow and preflight | An operator manually dispatches the repository `Remote State Maintenance` workflow, or docs production/preview deploy/cleanup starts while a previous run left a stale state-root lock | Manual maintenance checks out `main`, uses `vars.APPALOFT_SSH_HOST`, `vars.APPALOFT_SSH_USER` defaulting to `root`, `secrets.APPALOFT_SSH_PRIVATE_KEY`, defaults `remote_runtime_root` to `/var/lib/appaloft/runtime`, runs inspect or stale-only recovery through the CLI, shares the docs remote-state concurrency group, and writes a step summary. Docs deploy/preview/cleanup also run a stale-only preflight before mutation with a short state-root maintenance stale window so stale locks are archived without requiring a separate user workflow. Summaries include status, owner/correlation id, heartbeat, age, stale window, and recovered path without printing private key material | SSH transport error only when the target cannot be reached; active locks are not force deleted | Manual dispatch or docs workflow preflight -> CLI inspect or recover-stale -> GitHub step summary -> deploy/cleanup mutation |
