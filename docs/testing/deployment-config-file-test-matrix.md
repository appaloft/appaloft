...
| CONFIG-FILE-STATE-012E | integration | Active v2 recovery marker fences reads and resolves by revision | A process interruption leaves an active v2 marker before or after the atomic revision commit | Ordinary archive/revision reads fail closed; explicit inspect retry validates transaction-scoped paths and marker relations, then rolls back when revision remains expected or performs committed cleanup when revision equals next | `infra_error`, reason `remote_state_recovery_required` until explicit recovery succeeds | Publish active marker -> interruption -> read fence -> explicit locked resolver -> revision-directed rollback or cleanup |
