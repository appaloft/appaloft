...
| CONFIG-FILE-STATE-003 | integration | Remote state lock serializes concurrent deploys | Two config deploys target the same SSH-server state root | One workflow owns mutation at a time; the second may wait for a bounded retry window, then succeeds after release or fails with retriable lock error according to policy | `infra_error`, phase `remote-state-lock` when lock cannot be acquired within the retry window | Lock before identity/resource/env/deployment writes |
