...
| CONFIG-FILE-STATE-012F | integration | Legacy v1 recovery is strict and resumable | A released CLI left `{phase:"remote-state-sync-upload", backup}` after an interrupted copy-based upload | Explicit inspect retry requires the exact state-root backup path, numeric live revision, complete non-symlink backup components, and PGlite version evidence; it keeps the full backup as a read-only recovery source, replaces each partial live component idempotently, syncs the restored data root, removes the marker, then cleans the backup best-effort | `infra_error`, reason `remote_state_recovery_required` for unsafe legacy state | Validate legacy marker and full backup -> idempotent component copy restore -> sync restored data root -> remove marker -> best-effort remove backup |
