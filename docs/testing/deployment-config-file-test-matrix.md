...
| CONFIG-FILE-STATE-012E | integration | Active v2 recovery marker fences reads and resolves by revision | A process interruption leaves an active v2 marker before or after the atomic revision commit, including when remote disk pressure caused the interruption | Ordinary archive/revision reads fail closed; explicit inspect retry validates transaction-scoped paths and marker relations, then rolls back when revision remains expected or performs committed cleanup when revision equals next. Rollback restores authoritative components, removes superseded incoming/revision/backup staging to reclaim space, durably syncs the restored data root, and only then removes the recovery marker | `infra_error`, reason `remote_state_recovery_required` until explicit recovery succeeds | Publish active marker -> interruption -> read fence -> explicit locked resolver -> revision-directed rollback -> reclaim staging -> durability sync -> marker removal, or committed cleanup |
