...
| EDGE-PROXY-RELOAD-004A | integration | Imported certificate-backed route activation | `certificate-imported` selects a candidate for a durable manual-policy route | Provider materializes and applies the candidate through its secret-safe activation boundary | Domain route remains not ready until reload and proof succeed | Conditional |
...
| EDGE-PROXY-RELOAD-004D | opt-in Docker e2e | Hostname/SNI fingerprint proof | Real Traefik and Caddy fixtures each have old and candidate certificates | Candidate activation/reload runs, then a direct TLS client connects with binding hostname SNI | Each proxy serves the independently expected candidate fingerprint before readiness, and rollback restores the old fingerprint | Conditional |
