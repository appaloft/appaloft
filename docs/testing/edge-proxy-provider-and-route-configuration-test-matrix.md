...
| EDGE-PROXY-ROUTE-010 | integration | Managed route deployment identity | Caddy or Traefik receives a served route for one deployment | Provider route plan stamps served responses with the shared deployment identity header | Header value is the owning deployment id; redirect-only routes do not claim workload identity | Deployment Proof can compare the public response with the planned Deployment instead of trusting workload labels | Per route realization |
...
| EDGE-PROXY-RELOAD-004A | integration | Imported certificate-backed route activation | `certificate-imported` selects a candidate for a durable manual-policy route | Provider atomically replaces stable binding-scoped material through its secret-safe activation boundary without recreating the workload | Domain route remains not ready until reload and proof succeed | Conditional |
...
| EDGE-PROXY-RELOAD-004D | opt-in Docker e2e | Hostname/SNI fingerprint proof | Real Traefik and Caddy fixtures each have old and candidate material for the same certificate identity | Binding-scoped candidate activation/reload runs without workload replacement, then a direct TLS client connects with binding hostname SNI | Each proxy serves the independently expected candidate fingerprint before readiness, and rollback restores the old fingerprint | Conditional |
