...
| SOURCE-LINK-STATE-024 | integration | Repository config provenance persists together | Repository config writes preview dependency provenance, including the declared canonical dependency kind, preview storage provenance, and scheduled task provenance for the same preview source fingerprint | Source-link metadata stores all provenance groups without one write dropping another | None | Provenance write -> read source link |
