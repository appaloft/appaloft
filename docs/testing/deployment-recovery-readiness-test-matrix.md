...
| `DEP-REDEPLOY-WEB-001` | Web resource detail accepts redeploy or force redeploy. | The resource header and deployments tab open the deployment progress dialog immediately, use the accepted deployment id as the observation handle, and follow `deployments.timeline` / `deployments.timeline.stream` instead of showing only a success toast. | `apps/web/src/lib/console/console-page-structure.test.ts` | Passing |
| `DEP-REDEPLOY-WEB-002` | Web redeploy progress reaches a clear completed state. | The progress dialog has a single visible panel, keeps terminal success sticky after cleanup logs, animates its running indicator, and shows the reachable access URL when the route is ready. | `apps/web/src/lib/console/console-page-structure.test.ts`, `apps/web/src/lib/console/deployment-progress.test.ts` | Passing |
