...
| PROC-DELIVERY-WORKER-021A | Detached deployment work persists the authoritative Project owner organization and restores it as worker tenant context instead of using an ambient/default request tenant. | Application | `packages/application/test/create-deployment.test.ts` |
