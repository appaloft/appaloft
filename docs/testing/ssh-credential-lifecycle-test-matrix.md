...
Tests for SSH credential lifecycle must prove that reusable SSH credential detail and usage
visibility are safe, read-only, and shared across Web, CLI, HTTP/oRPC, and future MCP/tool
surfaces through the same operation contract.

The first behavior in this matrix is active query `credentials.show`.
...
| Persistence/read model | PG/PGlite can read credential detail and usage after restart/migration. |
| Operation catalog contract | `credentials.show` is added only when CLI and HTTP/oRPC transport metadata are ready. |
| Entry surfaces | CLI, API/oRPC, Web, and future tools reuse the query schema and show usage-read failure separately from zero usage. |
...
Future rotate/update and delete-when-unused commands must add new numbered rows before Code Round.
At minimum they must cover:

- in-use safety rejection;
- unused delete success;
