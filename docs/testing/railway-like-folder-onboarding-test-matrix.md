# Railway-Like Folder Onboarding Test Matrix

## Governing Sources

- [Railway-Like Folder Onboarding](../specs/142-railway-like-folder-onboarding/spec.md)
- [ADR-122](../decisions/ADR-122-railway-like-folder-onboarding.md)
- [Remote Agent Door Test Matrix](./remote-agent-door-test-matrix.md)

## Coverage

| ID | Layer | Scenario | Expected | Automation | Status |
| --- | --- | --- | --- | --- | --- |
| `FOLDER-ONBOARD-001` | CLI | Unlinked no-git cwd | Decision creates a Project named after the directory; Effect path persists the link. | `packages/adapters/cli/test/folder-project-onboarding.test.ts` | Passing |
| `FOLDER-ONBOARD-002` | CLI | Git remote cwd | Identity is the credential-free remote; a username-only local HTTPS origin is stripped before lookup, so its active Repository Binding is reused without creating a duplicate Project. Explicit locator userinfo stays invalid. | `packages/adapters/cli/test/local-git-workspace-context.test.ts`, `packages/adapters/cli/test/folder-project-onboarding.test.ts`, `packages/adapters/cli/test/agent-workspace-command.test.ts`, `packages/adapters/cli/test/remote-code-session.test.ts` | Passing |
| `FOLDER-ONBOARD-003` | CLI | Second command same cwd | Reuses persisted link; no second create. | `folder-project-onboarding.test.ts`, `project-command.test.ts` | Passing |
| `FOLDER-ONBOARD-004` | CLI | `project use` then later command | Folder link project id changes; later resolve uses it. | `project-command.test.ts`, `occupancy-context.test.ts` | Passing |
| `FOLDER-ONBOARD-005` | CLI | Exactly one Project | Uses that Project. | `folder-project-onboarding.test.ts` | Passing |
| `FOLDER-ONBOARD-006` | CLI | Several Projects | Dedicated onboarding TTY may prompt; `--yes` creates directory-named Project. TTY `code` inquires Continue → create+link by directory name before any alt-screen and never overlays a project select on Cloud Agents. | `folder-project-onboarding.test.ts` | Passing |
| `FOLDER-ONBOARD-007` | CLI / application | No git | Occupy/deploy succeed; no whoami resume; no `git init` / fetch / clone / materialize; leftover partial folder occupancy is repaired or replaced without `workspace_open_partial_recovery_required`; source `local-folder`. | `remote-code-session.test.ts`, `packages/application/test/agent-workspace-open.test.ts`, `packages/adapters/cli/test/agent-workspace-command.test.ts`, community initializer test | Passing |
| `FOLDER-ONBOARD-008` | CLI | Status and login | Status lines name create/reuse; Cloud deploy login miss starts folded login; coding-agent/CI/non-TTY require `--yes`; remote `code` login miss is still `Run appaloft login`. | `folder-project-onboarding.test.ts`, `remote-code-session.test.ts`, `cli-session-login.test.ts`, `deploy-door-guard.test.ts` | Passing |
| `FOLDER-ONBOARD-009` | CLI / TUI | Unlinked folder from `code` | TTY `code` / `code --pi` fullscreen-inquires before any alt-screen (Continue → create+link by directory name); `--yes` / no TTY creates without inquire; no silent TTY create+link; no Effect `Prompt.select` / clack inside alt-screen; if a select is forced, `^c` restores TTY and exits immediately without a 45s `Workspace CLI operation failed` hang; after occupy stay Cloud Agents + project, collapsed tree, attached agent. Occupancy never appears in TUI chrome, progress, help, errors, i18n, or public docs body. | `folder-project-onboarding.test.ts`, `agent-workspace-command.test.ts`, `workspace-control-presentation.test.ts`, `user-visible-occupancy-forbidden.test.ts` | Passing |
| `FOLDER-ONBOARD-010` | CLI / application | Binding Project deleted | Do not reuse the leftover binding or fail-closed with `workspace_open_project_unavailable`. TTY `code` inquires then creates; `--yes` / no TTY creates; door prefers the onboarded Project over a leftover Binding; initializer find-or-creates and rebinds. | `folder-project-onboarding.test.ts`, `remote-code-session.test.ts`, `community-workspace-activation-context-initializer.test.ts`, `agent-workspace-open-preflight.test.ts` | Passing |

