...
| `FOLDER-ONBOARD-009` | CLI / TUI | Unlinked folder from `code` | TTY `code` / `code --pi` auto-creates/links by directory name; no Effect `Prompt.select` / clack inside alt-screen; if a select is forced, `^c` restores TTY and exits immediately without a 45s `Workspace CLI operation failed` hang; after occupy stay Cloud Agents + project, collapsed tree, attached agent. | `folder-project-onboarding.test.ts`, `agent-workspace-command.test.ts`, `workspace-control-presentation.test.ts` | Passing |
