...
| `FOLDER-ONBOARD-006` | CLI | Several Projects | Dedicated onboarding TTY may prompt; `--yes` creates directory-named Project. `code` never prompts. | `folder-project-onboarding.test.ts` | Passing |
...
| `FOLDER-ONBOARD-009` | CLI / TUI | Unlinked folder from `code` | TTY `code` / `code --pi` auto-create/link by directory name; no Effect select; first chrome is Appaloft Cloud Agents + project, not Occupancy or workspace-list copy. | `folder-project-onboarding.test.ts`, `agent-workspace-command.test.ts` | Passing |
