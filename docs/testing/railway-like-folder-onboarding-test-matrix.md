...
| `FOLDER-ONBOARD-009` | CLI / TUI | Unlinked folder from `code` | TTY `code` / `code --pi` auto-creates/links by directory name; no Effect `Prompt.select` inside alt-screen (including no-git + several Projects); after occupy stay Cloud Agents + project, collapsed tree, attached agent — not leftover loading/stream/list. | `folder-project-onboarding.test.ts`, `agent-workspace-command.test.ts`, `workspace-control-presentation.test.ts` | Passing |
