...
| `FOLDER-ONBOARD-009` | CLI / TUI | Unlinked folder from `code` | TTY `code` / `code --pi` auto-creates/links by directory name; no Effect select inside Cloud Agents; first chrome is Appaloft Cloud Agents + preparing the agent, not Occupancy or workspace-list copy. | `folder-project-onboarding.test.ts`, `agent-workspace-command.test.ts` | Passing |
