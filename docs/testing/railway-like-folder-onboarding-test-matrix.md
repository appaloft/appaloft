...
| `FOLDER-ONBOARD-006` | CLI | Several Projects | Dedicated onboarding TTY may prompt; `--yes` creates directory-named Project. `code` inquires before the TUI and never overlays a project select on Cloud Agents. | `folder-project-onboarding.test.ts` | Passing |
...
| `FOLDER-ONBOARD-009` | CLI / TUI | Unlinked folder from `code` | TTY `code` / `code --pi` inquire before alt-screen; no clack inside TUI; `^c` cancels inquire; first chrome is Appaloft Cloud Agents + preparing the agent, not Occupancy or workspace-list copy. | `folder-project-onboarding.test.ts`, `agent-workspace-command.test.ts` | Passing |
