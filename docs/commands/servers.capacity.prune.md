...
- Remote PGlite upload safety backups under `state/backups/sync-*` must remain protected by the
  ...
  If an interrupted upload left the filesystem under disk pressure, explicit recovery removes
  superseded incoming/revision staging before syncing the committed data root, or restores the
  authoritative pre-commit components and removes incoming/revision/backup staging before syncing
  the restored data root. The marker remains until the applicable durability fence succeeds.
