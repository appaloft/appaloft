...
| WS-SCRATCH-COMPAT-013 | Durable open locators | caller uses `workspace open` or `workspace create` | Git is dirty/detached/unpushed, or the path is not a worktree | Dirty/detached/unpushed worktrees still return ADR-103 fail-closed errors before mutation. A non-git directory is a valid locator and occupies from a git remote; it does not resume an unrelated occupancy. |
