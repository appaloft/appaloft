...
| SANDBOX-SPEC-015 | Display name | A caller creates or reopens a Sandbox | `sandboxes.create` / `workspaces.open` persist a display name | Linked folder or git prefer directory or `repo@short-sha`; unlinked opens generate kebab adjective-noun. Reopen shows the same name. User-facing CLI/TUI never paint `sbx_`. JSON may still include `sandboxId`. |
