...
| SANDBOX-SPEC-015 | Display name | A caller creates or reopens a Sandbox | `sandboxes.create` / `workspaces.open` persist a display name | Explicit name wins; `folder.local` may use the directory; git occupy generates a kebab Agent handle (not `repo@short-sha`). Reopen shows the same name. User-facing CLI/TUI never paint `sbx_`. JSON may still include `sandboxId`. |
