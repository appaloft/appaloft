...
| SANDBOX-SPEC-009 | Pause versus snapshot | A provider declares pause and/or snapshot capabilities | The caller pauses/resumes or captures/restores | Pause keeps one Sandbox identity and reports `process-frozen` or `compute-released` continuity; snapshot produces an independent reusable Snapshot with explicit filesystem/memory capability truth. Compute-released recovery follows ADR-097 and is not a hidden reusable Snapshot. |
...
| SANDBOX-SPEC-015 | Display name | A caller creates or reopens a Sandbox | `sandboxes.create` / `workspaces.open` persist a display name | Linked/cwd prefer directory or `repo@short-sha`; unlinked/no-name opens generate a memorable kebab (Appaloft adjective-noun generator, not a Railway formula). Reopen shows the same name. User-facing CLI/TUI never paint `sbx_`. JSON may still include `sandboxId`. |
