...
| WS-AGENT-NAME-007 | CLI does not invent an Agent name | TTY or `--no-attach` occupy | before `workspaces.open` returns | chrome may say `Waking the agent` without a handle. After occupy, wake/banner/connecting use `WorkspaceOpenResult.name`. No folder+harness hash. |
...
| WS-AGENT-ID-008 | Occupy creates an OccupancyAgent | logged in + Server + occupy create | `workspaces.open` / `code --no-attach` creates a Sandbox | persist `agt_*` OccupancyAgent, return `WorkspaceOpenResult.agentId` and `name`. |
| WS-AGENT-ID-009 | Resume keeps the Agent | preferred occupy for the same subject+project+repo+branch | sandbox is reused or replaced without `--new` | same `agentId` and name; Agent `sandboxId` may retarget. |
| WS-AGENT-ID-010 | `--new` is a new Agent | user passes `--new` / `forceNew` | occupy creates a replacement Sandbox | previous OccupancyAgent is retired; new `agt_*` is returned. |
