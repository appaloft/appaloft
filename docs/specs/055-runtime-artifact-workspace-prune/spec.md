...
| RT-CAP-PRUNE-013 | Protect stopped containers by durable deployment ownership | server deployment history identifies current runtime owners, in-flight attempts, or referenced rollback candidates | stopped-container prune runs | application supplies complete active-runtime and rollback-candidate deployment id sets; matching containers are skipped, missing ownership labels fail closed, and only unprotected old containers are eligible. |
| RT-CAP-PRUNE-014 | Release an archived Resource's stopped current runtime | a Resource archive has durably completed its required runtime stop, while its terminal deployments and any explicit rollback candidate remain retained | stopped-container prune runs | the archived Resource's latest successful deployment is no longer classified as an active runtime owner, while in-flight deployments and explicit rollback-candidate deployment ids remain protected. |
