...
- [x] Orchestrate server-side config bootstrap through existing resource/environment commands and
  ...
  before mutation, applies source/runtime/network/health profile fields through explicit resource
  commands, applies plain `env` values through `environments.set-variable`, applies
  ...
  `environments.set-variable` from transient Action-supplied values, and rejects committed `source`
