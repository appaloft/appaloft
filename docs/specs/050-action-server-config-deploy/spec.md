...
| ACTION-SERVER-CONFIG-SPEC-005B | Preview policy can complete partial placement hints | A pull request preview workflow selects `server-config-deploy` and sends trusted repository/base-ref context plus project/environment/server placement hints but no resource id | The Action submits the server config deploy request | The wrapper forwards the partial trusted context instead of requiring a shared resource id, and the server treats those fields as preview policy hints: it resolves the complete preview project/environment/resource/server/destination target from preview policy before target command dispatch, then performs normal authorization, config bootstrap, preview route application, and ids-only deployment admission. |
...
| ACTION-SERVER-CONFIG-SPEC-010 | Source credentials remain transient | A source package credential is supplied for provider-backed package access | The server reads config and dispatches deployment | The server may expose the credential only through request-scoped integration auth for the accepted deployment source materialization and must not persist or echo the raw credential in resource profile, deployment state, logs, errors, summaries, or read models. |
| ACTION-SERVER-CONFIG-SPEC-011 | Source package selects runtime source | A `server-github-fetch` source package includes repository and revision metadata, and a pull-request preview includes a trusted head ref | The server accepts the config package | The server applies a neutral `git-github-app` resource source profile from the source package before creating the deployment, records the revision as source metadata, and uses the pull-request head ref instead of falling back to the resource default branch. |
