...
| CPS-DIAG-018 | Rotation source read diagnosis | Rotation cannot read a required source or inspect an optional source schema | The operation returns a safe error | The error exposes a fixed source-specific reason without SQL, table identifiers, paths, hostnames, values, ciphertext, or arbitrary database details. |
