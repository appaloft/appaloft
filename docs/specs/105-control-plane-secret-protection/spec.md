...
| CPS-DIAG-027 | Extended SQLSTATE class source diagnosis | A bounded source or column probe exposes a valid PostgreSQL SQLSTATE class | The class is one of the fixed operational, data, transaction, authorization, resource, system, configuration, foreign-data, procedural, or internal families | The operation publishes only the fixed family name scoped to the source/probe; it never publishes the SQLSTATE, message, query, relation, host, path, or row value, and unknown failures remain generic. |
| CPS-COMPAT-024 | Parameter-free secret source filter | Rotation reads Environment or Resource secret variables through an embedded PostgreSQL runtime | The source table exists and boolean filtering is required before migrations | The compatibility attempt compiles the PostgreSQL literal predicate `is_secret IS TRUE` with no bind parameter; CPS-COMPAT-025 supersedes the database predicate after live evidence showed that removing the bind parameter alone was insufficient. |
| CPS-COMPAT-025 | In-memory secret source filter | Rotation reads Environment or Resource variables through a legacy embedded PostgreSQL runtime | The table and `is_secret` column are readable, but database boolean predicates are not reliable before migrations | The source query selects `is_secret` without any database boolean predicate, excludes non-secret rows before inspection or plan accounting, preserves secret-only rotation semantics, and fails closed on every source error other than exact `42P01`. |
| CPS-DIAG-026 | Bounded source-column diagnosis | A source read fails generically without a structured SQLSTATE | The source is Environment variables and no safe plan can be produced | The operation performs ordered, read-only single-column probes and publishes only the first fixed column category or `row-materialization-failed`; probe values, row counts, SQL, messages, and database details remain unpublished. |
| CPS-COMPAT-028 | Bounded complete source scan | A rotation source contains more rows than one maintenance read page | Rotation planning classifies every source before migrations | The adapter uses deterministic keyset pagination over each source's stable identifier, visits every row exactly once on the immutable read mirror, never uses offset pagination, and fails closed if cursor progress is invalid. |
| CPS-DIAG-029 | Schema-shape versus row-read diagnosis | A combined Environment source read and structured SQLSTATE diagnosis both fail | The adapter probes the stable identifier before reading any row | A parameter-free zero-row query validates only the fixed source/column shape; a separate one-row probe validates row materialization. The operation publishes only `id-schema-*` or `id-row-*` fixed categories and never publishes SQL, values, row counts, messages, relation names, hosts, or paths. |
| CPS-DIAG-030 | Database versus source-shape diagnosis | A zero-row Environment identifier probe also fails without a structured SQLSTATE | The adapter diagnoses the read boundary without inspecting business rows | A constant database probe runs before a zero-row source-table probe and the identifier-column probe. The operation publishes only fixed `database-*`, `table-schema-*`, or `id-schema-*` categories and never publishes SQL, catalog contents, values, row counts, messages, relation names, hosts, or paths. |
| CPS-COMPAT-031 | SSH PGlite PostgreSQL major compatibility | A read-only SSH maintenance mirror was created by a different embedded PostgreSQL major | Rotation planning validates the mirror before opening it with the current PGlite runtime | Planning fails closed with a fixed migration-required reason and safe source/required major numbers; it does not inspect rows, mutate or upload the mirror, or reinterpret the state as corrupt. |
| CPS-COMPAT-032 | Read-only maintenance migration boundary | Rotation planning opens an SSH PGlite mirror whose schema may predate the current application | The server composes the maintenance query over the downloaded mirror | A read-only remote session never runs application migrations or legacy-state adoption; missing tables remain empty only for exact `42P01`, incompatible schema remains fail closed, and ordinary startup treats any migration error as a startup failure instead of continuing on a partially migrated connection. |
| CPS-DIAG-033 | Embedded runtime failure diagnosis | A source or bounded database probe fails without SQLSTATE because the embedded runtime, virtual filesystem, abort boundary, or database protocol failed | Rotation planning returns a safe error | The operation publishes only one fixed runtime family scoped to the source/probe; it traverses only bounded known wrappers and never publishes exception names, messages, paths, SQL, errno values, or runtime payloads. |
| CPS-DIAG-034 | Untyped runtime failure diagnosis | A source or bounded database probe fails without SQLSTATE through the standard JavaScript `Error` boundary | Rotation planning returns a safe error | The operation publishes only the fixed `untyped-runtime-failed` family scoped to the source/probe; it does not inspect or publish the exception message, stack, path, SQL, errno value, or payload. |
| CPS-COMPAT-036 | Empty secret envelope | A legacy secret-bearing record contains an empty string | Explicitly authorized rotation protects the record | The empty plaintext is represented by an authenticated AES-GCM envelope with zero ciphertext bytes, is classified under the active key, and materializes back to the empty string without weakening envelope validation. |
