...
| AGENT-SPEC-006 | Brokered model compatibility | A Pi Run receives an OpenAI-compatible capability whose internal broker URL does not identify the upstream provider | the harness writes its ephemeral custom-model configuration | Pi uses the Chat Completions API with conservative compatibility fields rather than provider auto-detection, while the scoped token remains confined to the ephemeral Agent home. |
