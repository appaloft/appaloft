...
| CONTROL-PLANE-CLI-019 | Unknown top-level commands fail before local or remote runtime initialization. | `packages/adapters/cli/test/control-plane-client.test.ts` | `bun test packages/adapters/cli/test/control-plane-client.test.ts -t CONTROL-PLANE-CLI-019` |
| CONTROL-PLANE-CLI-020 | Secret stdin from a pipe or owner-readable regular file is captured before parser/runtime initialization and preserved for typed remote dispatch without output leakage. | `apps/shell/test/run-control-plane-cli.test.ts`; `packages/adapters/cli/test/control-plane-client.test.ts` | `bun test apps/shell/test/run-control-plane-cli.test.ts packages/adapters/cli/test/control-plane-client.test.ts -t CONTROL-PLANE-CLI-020` |
| DEP-CREATE-ENTRY-008 | An existing Resource profile and target context can admit a first deployment through the namespaced ids-only CLI surface in remote control-plane mode. | `packages/adapters/cli/test/control-plane-client.test.ts` | `bun test packages/adapters/cli/test/control-plane-client.test.ts -t DEP-CREATE-ENTRY-008` |
| DEP-CREATE-ENTRY-009 | Pure local namespaced ids-only admission progresses synchronously and verifies terminal success before CLI shutdown. | `packages/adapters/cli/test/deployment-create-command.test.ts` | `bun test packages/adapters/cli/test/deployment-create-command.test.ts -t DEP-CREATE-ENTRY-009` |
