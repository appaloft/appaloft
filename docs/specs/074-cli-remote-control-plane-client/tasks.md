...
| DEP-CREATE-ENTRY-008 | An existing Resource profile and target context can admit a first deployment through the namespaced ids-only CLI surface in local or remote control-plane mode. | `packages/adapters/cli/test/deployment-create-command.test.ts`; `packages/adapters/cli/test/control-plane-client.test.ts` | `bun test packages/adapters/cli/test/deployment-create-command.test.ts packages/adapters/cli/test/control-plane-client.test.ts -t DEP-CREATE-ENTRY-008` |
