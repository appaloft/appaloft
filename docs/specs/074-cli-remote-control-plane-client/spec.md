...
| CLI-RCPC-SPEC-027 | setup agent help stays compact and write-free | The operator asks for help | The operator runs `appaloft setup agent --help` or `appaloft setup agent -h` | The CLI prints a compact stdout option table with human flag descriptions, exits 0, does not start setup, does not enter the Effect wizard or any TUI, and does not print Effect generic `A true or false value` or `This setting is optional` copy. |
...
- `appaloft setup agent --help` / `-h` now exit through the dedicated compact help renderer before
  Effect CLI help or setup write. The write path is unchanged.
