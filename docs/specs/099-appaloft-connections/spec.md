...
| APP-CONN-006A | Identity GitHub can still request repo | Account already has GitHub login | `/account/connections` | button stays enabled, copy says identity-only, and authorize uses `sign-in/social` with `repo` + `read:user`. |
