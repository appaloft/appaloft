...
| APP-CONN-015 | Web surfaces are contextual and central | A user binds a domain, chooses a source, registers a server, or configures notifications | Web needs a connection | Web can enter from a central Connections area or contextual workflow and still uses the same application services. The central area may use a neutral integration catalog presentation with category filters, provider cards, status readback, and safe action links; it must not require provider-specific private code in the public console shell. |
