...
| APP-CONN-015 | Web surfaces are contextual and central | A user binds a domain, chooses a source, registers a server, or configures notifications | Web needs a connection | Web can enter from a central Connections area or contextual workflow and still uses the same application services. The central area may use a neutral settings-style integration list with status readback, safe action links, and in-page detail dialogs; it must not require provider-specific private code in the public console shell. |
...
| APP-CONN-023 | GitHub App Git transport uses ephemeral authentication | A deployment worker receives a short-lived installation token for a credential-free GitHub HTTPS locator | It clones the repository or initializes submodules locally or through SSH | Git receives authorization only through ephemeral process input/environment; repository URLs, argv, `.git/config`, deployment snapshots, errors, and logs remain credential-free, while GitHub SSH submodule locators may be translated to the credential-free HTTPS origin. |
| APP-CONN-024 | GitHub App workers rehydrate credentials behind the safe projection boundary | A deployment or preview worker has tenant context while public connection read models expose only installation metadata | It needs GitHub source or feedback access | The worker asks the integration auth port for an installation credential at execution time; the port loads the tenant installation record and mints a short-lived token. Workers do not consume browser/API projections or select user OAuth for provider-app work, and token values remain absent from persistence, output, errors, and logs. |
