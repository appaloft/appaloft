...
| APPALOFT-MCP-019 | One-command agent setup writes skill and Local MCP | Cursor and/or OpenCode are on the detection list and a local product-session or bearer CLI profile is active | The operator runs `appaloft setup agent` | The CLI copies the Appaloft skill into detected Cursor and OpenCode skill directories plus `~/.agents/skills`, writes the same token-free Local MCP launchers as APPALOFT-MCP-018, and never stores tokens in `mcp.json` or `opencode.json`. |
