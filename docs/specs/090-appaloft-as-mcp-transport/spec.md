...
| APPALOFT-MCP-018 | Cursor and OpenCode host install reuse a logged-in profile | A local product-session or bearer CLI profile is active | The operator runs `appaloft auth mcp cursor install` or `appaloft auth mcp opencode install` | The CLI merges a token-free host launcher for `appaloft mcp remote-stdio --profile <active-or-requested>` into `~/.cursor/mcp.json` or `~/.config/opencode/opencode.json`, preserves other MCP servers, and does not copy credential material into those files. |
| APPALOFT-MCP-020 | Claude Code host install reuses a logged-in profile | A local product-session or bearer CLI profile is active | The operator runs `appaloft auth mcp claude-code install` | The CLI merges a token-free `~/.claude.json` `mcpServers.appaloft` stdio launcher for `appaloft mcp remote-stdio --profile <active-or-requested>`, preserves other MCP servers, and does not copy credential material into that file. |
| APPALOFT-MCP-019 | One-command agent setup writes skill and Local MCP | Cursor and OpenCode are on the agent list and a local product-session or bearer CLI profile is active | The operator runs `appaloft setup agent -y` | The CLI default-checks universal plus detected Cursor (`~/.cursor`) and Claude Code (`~/.claude`), copies byte-identical skills into those skill directories plus `~/.agents/skills`, writes token-free Cursor `~/.cursor/mcp.json` and Claude `~/.claude.json` launchers, leaves OpenCode uninstalled unless `--agent opencode`, and never stores tokens in editor files. |
