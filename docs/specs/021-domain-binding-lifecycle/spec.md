...
| DNS cleanup | Follow-up cleanup for DNS records. Domain binding deletion does not mutate DNS providers by itself; Appaloft-managed records must go through DNS connector cleanup, while manual records remain a user/provider action. |
...
| domain-bindings.delete-check / delete safety | ADR-002, ADR-017, ADR-019, async lifecycle/delete safety rules | API `GET /api/domain-bindings/{domainBindingId}/delete-check`; CLI `domain-binding delete-check`; Web resource/detail surfaces render blockers before delete through the same oRPC client | `domain-bindings.delete-check` | `ROUTE-TLS-READMODEL-012`, `ROUTE-TLS-ENTRY-023` | `packages/application/test/domain-binding-lifecycle.test.ts`, WebView resource delete test | None for this slice. |
| domain-bindings.delete | ADR-002, ADR-026, this spec | API `DELETE /api/domain-bindings/{domainBindingId}`; CLI `domain-binding delete --confirm`; Web resource/detail surfaces use the same command after safety preflight | `domain-bindings.delete` | `ROUTE-TLS-CMD-022`, `ROUTE-TLS-ENTRY-024`, `ROUTE-TLS-ENTRY-026` | Application use-case test, CLI/HTTP e2e, WebView resource delete test | Certificate revoke/delete is blocked and remains certificate lifecycle work; DNS cleanup remains connector/manual follow-up. |
...
- not delete manual DNS records or silently mutate DNS providers;
- require DNS connector cleanup for Appaloft-managed provider records when provider cleanup is needed;
