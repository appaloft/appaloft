...
| CERT-LIFE-AC-005 | Revoke provider-issued certificate | Active managed certificate exists | `certificates.revoke` runs | Provider boundary is invoked when supported, running/succeeded or retriable failed provider work is visible through safe `operator-work.*` process-attempt rows, certificate becomes `revoked`, and `certificate-revoked` is recorded |
