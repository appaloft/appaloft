...
| SWARM-SPEC-015 | Compose environment values survive rendering | A Compose-backed Swarm deployment contains plain runtime values with quotes, spaces, dollar signs, backslashes, colons, Unicode, or newlines | Appaloft renders and applies the generated override | The exact values round-trip into the Swarm service without literal quote corruption; default diagnostics expose keys/masks only, and the opt-in real smoke performs exact test-owned value comparison. |
| SWARM-SPEC-016 | Every promoted served route is terminally verified | A candidate owns multiple served domains/paths for its target service | Candidate tasks converge and route labels are promoted | Every distinct served route/domain passes public proof before superseded cleanup; failure restores and verifies previous route ownership before candidate cleanup. |
