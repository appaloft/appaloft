...
| WS-OPEN-LOCATOR-024 | Do not resume an unrelated occupancy | Directory has no `.git`; latest occupancy is another repo such as `examples` | `workspace open --profile --new` | Fail-closes `workspace_remote_repository_missing` for this folder. Does not dispatch `workspaces.open` for the unrelated occupancy and does not throw `Workspace path is not inside a Git worktree`. Default `code` is the occupancy door and occupies this folder after onboarding; git is not a `code` gate. |
...
| WS-OPEN-GIT-002 | Normalize Repository identity | Remote is scp SSH, `ssh://`, or HTTPS | identity is resolved | Equivalent locators match one connector-neutral identity. A username-only authority in the locally configured Git origin is discarded before identity or clone locator leaves the CLI adapter. Explicit locator userinfo and every password, query, or fragment are rejected. |
...
| WS-OPEN-CRED-007 | Resolve named and source Credentials | Profile requirements have installation bindings and the Repository may require authenticated fetch | preflight runs | Every required named Connection resolves exactly once; a request-scoped source credential provider may return transient HTTP Basic material for the credential-free HTTPS locator; typed Git credential approval receives secret material only through stdin, authenticated fetch uses a short-lived isolated memory-only credential cache, and exact cache cleanup is attempted after success or failure; missing/stale/unauthorized state fails before Sandbox creation with safe connection guidance, while source secrets remain absent from operation input, locator, argv, result, and logs. |
...
| WS-ATTACH-NATIVE-015 | Native attach handoff | Adapter declares native attach with an exact Sandbox-private server port | create/open attaches | Profile compilation preserves the port in the declarative Harness; a short-lived revocable capability is issued; explicit `local-client-exec` runs validated argv directly, otherwise safe argv is displayed. Missing or invalid ports fail before Sandbox effects. |
