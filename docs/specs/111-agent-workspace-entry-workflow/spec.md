...
| AGENT-WS-OPEN-008 | Run OpenCode task | OpenCode Runtime is ready | a fresh or continued managed Run is submitted | The long-lived `opencode serve` process remains available for native TUI attach while a separately scoped non-attached `opencode run --format json` emits bounded neutral events; its model capability is injected outside argv, must retain the bounded startup safety window rather than cover the whole maximum Run timeout, remains fail-closed after expiry, and is revoked on success, failure or cancellation. |
| AGENT-WS-SOURCE-014 | Materialize repository | A safe HTTPS Git repository locator and optional revision are supplied, and the admitted Sandbox Template may have populated non-conflicting harness state in the Workspace root | Workspace creation executes | The repository is initialized, fetched and checked out with argv-based execution through the provider-enforced egress allowlist before Runtime creation; non-conflicting template state remains, a path conflict fails without silent overwrite, and failure returns the created Workspace id with no false-ready Runtime. |
| AGENT-WS-SOURCE-015 | Materialize private repository through trusted composition | A trusted server-side SDK composition resolves transient HTTP Basic material for a credential-free HTTPS repository locator | Workspace source fetch or default-branch discovery executes | Credential material reaches only the Git process through Sandbox stdin/process environment, never enters the locator or argv, is not persisted by the Workspace input, and malformed material fails before Git execution; a provider returning no credential preserves the public-source path. |
...
| AGENT-ADAPTER-018 | Capability descriptor | A Pi, OpenCode or custom adapter is registered | Runtime readback is requested | The descriptor reports task mode, interaction, recovery, persistent paths and healthcheck without exposing credentials or provider handles. A Profile may alias a reviewed native harness only when its exact template identity/digest match and the Adapter declares a runtime requirement compatible with that native harness version; Adapter package version is independent and a missing or incompatible runtime requirement fails closed to the declarative harness. |
...
[Spec 120](../120-profile-aware-workspace-open-and-attach/spec.md) and
[ADR-103](../../decisions/ADR-103-profile-aware-workspace-open-and-attach.md) extend this workflow
