...
| PROC-DELIVERY-WORKER-021A | Deployment worker restores authoritative owner tenant | an organization-owned Project accepts detached deployment work while the request context has an absent, default, or different tenant | the durable work item is recorded and later drained | the handoff stores the Project `organizationId` as both tenant identity and tenant organization, and the worker restores that owner context before resolving tenant-scoped integrations. |
