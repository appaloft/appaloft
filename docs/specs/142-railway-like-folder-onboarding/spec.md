...
| `FOLDER-ONBOARD-009` | `code` unlinked folder | TTY `code` / `code --pi` auto-creates/links by directory name (`--yes` / Railway `up`). No Effect `Prompt.select` / folder-not-linked selector inside alt-screen, including no-git cwd with several Projects. First chrome is Appaloft Cloud Agents + centered preparing the agent. After occupy: Cloud Agents + project name, tree collapsed, coding agent full-screen — not a leftover loading box, streamed log, or workspace list. `--pi` does not change chrome. |
