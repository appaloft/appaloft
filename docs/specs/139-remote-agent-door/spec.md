...
| WS-REMOTE-PROGRESS-205 | Ctrl+C goes to the nested agent | TTY `appaloft code`; session exists | user presses Ctrl+C or Ctrl+R | TUI does not quit. The key is passed through as `\x03` / `\x12`. Occupancy TUI does not treat SIGINT as process quit. Stop typing is shift+esc or `^]` while focused. Process quit is only `q` or unfocused `^]`. |
