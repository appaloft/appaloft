...
| WS-REMOTE-RESUME-004 | Same person reconnect | I already have a non-terminal preferred Sandbox | `appaloft code` from a directory whose origin or explicit remote matches that occupancy | resumes that Sandbox. A different cwd origin occupies this folder's repository instead of the last occupancy. A no-git folder without an explicit git remote resumes the live occupancy and attaches; git is not a gate for default `code`. `workspace open` still fail-closes without a this-folder locator. |
