...
| WS-REMOTE-RESUME-004 | Same person reconnect | I already have a non-terminal preferred Sandbox | `appaloft code` from a directory whose origin or explicit remote matches that occupancy | resumes that Sandbox. A different cwd origin occupies this folder's repository instead of the last occupancy. A non-git cwd without a git remote fail-closes instead of resuming `examples`/whoami/latest occupancy. |
...
| WS-REMOTE-NO-UPLOAD-006 | No laptop upload | laptop tree is dirty / missing / not a repo | `appaloft code` | still occupies from remote SHA when origin exists; no file content upload; no `workspace_git_dirty`. Missing origin and no explicit remote locator fail-closes `workspace_remote_repository_missing` even when another occupancy exists. A non-git folder under `$HOME` does not treat the home worktree as this folder's locator and does not wait on that ancestor remote. |
...
| WS-REMOTE-PROGRESS-197 | TTY enters the TUI before composition | TTY `code`; sidecar already exists | `appaloftdev code` | Shell starts warming the occupancy TUI (alt screen) before importing/running local composition. Login is not a warmup gate. |
...
| WS-REMOTE-PROGRESS-201 | Resolving repository cannot hang | TTY `code` from a non-git folder that sits under a `$HOME` Git worktree; latest occupancy is another repo; or cwd origin matches a live occupancy | `appaloft code` | Locator discovery does not walk into `$HOME` as this folder's repo and does not `ls-remote` an ancestor/unreachable remote. A non-git folder fail-closes `workspace_remote_repository_missing` quickly (human guidance, TUI exits). It does not stay on `Resolving repository…`. A matching occupancy uses the stored SHA and does not wait on `ls-remote`. Unrelated occupancy is still not resumed. |
...
- CLI: default `appaloft code [path|git-remote] [--no-attach] [--local] [--new] [--profile <name-or-id>]`. A git remote is a locator, not a local path. A non-git directory can occupy from a git remote and must not resume an unrelated occupancy. Default OpenCode omits the invisible `appaloft-remote` name. Durable `appaloft workspace open [path|git-remote]` / `create` accept `--server` and default to the enrolled BYOS Server.
