...
| WS-REMOTE-PROGRESS-201 | No-git `code` occupies this folder | TTY or `--no-attach` `code` from a folder with no own `.git`, including a child of an ancestor git repo such as `examples`; leftover occupancy `sbx_c343gwqfn7yd` exists | `appaloft code` / `appaloft code --no-attach` | Door occupies this folder's project (`folder.local/cwd/<name>`) after onboarding. It does not resume `examples` / `truefile` / latest leftover occupancy, does not walk an ancestor `.git`, does not stay on `Resolving repository…`, and does not `ls-remote` an ancestor/`$HOME` remote. `--new` without a locator still fail-closes. `workspace open` stays Git-safe. |
