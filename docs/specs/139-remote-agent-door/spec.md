...
| WS-REMOTE-MCP-DISCOVERY-021 | Occupancy can discover deploy ids | occupancy OpenCode/Pi has first-party `appaloft-tools` | Agent prepares serve or runs a Task | first-party binding always includes `projects_list`, `environments_list`, `resources_list`, `resources_show`, `servers_list`, `deployments_create`, and `deployments_show`. Tenant MCP Connections are unchanged. Invisible `appaloft-remote` default lists the same tools. |
