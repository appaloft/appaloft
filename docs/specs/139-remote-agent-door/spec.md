...
| WS-REMOTE-PROGRESS-193 | TTY `code`/`workspace` is the occupancy TUI | interactive TTY; not `--no-attach` / `--json` / `--no-tui` | `appaloft code` or `appaloft workspace` | CLI enters the existing occupancy Workspace TUI (alt screen) immediately. It does not stream a pile of stderr status lines as the primary UX. `--no-attach` and non-TTY keep one-line stderr progress and the first-byte bar. |
| WS-REMOTE-PROGRESS-194 | TTY `code` wait is a collapsed preparing panel | interactive TTY `appaloft code`; not `--no-attach` | launch occupy | The manage TUI opens with the workspace tree/list **collapsed** so the wait gets the whole window. Centered content panel: title spinner + **preparing the agent**; occupy steps are a left-aligned block in that panel (Braille spinner on the live step, ✓ on finished steps). Launch reports steps through a Progress sink to that screen — no `println` of `Opening remote session…`. Attach replaces the wait in the same fullscreen pane. Skill copy stays fail-soft and time-bounded. |
...
| WS-REMOTE-PROGRESS-198 | Missing TUI sidecar names the binary and next command | TTY `appaloftdev workspace` or explicit ensure/build; `appaloft-workspace-tui` is missing; rustc is too old or cargo cannot build | first TTY run | The ensure/build path names `appaloft-workspace-tui` and the next command (`rustup toolchain install stable` and `cargo build --locked --manifest-path apps/workspace-control-tui/Cargo.toml`), plus a toolchain note when rustc is older than 1.88. TTY `code` does not dump rustup: missing or stale chrome fails closed with one human message after restoring the TTY (WS-REMOTE-PROGRESS-219). `--no-attach` / headless keep working without the TUI binary. |
...
| WS-REMOTE-PROGRESS-203 | Parent enters occupancy alt-screen before sidecar lookup | TTY `workspace` | `appaloft workspace` | `enterOccupancyAltScreen` writes `ESC[?1049h`, **Appaloft Cloud Agents**, and **preparing the agent** with only `node:fs` `writeSync`. It does not resolve, cargo-build, or spawn `appaloft-workspace-tui`. Sidecar lookup starts after that first frame. TTY `code` inquires before any alt-screen. |
