...
| WS-REMOTE-NO-UPLOAD-006 | No laptop upload | laptop tree is dirty / missing / not a repo | `appaloft code` | still occupies from remote SHA when origin exists; no file content upload; no `workspace_git_dirty`. Missing origin and no explicit remote locator fail-closes `workspace_remote_repository_missing` even when another occupancy exists. |
...
| WS-REMOTE-PROGRESS-197 | TTY enters the TUI before composition | TTY `code`; sidecar already exists; user is logged in | `appaloftdev code` | Shell warms the occupancy TUI (alt screen) before importing/running local composition. |
