...
| WS-REMOTE-PROGRESS-197 | TTY enters the TUI before composition | TTY `code`; sidecar already exists | `appaloftdev code` | Shell starts warming the occupancy TUI (alt screen) from the renderer module before importing/running local composition. Occupancy TUI enters alt-screen before the parent handshake. Login is not a warmup gate. First alt-screen is under 1s. |
