...
| WS-REMOTE-PROGRESS-187 | Occupy path never sits silent | login check, server lookup, this-folder pick/create, SSH/worker open, skill prepare, or attach is about to start | `appaloft code` or `appaloft code --no-attach` | CLI prints a short present-tense status line before that slow step (for example `Checking login…`, `Opening this folder…`, `Preparing disk on hostinger…`, `Preparing skills…`, `Attaching…`). No happy-path gap stays silent for more than ~300ms. The word Occupancy does not appear in chrome or status lines. |
