...
| WS-REMOTE-RESUME-004 | Same person reconnect | I already have a non-terminal preferred Sandbox | `appaloft code` from a directory whose origin or explicit remote matches that occupancy | resumes that Sandbox. A different cwd origin occupies this folder's repository instead of the last occupancy. A no-git folder without an explicit git remote resumes the live occupancy and attaches; git is not a gate for default `code`. With no live occupancy, Spec 142 create-or-links this folder and occupies `folder.local`. `workspace open` still fail-closes without a this-folder locator. |
...
| WS-REMOTE-NO-UPLOAD-006 | No laptop upload | laptop tree is dirty / missing / not a repo | `appaloft code` | still occupies from remote SHA when origin exists; no file content upload; no `workspace_git_dirty`. A no-git folder resumes the live occupancy without `ls-remote` or an ancestor `$HOME` git remote. With no occupancy, it occupies this folder (`folder.local/cwd/<name>`) after folder-project onboarding. `--new` without a this-folder locator still fail-closes. `workspace open` still requires a git remote or this folder's worktree. |
...
| WS-REMOTE-PROGRESS-201 | No-git `code` occupies and attaches | TTY `code` from a folder with no `.git`; live occupancy exists on the enrolled Server | `appaloft code` | Door resumes that occupancy from stored repo/SHA and attaches. It does not stay on `Resolving repository…`, does not `ls-remote` an ancestor/`$HOME` remote, and does not make git a gate. `--new` without a locator still fail-closes. `workspace open` stays Git-safe. |
...
- CLI: default `appaloft code [path|git-remote] [--no-attach] [--local] [--new] [--profile <name-or-id>]`. A git remote is a locator, not a local path. A no-git folder without an explicit remote resumes the live occupancy; git is not a `code` gate. Default OpenCode omits the invisible `appaloft-remote` name. Durable `appaloft workspace open [path|git-remote]` / `create` accept `--server` and default to the enrolled BYOS Server; those delivery doors still fail-close without a this-folder locator.
