...
| WS-REMOTE-PROGRESS-195 | Source `appaloftdev` builds the occupancy TUI sidecar | TTY `appaloftdev code` in a checkout; `appaloft-workspace-tui` is not on PATH or prebuilt | first TTY run | CLI finds an existing sidecar or `cargo build`s `apps/workspace-control-tui` when the toolchain can produce it. |
