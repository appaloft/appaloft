...
| WS-REMOTE-PROGRESS-201 | No-git `code` occupies this folder | TTY `code` from a folder with no `.git`; another repository's occupancy exists on the enrolled Server | `appaloft code` | Door occupies this folder's project after onboarding. It does not resume `examples` / `truefile` / latest unrelated occupancy, does not stay on `Resolving repository…`, does not `ls-remote` an ancestor/`$HOME` remote, and does not make git a gate. `--new` without a locator still fail-closes. `workspace open` stays Git-safe. |
...
| WS-REMOTE-PROGRESS-205 | Ctrl+C goes to the nested agent | TTY `appaloft code`; session exists | user presses Ctrl+C or Ctrl+R | TUI does not quit. The key is passed through as `\x03` / `\x12`. Occupancy TUI does not treat SIGINT as process quit. `q` and `^]` do not exit the process. Stop typing is shift+esc or focused `^]`. Process quit is only unfocused `^q`, shown in the footer and `?`. |
| WS-REMOTE-PROGRESS-206 | In-session `x` returns to the Cloud Agents list | TTY `appaloft code`; session exists | user presses `x` | TUI shows Appaloft Cloud Agents plus the project name in the list header, keeps `session_id`, and does not send Quit. No user-visible Occupancy. Footer documents `enter connect` and `^q quit`. |
