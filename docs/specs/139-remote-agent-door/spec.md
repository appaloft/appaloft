...
| WS-REMOTE-PROGRESS-197 | TTY enters the TUI before composition | TTY `code`; sidecar already exists | `appaloftdev code` | Shell starts warming the occupancy TUI from the slim launch module (no presentation/application/core/reflect-metadata) before those imports and before `run.ts`. Occupancy TUI enters alt-screen and paints the collapsed **preparing the agent** frame before the parent handshake. Login is not a warmup gate. First useful alt-screen is under 1s. |
...
| WS-REMOTE-PROGRESS-202 | First useful alt-screen is not blocked on composition | TTY `code`; sidecar already exists | `appaloft code` | Shell static-imports `@appaloft/adapter-cli/workspace-tui-launch` only. That module does not import presentation, operate-presentation, `@appaloft/application`, `@appaloft/core`, or `reflect-metadata`. Existing-binary warmup skips cargo/`rustc`. `--no-attach` still skips warmup and works without the TUI binary. |
