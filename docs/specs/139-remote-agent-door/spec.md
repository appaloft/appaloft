...
| WS-REMOTE-PROGRESS-193 | TTY `code`/`workspace` is the occupancy TUI | interactive TTY; not `--no-attach` / `--json` / `--no-tui` | `appaloft code` or `appaloft workspace` | CLI enters the existing occupancy Workspace TUI (alt screen) immediately. Progress is centered/footer text on that TUI from the first frame (`Connecting to Appaloft…`, then occupy / copy skills / attach). It does not stream a pile of stderr status lines as the primary UX. Attach starts as soon as occupancy exists. Skill copy stays fail-soft and time-bounded. `--no-attach` and non-TTY keep one-line stderr progress and the first-byte bar. |
