...
| WS-REMOTE-PROGRESS-198 | Missing TUI sidecar names the binary and next command | TTY `appaloftdev code`; `appaloft-workspace-tui` is missing; rustc is too old or cargo cannot build | first TTY run | CLI does not exit with a one-line `Workspace renderer is unavailable`. The user-visible message names `appaloft-workspace-tui` and the next command (`rustup toolchain install stable` and `cargo build --locked --manifest-path apps/workspace-control-tui/Cargo.toml`), plus a toolchain note when rustc is older than 1.88. `--no-attach` / headless keep working without the TUI binary. |
...
| WS-REMOTE-PROGRESS-203 | Parent enters occupancy alt-screen before sidecar lookup | TTY `code` | `appaloft code` | `enterOccupancyAltScreen` writes `ESC[?1049h` and **preparing the agent** with only `node:fs` `writeSync`. It does not resolve, cargo-build, or spawn `appaloft-workspace-tui`. Sidecar lookup starts after that first frame. |
