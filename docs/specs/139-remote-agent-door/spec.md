...
| WS-REMOTE-PROGRESS-207 | `?` shows the Railway `ca` key table | TTY `appaloft code` | user presses `?` | Help lists Railway keys. Missing capabilities say unavailable. `⌥s` setup is not in this door. s/w/d map to pause/resume/terminate. |
