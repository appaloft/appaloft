...
- [x] occupy / Preparing disk HTTP 502/503 or Cloudflare bad-gateway / origin-incomplete after TUI first paint is a human next step (`1049l` then print); `--omp` retry names hostinger `srv_4lifk0yrcecy`; `--pi` stays Occupancy-free (WS-REMOTE-COMPAT-222)
- [x] occupy / Preparing disk origin HTTP 502/503 keeps the Cloud Agents wait panel up, marks the disk step retrying, and retries disk prep on hostinger `srv_4lifk0yrcecy`; attach waits for success; exhausted failure marks failed, leaves alt-screen once, then WS-REMOTE-COMPAT-222 (WS-REMOTE-PROGRESS-223)
