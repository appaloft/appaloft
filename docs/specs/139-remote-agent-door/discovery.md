...
| D166 | Preparing-disk 502/503 stays on the wait panel and retries | Live pin `c2d18209` / #1348 only remapped the post-flash contract sentence after the TUI already left alt-screen. Required: when Preparing disk hits origin HTTP 502/503 / Cloudflare bad gateway / incomplete origin, the TUI stays on `preparing the agent` / Preparing disk, retries `workspaces.open` on the same enrolled hostinger Server, and attaches only after disk prep succeeds. Occupancy / `sbx_` never appear. First paint stays Cloud Agents + preparing the agent. Only exhausted/final failure does `1049l` then the D165 human next step. Do not touch #1349 first-screen chrome or Cloud admission cap. |
