...
| D158 | Ctrl+C is never TUI quit | Live walk 2026-08-20: TTY `code` Ctrl+C quit the occupancy TUI. Railway session `^c` / `^r` pass through to the agent. Railway `?` documents `^c` as ca-list quit; live walk overrides that for an attached agent. Do not bind quit to `x` or Ctrl+C. |
| D159 | In-session `x` returns to the Cloud Agents list | Live walk 2026-08-20: Railway `x` from a session opens the ca manage list (header stays CLOUD-AGENTS / project; CLI stays up). `q` / `^]` must not exit the process. Dedicated CLI quit is unfocused `^q`, documented in the footer and `?`. |
