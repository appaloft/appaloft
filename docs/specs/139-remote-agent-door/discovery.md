...
| D158 | Ctrl+C is never TUI quit | Live walk 2026-08-20: TTY `code` Ctrl+C quit the occupancy TUI. Railway `^c` / `^r` pass through to the agent; leave typing with shift+esc or `^]`. Quit only via explicit `q` / `^]` when not typing. |
