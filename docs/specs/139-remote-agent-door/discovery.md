...
| D136 | Slice 43 occupancy `Input validation failed` names the enrolled Server | Live Cloud `9ba333eb` rejects git-remote `workspaces.open` `targetServerId` as unstructured `BAD_REQUEST`. Railway would not say only "validation failed". Live pin `54065181` / Cloud #1051 and pin `67e3a052` / Cloud #1053 folder.local `--server` hostinger must not reuse this remap: that door does not POST `/workspaces/open`. After #1342 the folder.local door returns the raw Cloud error; the first write after the Cloud Agents screen is `sandboxes.create`. A remaining unstructured `Input validation failed` must become a human next step (project link / Pi template / Server / skills / credential), never the bare string. |
...
| D156 | Occupancy may offer allowlisted HOME skills | Owner 2026-08-19. Railway cloud agents only sync `~/.claude/skills`, `~/.codex/skills`, `~/.grok/skills`, `~/.agents/skills`, add-only, and fail files >10MB. Their docs do not mention plugins, MCP config, `~/.cursor/skills`, or `~/.config/opencode/skills`. Appaloft occupy may offer those Railway-aligned roots plus `~/.cursor/skills` and `~/.config/opencode/skills` (beyond Railway). Only directories that contain `SKILL.md`. Add-only. Skill-tree copy skips `mcp.json` / tokens / cookies / `.env` / plugin binaries / skill-tree `auth.json`. Copy from the existing CLI occupy `WriteSandboxFile` path; the Sandbox cannot see laptop HOME. First-party Appaloft skill offer stays. No new setup command and not the local Agent door. |
| D157 | Nested agent copy reaches the laptop via OSC 52 | Live walk 2026-08-20: OpenCode/Grok copy inside TTY `code` stayed on the remote agent. Desktop has a clipboard plugin; the CLI TUI must passthrough `\x1b]52` to the local terminal. Do not run remote `pbcopy`. Write failure says the local terminal has OSC 52 disabled; do not fake success. |
...
| D162 | No-git cwd occupies this folder, never leftover examples | Live pin `84678293`: `appaloft code --no-attach` from a no-git cwd still resumed `github.com/appaloft/examples@1a23b77` / `sbx_c343gwqfn7yd` because the door walked an ancestor `.git`. Default `code` and `--no-attach` occupy `folder.local/cwd/<name>` after this-folder onboarding. `--new` without a locator stays fail-closed. Banner/status never say Occupancy, `Copying skills`, or `Choosing occupancy`. No invented sslip / agent public domain. |
| D163 | No-git occupy does not clone or fail-close on missing remote | Live pin `1d8b970e`: after D162, `code --no-attach` from `/home/box/nux-code-silence-cwd` created `prj_7fky4yjn1l1c` then failed `workspace_open_source_materialization_failed` on `git init` / missing remote, and a retry failed `workspace_open_partial_recovery_required`. Live pin `c230a1dc`: application repair did not run on the `--no-attach` door, which still dumped `workspace_open_partial_recovery_required`. Folder occupancy occupies this folder's project on hostinger without `git init`, `remote add`, `fetch`, or clone. The `code --no-attach` door repairs that leftover or silently replaces it (`forceNew` on the already-resolved folder locator) without dumping `workspace_open_partial_recovery_required` or asking the user to pass `--new`. `--new` without a locator stays fail-closed. |
| D164 | Occupancy writes my vendor credential onto my disk | Owner 2026-08-20. Railway `code --claude|--codex|--grok` writes vendor auth over SSH onto the VM, not as variables. Appaloft `code` must do the same through `WriteSandboxFile` onto occupancy HOME (`HOME=/workspace`). Grok/Codex copy laptop `auth.json`. Claude copies a setup-token, not `~/.claude/.credentials.json`. `--opencode/--pi/--omp` are our aliases; `--claude/--codex/--grok` are Railway-aligned. One alias per invocation. `--harness` is compatibility only. Default follows the saved occupancy-agent preference (like Railway `ca setup`), then laptop login/install. First-party Appaloft skill + secret-free Appaloft MCP stay. Connecting-step data is produced; TUI layout is another agent. Not the local setup-agent door (#1316). No `:8080` agent hostname. Mental verification only against Hostinger `srv_4lifk0yrcecy`. Banner/status still never say Occupancy, `Copying skills`, or `Choosing occupancy`. |
| D165 | Preparing-disk Cloudflare 502/503 is a human next step | Live confirmed 2026-08-21 on user Mac, pin `414d63e8` / appaloft-cloud main. `appaloftdev code --omp` on hostinger `srv_4lifk0yrcecy` drew Cloud Agents, Checking login ✓, Preparing skills ✓, Preparing disk ! red, then flash-quit with only `The server returned an error that did not match the Appaloft error contract. HTTP 502 Body: {cloudflare 502 bad gateway, origin invalid or incomplete response}`. Same contract as #1334 / WS-REMOTE-COMPAT-220: after TUI first paint, `1049l` then print a human next step. Remap occupy 502/503 / Cloudflare bad gateway / origin incomplete to Cloud temporarily unreachable + retry `code --omp --server srv_4lifk0yrcecy` (`--pi` when that is the opened agent). Keep HTTP status. Do not dump the Cloudflare body. Do not say Occupancy or `sbx_`. Do not touch #1347 / Operation check denied / create-project org / deploy cwd / SSH. Do not try to fix Cloudflare or deploy production. |
| D166 | Preparing-disk 502/503 stays on the wait panel and retries | Live pin `c2d18209` / #1348 only remapped the post-flash contract sentence after the TUI already left alt-screen. Live pin still flashed: `1049l` before the error and another `1049l` after, leftover spinner `⠼⠼`, stacked clipped titles `appaloft-clo` / `appaloft-clocloud`, Preparing disk never showed red `!`. Required: when Preparing disk hits origin HTTP 502/503 / Cloudflare bad gateway / incomplete origin, the TUI stays on `preparing the agent` / Preparing disk, marks the disk step retrying (or failed on the last attempt), retries `workspaces.open` on the same enrolled hostinger Server, and attaches only after disk prep succeeds. Occupancy / `sbx_` never appear. First paint stays Cloud Agents + preparing the agent. Only exhausted/final failure leaves alt-screen **once, clean** (close rust first; no double `1049l`, no leftover spinner, no stacked clipped titles) then the D165 human next step. Do not touch #1349 first-screen chrome or Cloud admission cap. |
| D167 | Preparing disk must become a live session or fail closed | Live pin `9f901309` / #1364 stay/retry on hostinger `srv_4lifk0yrcecy` from linked cwd `/Users/nichenqin/projects/appaloft-cloud`. `appaloftdev code --pi --server srv_4lifk0yrcecy` first-painted preparing the agent in 85ms, then sat on Preparing disk ~87s. Raw HTTP 502 did not paint. `Opening folder.local` did not appear. Ctrl-C then empty after-alt and CLI exit 0. Never a live SESSION (no typing, no tree/session editor). Unlimited stay/retry plus swallowing remapped 502/SIGINT left the panel spinning: a hang inside the first `await open()` is not a 502 so the helper did not retry, and abort was only checked between attempts. Required: retry occupy until attach + terminal-ready or a deadline; abort the in-flight open on Ctrl-C / quit; if it never attaches, fail closed with a human Cloud-unreachable / disk-prep sentence, no `Opening folder.local`, non-zero exit. Do not paint occupancy jargon. Do not regress D165/D166 flash-crash or folder.local breadcrumb. Do not mix with deploy/nux parent-path work. |
| D168 | Default `code` replaces a partial preferred Workspace without making the user pass `--new` | Owner production feedback 2026-08-22 / #1374: `appaloftdev code --codex` from the Git-backed `appaloft-cloud` folder restored the TTY and exited `workspace_open_partial_recovery_required`; that is not an accepted one-command Agent door. On this exact structured error, the `code` presentation retries once with the same resolved door and `forceNew: true`. It preserves the partial Sandbox and creates an isolated replacement. Lower-level `workspace open` keeps exact partial recovery evidence and fail-closes. No automatic terminate, source rewrite, or retry of unrelated errors. |
| D169 | First paint is the wait panel; reopen must not recreate disk | Owner 2026-08-24. Empty parent splash (only `Appaloft Cloud Agents` / `preparing the agent` in the corner) is not a product screen — Codex 0.148 shows the real TUI immediately. Parent first frame must already be the centered wait panel. Preparing disk is always slow because live Cloud `sandboxes.create` still cannot persist occupancy extras (pre-#1339), so the next open cannot match preferred and creates another container. CLI keeps a local last-workspace resume keyed by folder + Server + Profile and resumes that listed Sandbox instead of creating. |
...
- Copy laptop `mcp.json` secrets, chat cookies, `.env`, or editor plugin binaries into occupancy.
...
| Personal `auth.json` copied to my VM | Occupy writes my laptop vendor `auth.json` / Claude setup-token onto my occupancy HOME |
