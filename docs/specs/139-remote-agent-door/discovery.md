...
| D163 | No-git occupy does not clone or fail-close on missing remote | Live pin `1d8b970e`: after D162, `code --no-attach` from `/home/box/nux-code-silence-cwd` created `prj_7fky4yjn1l1c` then failed `workspace_open_source_materialization_failed` on `git init` / missing remote, and a retry failed `workspace_open_partial_recovery_required`. Folder occupancy occupies this folder's project on hostinger without `git init`, `remote add`, `fetch`, or clone. The next `--no-attach` repairs that leftover partial on this folder's project, or replaces it without dumping `workspace_open_partial_recovery_required` or asking the user to terminate. `--new` without a locator stays fail-closed. |
