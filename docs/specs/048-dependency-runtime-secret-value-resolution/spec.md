...
| --- | --- | --- | --- | --- |
| DEP-BIND-SECRET-RESOLVE-001 | Store imported Postgres connection value | Operator imports an external Postgres dependency with a raw connection URL and no external `secretRef` | `dependency-resources.import` succeeds | The raw connection URL is stored through the dependency secret-value store, the dependency resource persists only a safe Appaloft-owned secret reference and masked endpoint summary, and list/show/events/logs omit the raw URL. |
| DEP-BIND-SECRET-RESOLVE-002 | Store imported Redis connection value | Operator imports an external Redis dependency with a raw connection URL and no external `secretRef` | `dependency-resources.import` succeeds | The raw Redis URL is stored through the dependency secret-value store, the dependency resource persists only a safe Appaloft-owned secret reference and masked endpoint summary, and list/show/events/logs omit the raw URL. |
...
| DEP-BIND-SECRET-RESOLVE-008 | Connect Compose services to a realized single-server dependency | A local-shell or generic-SSH Compose deployment resolves a dependency URL whose host is the Appaloft-managed container for the captured dependency resource | the ownership override is generated before `compose up` | Every service in the Compose project is attached to the existing Appaloft managed-dependency network, the top-level external network is declared once, and route-specific attachment does not duplicate the same network key. Imported external dependency URLs do not select this network. |
...
adapters render safe handles. Postgres and Redis closed-loop verification is covered by
`DEP-RES-PG-CLOSED-LOOP-001` and `DEP-RES-REDIS-CLOSED-LOOP-001`.
