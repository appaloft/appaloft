...
describe("organization auth management console surface", () => {
  ...
  test("[SETTINGS-WEB-001] account and organization settings use sidebar shell and neutral contracts", async () => {
    ...
    expect(accountSecuritySource).toContain('from "$lib/console/layout-classes"');
    expect(accountSecuritySource).toContain("subnavLayoutClass");
    expect(accountSecuritySource).toContain("subnavContentClass");
