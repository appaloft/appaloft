    ...
    while running && !stop.load(Ordering::Relaxed) {
        ...
        for event in events {
            ...
            match event {
                ...
                Event::Key(key)
                    if key.kind == KeyEventKind::Press && state.recovery_form.is_some() =>
                {
                    match key.code {
                ...
                Event::Key(key)
                    if key.kind == KeyEventKind::Press && state.delivery_form.is_some() =>
                {
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            if let DeliveryDecision::Dispatch(submission) =
                ...
                Event::Key(key) if key.kind == KeyEventKind::Press && state.action_menu_open => {
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            if let ActionDecision::Dispatch(action) =
