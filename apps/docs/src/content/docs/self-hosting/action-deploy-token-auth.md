...
## Action deploy token 认证 [#self-hosting-action-deploy-token-auth]
...
## 安装后保存 GitHub Secret [#self-hosting-action-token-setup]
...
## Scope 和当前限制 [#self-hosting-action-token-scope]

Deploy token 可以限制到 workflow command、project、environment、resource、server 和 repository。
当 token 唯一指定 project、environment、resource 和 server 时，普通 self-hosted Action deploy 可以
不传这些 ids；server 会结合 token scope、source-link 和 repository facts 解析目标。当前安装器创建
的是初始自托管 Action token，用来让已有 server API deploy、server config deploy 和 preview cleanup
路径先具备认证边界。
...
## 401 和 403 [#self-hosting-action-auth-errors]
...
project、environment、resource、server 或 workflow command 不匹配。显式 bootstrap ids 或已有
source link 指向 token scope 之外的目标时也会返回 403。请更新 token scope、重新 link source，或移除
冲突的 bootstrap ids。

## 轮换和撤销 [#self-hosting-action-token-rotation]
