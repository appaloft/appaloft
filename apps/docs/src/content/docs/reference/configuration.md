...
| `APPALOFT_WORKER_SLOT` | unset | 当前进程的可选显式 slot。Docker Compose/Kubernetes replicas 场景保持 unset；standalone worker 会通过共享 heartbeat store 租约领取一个 slot，`APPALOFT_WORKER_COUNT` 仍表示整个 group 的预期容量。 |
