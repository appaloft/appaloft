...
```bash
appaloft up                                   # 部署当前目录
appaloft setup agent                          # 在本机写入 Skill 与 MCP
appaloft code                                 # 打开 Workspace 会话
...
## Up：第一次部署入口 <a id="cli-up-deployment-entrypoint" />

```bash
appaloft up [path-or-source] [--yes] [--project <projectId>] [--json]
```

`appaloft up` 是从文件夹到线上应用的主入口。省略路径时使用当前目录；Git 不是前置条件。
...
使用 `appaloft up`。完整任务流程见[第一次部署](/docs/start/first-deployment/#cli-up-first-deployment)。

## Workspace 快速入口

`appaloft code` 登录后在团队默认已注册 Server 上打开 Workspace 会话。源是远端 SHA。
...
`appaloft workspace open [path|git-remote]` 与 `code` 使用同一套定位参数，并走
`workspaces.open`。Git worktree 仍要求 clean 且已 push；非 git 目录通过 git remote
占用当前目录，不会复用无关会话。完整说明见
[从本地目录打开 Agent](/docs/agents/workspaces/#agent-workspace-open)。
...
[管理正在运行的 Workspace](/docs/agents/workspaces/#workspace-control-tui)。
...
有 live outbound Server Worker 时添加 `--server <serverId>`，无需 inbound SSH。完整配置、状态、
HTTPS、watch 和 cleanup 说明见[本地开发会话](/docs/start/local-development/#local-development-session)。
...
`appaloft login` 和 `appaloft auth login` 默认连接 Appaloft Cloud（`https://app.appaloft.com`）。第一次 Cloud `appaloft up` 在没有 Profile 时会启动同一套浏览器登录，然后继续部署——不必先单独跑 `appaloft login`。`appaloft deploy` 在 1.x 中走同一兼容路径。要连接自托管 Appaloft 或其他受信任端点，需要显式传入 `--url <url>`。登录成功后，CLI 会把端点、Profile 名称、认证引用和握手摘要保存到本机 CLI Profile——这个 Profile 位于 `APPALOFT_HOME` 或用户本机 Appaloft home 目录，**不属于仓库配置**，不会被提交到 `appaloft.yml`。
...
## MCP 宿主安装 <a id="cli-mcp-host-install" />
...
```bash
appaloft up --control-plane-mode self-hosted --control-plane-url https://console.example.com
```
