...
`npx skills add` 只复制 Skill，不会写入 MCP。Cursor 和 OpenCode 复用已经登录的 Appaloft profile，把 token-free 的 stdio 启动项写入编辑器配置：
...
Cursor 合并 `~/.cursor/mcp.json` 的 `mcpServers.appaloft`。OpenCode 合并 `~/.config/opencode/opencode.json` 的 `mcp.appaloft`。Codex 仍需要专用 bearer profile：`appaloft auth mcp login` 然后 `appaloft auth mcp codex install`。无 git 的目录可以用 `appaloft deploy .`。
