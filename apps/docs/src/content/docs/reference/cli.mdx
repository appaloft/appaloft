...
`appaloft code` 登录后打开远端 Agent 会话，需要已注册 Server 和 Project Binding。笔记本路径只
用来定位 Binding，脏文件不会上传。`--no-attach` 只打印身份横幅。`--local` 才是本机 Scratch。
...
`--profile` 和 `--new` 仍走远端 `workspaces.open`，不检查脏笔记本树。`workspace open` 仍要求
clean 且已 push 的 Git branch。完整说明见
