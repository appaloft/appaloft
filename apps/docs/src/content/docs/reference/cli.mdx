...
appaloft code                                 # 打开默认 Server 上的远端 Agent 会话
...
`appaloft code` 登录后打开远端身份门，需要已注册 Server。解析 Server 和 Binding SHA 后，
本机 native-attach OpenCode/Pi。不要求 Adapter、Profile 或 Sandbox。脏文件不会上传。
`--no-attach` 只打印身份横幅。`--local` 才是本机 Scratch。
...
`workspace open` 仍要求 clean 且已 push 的 Git branch，并走 `workspaces.open`。完整说明见
