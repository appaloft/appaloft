...
appaloft code                                 # 在本机打开本地 Scratch Agent 会话
...
`appaloft code [path]` 在本机打开 Scratch 会话。不要求 Git、登录、Binding 或 Cloud。path 默认为
`.`。`--no-attach` 只打印 scratch 横幅，不启动 Agent TUI。
...
`--profile` 和 `--new` 仍是 durable `workspaces.open`，要求 clean 且已 push 的 Git branch。已有
自动化可继续使用 `appaloft workspace open .`。完整说明见
