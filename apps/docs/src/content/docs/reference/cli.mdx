...
**preparing the agent** 后再 attach。`--no-attach` 用一行 stderr 进度和占用横幅，不 attach。默认 `code` 会选 live occupancy，
同名 leftover Profile 不能挡住第一次成功。`--profile` 只是兜底。`--new` 占用 cwd
源，不会静默 resume whoami。打开失败会写出正在打开的仓库以及缺少的 Binding 或
Profile。已注册 BYOS 就是放置目标，`--server` 可钉死，不要求 managed。`--local` 才是本机
Scratch。
...
一条命令会给已检测到的 Cursor 和 OpenCode 同时写入 Skill 和 Local MCP。Token 留在 Appaloft CLI profile，不会写入编辑器配置：
...
`appaloft setup agent` 会把 Skill 复制到已检测的 Cursor / OpenCode Skill 目录（`~/.cursor/skills`、`~/.config/opencode/skills`）以及 `~/.agents/skills`，再写入 token-free 的 stdio 启动项。Cursor 合并 `~/.cursor/mcp.json` 的 `mcpServers.appaloft`。OpenCode 合并 `~/.config/opencode/opencode.json` 的 `mcp.appaloft`。显式 sibling 仍然可用：`appaloft auth mcp cursor install` 和 `appaloft auth mcp opencode install`。`npx skills add` 仍然只复制 Skill。Codex 仍需要专用 bearer profile：`appaloft auth mcp login` 然后 `appaloft auth mcp codex install`。无 git 的目录可以用 `appaloft deploy .`。
