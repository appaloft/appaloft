...
## 重新关联部署来源 [#deployment-source-relink]
...
## 清理 Preview 部署 [#deployment-preview-cleanup]
...
## 检查部署恢复就绪 [#deployment-recovery-readiness]
...
## Agent 部署恢复 [#agent-deploy-recovery]
...
## 处理长时间无活动的 attempt [#deployment-recovery-stale-attempts]
...
## 归档和清理 attempt [#deployment-recovery-archive-prune]
...
## 重试还是回滚 [#deployment-retry-or-rollback]
...
## 入口差异 [#deployment-recovery-surfaces]
