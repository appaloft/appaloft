...
预览会显示检测到的 framework/runtime 证据、选中的 planner、support tier、artifact 类型、install/build/start/package 命令、内部端口、健康检查、访问路由摘要、warning 和 unsupported reasons。用户可以据此先修 resource source/runtime/network/health/access profile，再运行真正的部署。
