...
如果原始 deploy 命令已经断开，但仍需要结构化 replay 或 live event stream，可以使用 `appaloft deployments events <deploymentId> --follow --json`。这个 stream 可能返回 event、heartbeat、gap、closed 或 error envelope；gap 表示观测连续性不完整，应重新打开观测或查看 deployment detail 后再决定恢复动作。
