...
这个命令只通过共享的 `deployments.create` 合约发送既有上下文 ID，不读取或上传本地源码包和仓库配置。返回 deployment id 只表示 attempt 已接受；仍需通过 timeline 和 proof 命令验证执行结果。
...
<h3 id="deployment-proof">部署证明</h3>
