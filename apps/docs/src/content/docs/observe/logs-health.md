...
## 运行时控制 [#resource-runtime-controls]
...
## Restart 不是 Redeploy [#runtime-restart-vs-redeploy]
...
## Start 被阻塞时 [#runtime-control-blocked-start]
...
## 入口说明 [#observe-log-health-surfaces]
...
## Agent 部署后检查 [#agent-deploy-follow-up]

Agent 完成部署后应该把日志和健康摘要作为后续检查入口，而不是让用户寻找服务器原始日志。推荐返回：
...
如果访问地址不可用，先读取这些结构化信息，再给下一步操作。

## 如何根据结果恢复 [#observe-log-health-recovery]
