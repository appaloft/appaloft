...
重命名服务器只修改显示名称。它不会改变 server id、host、provider、credential、proxy、lifecycle 或历史部署/域名/审计引用。active 和 inactive 服务器都可以通过 CLI/API 或 Web 服务器详情页重命名；已经删除的服务器不会出现在普通重命名入口中。
...
停用服务器用于阻止它继续作为新的部署、调度或代理配置目标。停用不会停止已有运行任务，也不会删除部署历史、域名、证书、凭据、路由、日志或审计记录。CLI/API 和 Web 服务器详情危险操作页使用同一个单向停用命令；Web 表单需要输入完全一致的 server id 才会提交。
...
SSH 连接结果会区分 host 解析失败、网络/主机不可达、认证失败和 host-key 验证失败。
Appaloft 将规范化 host 与 port 分别传给 OpenSSH；`known_hosts` 与严格 host-key 检查仍由
OpenSSH 管理。IPv6 endpoint 显示为 `[2001:db8::1]:22`。
