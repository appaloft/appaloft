...
## 宿主安装 <a id="appaloft-mcp-host-install" />
...
`appaloft setup agent` 会把 Skill 复制到已检测宿主的 `~/.cursor/skills/appaloft`、`~/.config/opencode/skills/appaloft` 和 `~/.agents/skills/appaloft`，再写入与 sibling 命令相同的 Local MCP 启动项。Cursor 合并 `~/.cursor/mcp.json` 的 `mcpServers.appaloft`。OpenCode 合并 `~/.config/opencode/opencode.json` 的 `mcp.appaloft`（`type: "local"`，`command: [...]`，`enabled: true`）。两者都启动 `appaloft mcp remote-stdio --profile <active-or-requested>`。

显式 sibling 仍然可用：`appaloft auth mcp cursor install` 和 `appaloft auth mcp opencode install`。`npx skills add` 仍然只复制 Skill。Codex 仍需要专用 bearer profile：`appaloft auth mcp login` 然后 `appaloft auth mcp codex install`。无 git 的目录可以用 `appaloft deploy .`。
