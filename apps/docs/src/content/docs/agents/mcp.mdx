...
`npx skills add` 不会写入 MCP。在已经登录的本机 Appaloft profile 上，用同一命令家族写入宿主配置（编辑器文件里不存放 token）：
...
Cursor 会合并 `~/.cursor/mcp.json` 的 `mcpServers.appaloft`。OpenCode 会合并 `~/.config/opencode/opencode.json` 的 `mcp.appaloft`（`type: "local"`、`command: [...]`、`enabled: true`）。两者都启动 `appaloft mcp remote-stdio --profile <当前或指定 profile>`。Codex 仍需要专用 bearer profile：`appaloft auth mcp login` 然后 `appaloft auth mcp codex install`。无 git 的目录可以用 `appaloft deploy .` 部署。
