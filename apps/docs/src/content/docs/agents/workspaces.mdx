...
CLI 只用 `.` 查找 Git root、upstream remote、branch 和 HEAD SHA，不会上传本地目录或未提交
...
```bash
appaloft workspace open . --new
```

只创建/恢复并输出 descriptor，不进入 Agent 界面：

```bash
appaloft workspace open . --no-attach
```
...
Terminal Session 由 Appaloft 托管的 PTY 支撑——客户端断线只是"detach"，只要 Session 的存活时间和 Sandbox 仍然有效，再次运行 `workspace open` 就会重用 Session、重放有限历史输出并继续同一个 Agent 进程：
