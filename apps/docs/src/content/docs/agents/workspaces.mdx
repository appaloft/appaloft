...
工作区列表**折叠**，等待占满整窗。父进程第一帧就已经是中间等待面板（**preparing the agent**
加 Checking login / Preparing skills / Preparing disk），不是只有左上角两行的空屏。
occupy / copy skills / attach 作为左对齐步骤（Braille spinner，完成后 ✓）。attach
在同一全屏窗格里接上。再次打开同一 folder 时，若上次占用的磁盘还在，会 resume 而不是再 create。
`workspace` 打开同一个 TUI 作为导航。不以刷 stderr 状态行为主 UX。
...
`<name>` 是 Agent 的可读名，不是 `sbx_…`。git 占用会生成 kebab 名，例如
