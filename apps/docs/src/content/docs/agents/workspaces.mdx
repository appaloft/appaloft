...
若 **Preparing disk** 因为 Cloud 暂时不可达而失败（HTTP 502 / 503，包括
Cloudflare bad-gateway 页面），Cloud Agents 画面会先恢复终端，再打印下一步，
例如 `appaloft code --omp --server srv_4lifk0yrcecy`。`--omp` 是 OpenCode
