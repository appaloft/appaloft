...
clean 且已 push。`--profile` 在 `appaloft code` 和 `appaloft workspace open` 上都选择
Agent Workspace Profile，不同于全局 `--control-plane-profile`。默认 `code` 优先选有
live occupancy 的安装，而不是被同名死 Profile 挡住。脏树、detached、缺 upstream 或 remote tip
