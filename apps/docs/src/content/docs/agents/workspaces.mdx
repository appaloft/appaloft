...
第一次交互式 `appaloft code` 会问要不要带上本地 skills；如果本机有多个根，再问读哪一个目录。
答案写在 `~/.appaloft/agent-prefs.json`。之后启动沿用这次选择。`--yes` 或非 TTY 首次运行会把同步关掉。
...
因为这是用户实际使用的根；这两项**超出 Railway 文档范围**。占用只复制已保存的那一个根，
不会一次拷全部。
