...
默认 `appaloft code [path]` 打开本机 Scratch 会话，path 默认为 `.`。空目录、脏工作树、detached
HEAD、未 push、未登录、没有 Binding/Profile/Cloud 都可以进入。CLI 不会调用 `workspaces.open`，
也不会上传本地文件。横幅是：
...
默认先找本机 `opencode`，否则 `pi`。都没有时提示安装；拒绝安装是 attach 前唯一硬失败。
Scratch 会向本机 Agent 提供公共 Appaloft skill，不会改所选目录里的项目文件。
`--no-attach` 只解析 scratch、打印横幅和 harness，然后退出。

durable Workspace 仍用：
...
这些路径继续走 `workspaces.open`：只用 path 查找 Git root、upstream、branch 和 HEAD SHA，要求
...
Terminal Session 由 Appaloft 托管的 PTY 支撑——客户端断线只是"detach"。只要 Session 的存活时间和
Sandbox 仍然有效，再次运行 `appaloft workspace open` 就会重用 Session、重放有限历史输出并继续
同一个 Agent 进程：
...
默认 `appaloft code` 会打开新的本机 Scratch 会话，不会重连 durable Workspace。已有脚本可以继续
使用 `appaloft workspace open .`；它不会被弃用或改变机器可读行为。
