...
无需本地 clone 即可占用该仓库。TTY 上默认 `code` / `workspace` 会立刻进入占用 Workspace TUI
（备用屏），首帧就在中间/页脚显示进度（`Connecting to Appaloft…`，然后 occupy / copy skills /
attach），不以刷 stderr 状态行为主 UX。占用一返回就 attach。HOME skill 复制限时且失败软处理。
`--no-attach` 和非 TTY 脚本仍用一行 stderr 进度（含进程启动后的第一字节状态行），并打印 Remote
横幅。
横幅是：
