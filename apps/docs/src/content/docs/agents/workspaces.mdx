...
默认 `appaloft code` 先解析团队默认已注册 Server 和 Binding 远端 SHA，再在本机 native-attach
OpenCode（否则 Pi，否则 Oh My Pi）。不要求 Adapter、Profile 或 Sandbox。笔记本路径不是
Workspace 真相，脏树也不会上传。横幅是：
