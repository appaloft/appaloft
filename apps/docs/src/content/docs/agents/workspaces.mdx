...
<Aside type="caution" title="成熟度：Public alpha">
  Workspace 的 CLI、SDK 和底层操作属于公开 Appaloft 能力；具体 Sandbox
  Provider、模板、网关和公网地址能力取决于部署方的运营配置。
</Aside>
...
`--claude`、`--grok` 选择这台笔记本上的厂商凭证，并映射到 OpenCode Cloud Agents
运行时。`--codex` 在远端 Sandbox 启动 Codex，并把 `~/.codex/auth.json` 写到会话
HOME。`--opencode`、`--pi`、`--omp` 选择对应 Cloud Agents harness。`--harness
opencode|pi|omp|codex` 只作兼容。未传别名时，`code` 使用已保存的 Cloud Agents
...
删除远端文件不会撤销对应的上游登录会话。若凭证可能已经泄露，还要在 Codex / OpenAI
账号安全设置中撤销对应会话。
...
<a id="agent-workspace-preparing-disk" />若 **Preparing disk** 遇到暂时的 Cloud 网关失败（HTTP 502 /
