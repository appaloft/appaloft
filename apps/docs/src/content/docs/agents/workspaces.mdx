...
在 clean 且已经 push 的 Git branch 中运行：
...
部署继续 fail closed，并给出原有设置入口。initializer 必须在创建前完成其部署所需的授权与 admission，
且不能覆盖已有、冲突、disabled 或未授权状态。
...
成功结果和 Workspace detail 会返回安全的 activation 与目标选择证据：

- activation 中的 Project、Repository Binding 和 Profile 分别标记为 `created` 或 `reused`；
- target class 是 `managed`、`registered-server` 或 `local`，selection source 是
  `platform-default`、`saved-policy` 或 `explicit`，并带稳定 reason；
- 证据不包含 Server id、host、provider handle、容量探针或 credential。再次 open 恢复同一
  Workspace 时会返回原证据，不重新 placement；旧 Workspace 无证据时明确显示
  `legacy-unclassified`，不会猜测执行位置。

部署方的 entitlement、target policy 或容量失败必须在 Sandbox/provider effect 前失败。错误可以提示
等待、重试或显式接入其他执行位置，但 Appaloft 不会静默从 managed 切换到 registered Server 或本机。
