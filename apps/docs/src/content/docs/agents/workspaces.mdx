...
`appaloft code --claude`、`--codex`、`--grok` 选择这台笔记本上的厂商凭证。它们映射到
现有 occupancy 运行时（默认 OpenCode）。`--harness opencode|pi|omp` 仍是高级覆盖，
不会发明新的运行时。未传厂商旗标时，`code` 跟随这台笔记本已登录或已安装的厂商。
