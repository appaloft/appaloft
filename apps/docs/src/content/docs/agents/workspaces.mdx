...
修改状态。交付操作不会 detach 当前 Agent Session。带 deployment identity 的 Promotion 会查询
...
approve/deliver`、`sandbox promote accept/retry` 和 `deployment proof`。

这些路径不会初始化 renderer，并返回稳定的 headless 状态或继续执行现有子命令。Windows 当前仅
保证 help/headless 命令安全；嵌入模式需要独立的 release 与终端验收。
