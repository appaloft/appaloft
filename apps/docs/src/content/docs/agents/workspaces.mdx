...
clean 且已 push。默认 `appaloft code` 优先选 live occupancy，同名 leftover 不能挡住第一次成功，
...
可钉死），不会再索要 managed 容量。脏树、detached、缺 upstream 或 remote tip
不一致会在任何远端创建前失败。V1 不做隐式 sync 或 patch upload。
