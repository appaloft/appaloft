...
Appaloft Skill 是面向 AI Agent 的完整产品入口，与 CLI、HTTP API、Web 控制台、MCP 工具一样，都映射到同一套 Appaloft 业务操作（operation catalog）——区别只是使用者是 AI，而不是人类。它**不是**新的业务操作，也不是 `appaloft up` / `appaloft deploy` 的包装器。
