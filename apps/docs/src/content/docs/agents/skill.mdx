...
显式 MCP sibling 仍然可用：`appaloft auth mcp cursor install`、`appaloft auth mcp claude-code install` 和 `appaloft auth mcp opencode install`。OpenCode Skill/MCP 需要 `--agent opencode` 或这些 sibling。Codex 仍使用专用 bearer MCP profile：`appaloft auth mcp login` 然后 `appaloft auth mcp codex install`。无 git 的目录可以直接 `appaloft deploy .`。
