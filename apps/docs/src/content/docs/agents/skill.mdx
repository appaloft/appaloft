...
Codex 仍使用专用 bearer MCP profile：`appaloft auth mcp login` 然后 `appaloft auth mcp codex install`。无 git 的目录可以直接 `appaloft deploy .`。
