...
Source profile 改变后，新的部署会读取新来源。已经完成或正在执行的部署继续使用自己的部署快照。
...
配置后可以通过资源详情确认 source/runtime profile 摘要。下一次部署会把这些 profile 写入部署快照。部署失败时先看失败阶段：detect 多半是 source 问题，plan 多半是 source/runtime/profile 匹配问题。
