...
## 依赖资源生命周期 [#dependency-resource-lifecycle]
...
## Blueprint 依赖协议 [#blueprint-dependency-contract]
...
## Blueprint catalog 和安装 [#blueprint-catalog-installation]
...
## 绑定到 Resource [#dependency-resource-binding]
...
## 部署已绑定的依赖 [#dependency-runtime-injection]
...
## Runtime injection 被阻塞 [#dependency-runtime-injection-blocked]
...
## Binding secret 轮换 [#dependency-secret-rotation]
...
## 备份和恢复 [#dependency-backup-restore]

Backup 会创建 safe restore point；restore 默认恢复到原 dependency，也可以通过
`--target-dependency` 恢复到同项目、同环境中已 ready 的同类型 dependency。两种方式都必须明确确认
数据可能被覆盖，且 runtime 不会自动重启。
...
跨资源恢复会记录目标，但不会自动切换应用流量。恢复不会修改 ResourceBindings、deployment
rollback/redeploy 状态、运行时进程或历史 deployment snapshot。
...
## 在 appaloft.yaml 中引用已导入依赖 [#dependency-config-file]
