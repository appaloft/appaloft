...
| 输入 | 说明 | 默认值 |
| --- | --- | --- |
| Project | 资源、环境和部署历史的组织边界 | 第一次在未关联文件夹里 `up`/`code` 时按目录名创建，或使用你唯一的项目；也可用 `project use` |
...
## CLI 操作步骤 <a id="cli-up-first-deployment" />

```bash
# 1. `up` 是从当前文件夹部署的主入口，会在需要时直接登录。自托管仍可先跑
#    `appaloft login --url https://your-appaloft-host`。
#    编码 Agent / CI / 非 TTY 环境会先打印计划，除非传入 --yes。
appaloft up

# 2. 创建项目（可选）。第一次 up/code 也会按目录名创建并记住这个文件夹
...
# 4. 已经登录时，同一条文件夹 up 会继续创建或复用这个应用。
#    目录里有 public/index.html 时会自动选 static，不必再写 --method。
#    源目录保持当前文件夹本身（带连字符的名字也合法），不会收成上一级。
#    SSH 打包也用这个完整目录；缺目录时报这个完整路径，不会报上一级。

# 5. 或者显式指定配置文件 / profile
appaloft up --config appaloft.yml --config-profile staging

# 6. 已经构建好的静态目录，跳过构建步骤直接发布。`.` 表示把该目录本身当作发布根。
...
`appaloft deploy` 在 1.x 中仍是同一工作流的受支持兼容写法；新命令、文档和自动化应优先使用
`appaloft up`。两种写法共享登录、文件夹关联、`--yes` 防护、`--json` 输出和终态验证。
...
`appaloft up` 会等到部署进入终态后再退出。构建失败（例如 SSH Docker 镜像构建失败）必须非零退出，并且不会把预留/旧 URL 当成成功地址打印出来。
