...
| 输入 | 说明 | 默认值 |
| --- | --- | --- |
| Project | 资源、环境和部署历史的组织边界 | 第一次在未关联文件夹里 `deploy`/`code` 时按目录名创建，或使用你唯一的项目；也可用 `project use` |
...
```bash
# 1. 第一次 deploy 会在需要时直接登录（Railway `up` 手感）。自托管仍可先跑
#    `appaloft login --url https://your-appaloft-host`。
#    编码 Agent / CI / 非 TTY 环境会先打印计划，除非传入 --yes。
appaloft deploy
appaloft deploy .

# 2. 创建项目（可选）。第一次 deploy/code 也会按目录名创建并记住这个文件夹
...
# 4. 已经登录时，同一条文件夹 deploy 会继续创建或复用这个应用。
...
`appaloft deploy` 会等到部署进入终态后再退出。构建失败（例如 SSH Docker 镜像构建失败）必须非零退出，并且不会把预留/旧 URL 当成成功地址打印出来。
