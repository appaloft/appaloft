...
当 Agent 宿主配置了 Appaloft MCP 时，使用 `appaloft mcp stdio`、`appaloft mcp serve` 或 `npx appaloft-mcp` 暴露同一套 operation catalog。Skill 复制不会写入 MCP；Cursor / OpenCode 用已有命令写入宿主配置：
...
# 把 token-free 的 MCP 条目写入 Cursor 或 OpenCode
