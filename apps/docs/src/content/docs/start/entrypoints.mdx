...
当 Agent 宿主配置了 Appaloft MCP 时，使用 `appaloft mcp stdio`、`appaloft mcp serve` 或 `npx appaloft-mcp` 暴露同一套 operation catalog。`npx skills add` 只复制 Skill，不写 MCP。登录后 `appaloft setup agent` 会给默认勾选的 Cursor / Claude Code 写入 Skill 和 MCP。OpenCode 需要 `--agent opencode` 或 sibling 安装：
...
# 一条命令写入本地 Agent 门；OpenCode 需显式安装
appaloft setup agent
appaloft auth mcp opencode install
```

MCP 工具复用与 CLI/API 相同的操作、输入解释和恢复说明，详见 [MCP 与工具协议](/docs/agents/mcp/)。在 Grok Bot 里安装 [Grok Bot 插件](/docs/agents/grok-bot-plugin/) 即可接到同一套操作；那条路径用托管 MCP [`https://app.appaloft.com/mcp`](https://app.appaloft.com/mcp) 做 OAuth，不会取代本机的 `appaloft setup agent`。
