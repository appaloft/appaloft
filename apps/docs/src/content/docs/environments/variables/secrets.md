...
资源级 secret 可以用 `appaloft resource secrets create` 创建，用 `rotate` 轮换，用 `delete`
移除，用 `list`/`show` 查看 masked 状态。这些操作只影响后续部署快照，不会热更新正在运行的实例。

新 secret 应通过标准输入传入，避免出现在进程参数列表中：
...
`--stdin` 不能与 positional value 同时使用。Appaloft 会移除一个末尾换行，拒绝空输入，
并且不会打印 secret。
