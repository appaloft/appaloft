...
| --- | --- | --- |
| Project | The organizing boundary for resources, environments, and deployment history | First `deploy`/`code` in an unlinked folder creates one named after the directory, or uses your only project; `project use` switches |
...
```bash
# 1. First deploy signs you in when needed (Railway `up` feel). Self-hosted can still
#    run `appaloft login --url https://your-appaloft-host` first.
#    Coding-agent / CI / non-TTY runs print a plan unless you pass --yes.
appaloft deploy
appaloft deploy .

# 2. Create a project (optional). First deploy/code also creates one named after this folder
...
# 4. Already signed in? The same folder deploy continues to create or reuse this app.
...
appaloft deploy . --as static-site --publish-dir .
...
`appaloft deploy` waits until the deployment reaches a terminal status. A build failure (including SSH Docker image build failure) exits non-zero and does not print a reserved or previous URL as success.
