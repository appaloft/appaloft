...
When an agent host is configured with Appaloft MCP, `appaloft mcp stdio`, `appaloft mcp serve`, or `npx appaloft-mcp` expose the same operation catalog. Copying the skill with `npx skills add` does not write MCP. After `appaloft login`, `appaloft setup agent` writes skill plus MCP into default-checked Cursor and Claude Code hosts. OpenCode needs `--agent opencode` or the sibling install:
...
# One-command local Agent door, then explicit OpenCode if needed
appaloft setup agent
appaloft auth mcp opencode install
```

MCP tools reuse the exact same operations, input interpretation, and recovery guidance as the CLI/API — see [MCP And Tool Protocols](/docs/en/agents/mcp/). In Grok Bot, install the [Grok Bot plugin](/docs/en/agents/grok-bot-plugin/) to reach that same catalog; that path authenticates with OAuth on the hosted MCP at [`https://app.appaloft.com/mcp`](https://app.appaloft.com/mcp) and does not replace laptop `appaloft setup agent`.
