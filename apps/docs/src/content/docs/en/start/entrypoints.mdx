...
When an agent host is configured with Appaloft MCP, `appaloft mcp stdio`, `appaloft mcp serve`, or `npx appaloft-mcp` expose the same operation catalog. Copying the skill does not write MCP; Cursor and OpenCode use existing commands to write host config:
...
# Write a token-free MCP entry into Cursor or OpenCode
