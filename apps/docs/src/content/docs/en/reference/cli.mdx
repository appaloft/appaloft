...
`appaloft code` opens a remote Agent session after login. It requires an enrolled Server and a
Project Binding. The laptop path is only a locator; dirty files are not uploaded. Use
`--no-attach` to print the identity banner without attaching. Use `--local` for this-Mac Scratch.
...
`--profile` and `--new` keep remote `workspaces.open` meaning and do not inspect a dirty laptop
tree. `appaloft workspace open` still requires a clean, pushed Git branch. See
