...
files are not uploaded. While opening, it prints short status lines. The
Remote banner prints as soon as occupy returns, then default `code` attaches.
Skill copy is fail-soft and time-bounded. Use `--no-attach` to print the
same progress and occupancy banner without attaching. Default `code` picks the live occupancy when leftover
