...
`npx skills add` only copies the skill; it does not write MCP. Cursor and OpenCode reuse an already-logged-in Appaloft profile and write a token-free stdio launcher into editor config:
...
Cursor merges `~/.cursor/mcp.json` `mcpServers.appaloft`. OpenCode merges `~/.config/opencode/opencode.json` `mcp.appaloft`. Codex still needs the dedicated bearer profile: `appaloft auth mcp login` then `appaloft auth mcp codex install`. A directory without git can be deployed with `appaloft deploy .`.
