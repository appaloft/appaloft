...
appaloft code                                 # Open a local scratch Agent session on this Mac
...
`appaloft code [path]` opens a local Scratch session on this Mac. Git, login, Binding, and Cloud
are not required. The path defaults to `.`. Use `--no-attach` to print the scratch banner without
spawning the Agent TUI.
...
`--profile` and `--new` keep durable `workspaces.open` meaning and still require a clean, pushed
Git branch. Existing automation can keep using `appaloft workspace open .`. See
