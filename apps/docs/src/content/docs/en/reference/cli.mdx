...
appaloft setup agent                          # Teach local hosts skill + MCP
appaloft code                                 # Open a Workspace session
...
## Up: first deployment entrypoint <a id="cli-up-deployment-entrypoint" />

```bash
appaloft up [path-or-source] [--yes] [--project <projectId>] [--json]
```

`appaloft up` is the primary folder-to-live-app entrypoint. It uses the current directory when the
...
[Your First Deployment](/docs/en/start/first-deployment/#cli-up-first-deployment) for the full task.

## Workspace quick entry

`appaloft code` opens a Workspace session on the team's default enrolled Server
