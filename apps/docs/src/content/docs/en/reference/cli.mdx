...
appaloft code                                 # Open or resume an Agent Workspace from this Git repository
...
From a clean, pushed Git branch, `appaloft code [path]` creates or resumes a Profile-aware
Workspace through the existing `workspaces.open` operation. When the Adapter supports attach, it
enters the Agent-owned TUI or native client. The path defaults to `.`.
...
`--profile` selects an Agent Workspace Profile; the global `--control-plane-profile` selects a
saved local control-plane connection. Existing automation can keep using
`appaloft workspace open .` with unchanged semantics and machine-readable behavior. See
[Open a Workspace from a local repository](/docs/en/agents/workspaces/#agent-workspace-open) for
prerequisites, reconnection, and safe failure guidance.
