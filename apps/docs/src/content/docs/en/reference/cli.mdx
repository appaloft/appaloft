...
One command leaves detected Cursor and OpenCode with both the Appaloft skill and Local MCP. Tokens stay in the Appaloft CLI profile, not in editor config:
...
`appaloft setup agent` copies the skill into detected Cursor and OpenCode skill directories (`~/.cursor/skills`, `~/.config/opencode/skills`) plus `~/.agents/skills`, then writes token-free stdio launchers. Cursor merges `~/.cursor/mcp.json` `mcpServers.appaloft`. OpenCode merges `~/.config/opencode/opencode.json` `mcp.appaloft`. Explicit siblings remain: `appaloft auth mcp cursor install` and `appaloft auth mcp opencode install`. `npx skills add` still only copies the skill. Codex still needs the dedicated bearer profile: `appaloft auth mcp login` then `appaloft auth mcp codex install`. A directory without git can be deployed with `appaloft deploy .`.
