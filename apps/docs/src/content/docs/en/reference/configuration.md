...
| `APPALOFT_WORKER_SLOT` | unset | Optional explicit slot for this process. Leave unset for Docker Compose/Kubernetes replicas; standalone workers lease one slot from the shared heartbeat store while `APPALOFT_WORKER_COUNT` remains the expected group size. |
