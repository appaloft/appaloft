...
- With no Git, creates a project named after the directory (`--yes` / non-interactive / `appaloft code`), or uses your only project when you have exactly one.
- `appaloft code` never asks you to pick a project inside the session TUI. With several projects, `deploy` or `project use` is the place to choose; `code` creates or links the directory-named project.
