...
## Host install <a id="appaloft-mcp-host-install" />
...
`appaloft setup agent` copies the skill into `~/.cursor/skills/appaloft`, `~/.config/opencode/skills/appaloft`, and `~/.agents/skills/appaloft` for detected hosts, then writes the same Local MCP launchers as the sibling commands. Cursor merges `~/.cursor/mcp.json` `mcpServers.appaloft`. OpenCode merges `~/.config/opencode/opencode.json` `mcp.appaloft` (`type: "local"`, `command: [...]`, `enabled: true`). Both launch `appaloft mcp remote-stdio --profile <active-or-requested>`.

Explicit siblings remain available: `appaloft auth mcp cursor install` and `appaloft auth mcp opencode install`. `npx skills add` still only copies the skill. Codex still needs the dedicated bearer profile: `appaloft auth mcp login` then `appaloft auth mcp codex install`. A directory without git can be deployed with `appaloft deploy .`.
