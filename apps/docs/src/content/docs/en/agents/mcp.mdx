...
`npx skills add` does not write MCP. After a local Appaloft profile exists, use the same command family to write host config without putting tokens in editor files:
...
Cursor merges `~/.cursor/mcp.json` `mcpServers.appaloft`. OpenCode merges `~/.config/opencode/opencode.json` `mcp.appaloft` (`type: "local"`, `command: [...]`, `enabled: true`). Both launch `appaloft mcp remote-stdio --profile <active-or-requested>`. Codex still needs the dedicated bearer profile: `appaloft auth mcp login` then `appaloft auth mcp codex install`. A directory without git can be deployed with `appaloft deploy .`.
