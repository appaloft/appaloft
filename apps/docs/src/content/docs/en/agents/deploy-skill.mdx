...
`appaloft up` is the canonical entrypoint; `appaloft deploy` remains the supported 1.x compatibility
spelling for the same workflow. An agent or other non-TTY caller should pass `--yes` only after the
user has authorized deployment mutation.
