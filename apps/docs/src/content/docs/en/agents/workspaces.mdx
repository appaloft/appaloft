...
The CLI uses `.` only to find the Git root, upstream remote, branch, and HEAD SHA. It does not
...
While the session TTL and Sandbox remain valid, running `workspace open` again reuses the Session,
