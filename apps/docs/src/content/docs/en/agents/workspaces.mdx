...
Default `appaloft code` resolves the team's default enrolled Server and Binding remote SHA,
then native-attaches OpenCode on this laptop (otherwise Pi, otherwise Oh My Pi). Adapter,
Profile, and Sandbox are not required. The laptop path is not Workspace truth, and dirty
files are not uploaded. The banner is:
