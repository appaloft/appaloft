...
`appaloft code --claude`, `--codex`, and `--grok` pick the vendor credential on this
laptop. They map onto the existing occupancy runtime (OpenCode by default).
`--harness opencode|pi|omp` remains an advanced override and does not invent a new
runtime. With no vendor flag, `code` follows what is signed in or installed on this
laptop.
