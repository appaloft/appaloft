...
Default `appaloft code [path]` opens a local Scratch session. Path defaults to `.`. Empty, dirty,
detached, unpushed, logged-out, and Binding-less directories are valid. The CLI does not call
`workspaces.open` and does not upload local files. The banner is:

```text
Local scratch · this Mac · not saved remotely
```

The default harness is OpenCode if present, otherwise Pi. If neither binary exists, Appaloft
prompts to install. Refusing install is the only hard failure before attach. Scratch offers the
public Appaloft skill to that local Agent without writing project files. `--no-attach` resolves
scratch, prints the banner and harness, then exits.

Durable Workspace still uses:

```bash
appaloft workspace open
appaloft code . --profile <workspace-profile>
appaloft code . --new
```

Those paths still dispatch `workspaces.open`. The CLI uses the path only to find the Git root,
...
While the session TTL and Sandbox remain valid, running `appaloft workspace open` again reuses the
Session, replays bounded output, and continues the same Agent process:

```bash
appaloft workspace open
```

Default `appaloft code` opens a new local Scratch session and does not reconnect a durable
Workspace. Existing automation can keep using `appaloft workspace open .`; it is not deprecated
and its machine-readable behavior is unchanged.
