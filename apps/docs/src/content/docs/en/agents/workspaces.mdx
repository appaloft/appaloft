...
`--profile` selects an Agent Workspace Profile on both `appaloft code` and
`appaloft workspace open`. It is distinct from `--control-plane-profile`. Default
`code` prefers a live occupancy over dead same-name Profiles.
