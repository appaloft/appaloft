...
map onto the existing OpenCode Cloud Agents runtime. `--opencode`, `--pi`, and `--omp`
pick that Cloud Agents harness. They do not invent a new runtime. `--harness
opencode|pi|omp` is compatibility only. With no alias, `code` uses the saved
Cloud Agents preference, then what is signed in or installed on this laptop.
