...
disk preparation on the same enrolled Server. Attach starts only after disk
prep succeeds. The wait panel does not leave to print a folder path or restore
the terminal just because a short retry burst failed. This stay/retry is the
same occupy, not a second local folder open. `--omp` is the OpenCode
