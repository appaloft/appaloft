...
`Attaching…`, then attaches as soon as the occupancy exists. HOME skill copy can finish
in the background. `--no-attach` prints the same progress and stops after occupy.
The banner is:
