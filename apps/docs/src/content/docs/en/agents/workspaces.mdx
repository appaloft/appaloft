...
Agents wait stays up and retries disk preparation on the same enrolled Server.
Attach starts only after disk prep succeeds. If those retries are exhausted, the
screen restores the terminal first, then prints a next step such as
