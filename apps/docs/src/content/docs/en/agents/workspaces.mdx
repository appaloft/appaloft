...
clone. On a TTY, default `code` and `workspace` enter the occupancy Workspace TUI
(alt screen) immediately and show centered/footer progress from the first frame
(`Connecting to Appaloft…`, then occupy / copy skills / attach). They do not stream
status lines as the primary UX. Attach starts as soon as occupy returns. HOME skill
copy is fail-soft and time-bounded. `--no-attach` and non-TTY scripts keep one-line
stderr progress (including a first-byte status line before local database or
composition boot) and print the Remote banner.
The banner is:
