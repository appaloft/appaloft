...
Agents wait stays up, marks that disk step as retrying, and retries disk
preparation on the same enrolled Server. Attach starts only after disk prep
succeeds. If those retries are exhausted, the disk step is marked failed, the
screen restores the terminal once, then prints a next step such as
`appaloft code --omp --server srv_4lifk0yrcecy`. `--omp` is the OpenCode
