...
<Aside type="caution" title="Maturity: Public alpha">
  Workspace's CLI, SDK, and underlying operations are a public Appaloft capability; the specific
  Sandbox provider, templates, gateway, and public-address capabilities depend on the operator's
...
clone. On a TTY, default `code` first prints **Loading your project** on the
normal screen, then opens the Cloud Agents TUI with the workspace list
**collapsed**. The wait panel is **preparing the agent** with Using your
credential / Including your skills / Waking the agent, including a generated
`adjective-noun` agent name. It is not Checking login or Preparing disk.
Occupy / copy skills / attach stay a left-aligned step list (Braille spinner,
then ✓). Attach replaces that wait in the same fullscreen pane. Reopening the
same folder resumes the last live disk instead of creating another one.
`workspace` opens the same TUI as a navigator. They do not stream status lines
as the primary UX.
...
steps `Using your {Grok|Codex|Claude} credential on the agent`,
`Including N of your skills`, and `Woke agent {name}`. `N` is the number of
skills actually copied.
A source checkout (`appaloftdev`) finds or `cargo build`s the workspace TUI
...
`conflict at workspace-control-select` or a leftover bootstrap conflict
in the footer.
The banner is:
...
<a id="agent-workspace-display-name" />
`<name>` is the Agent handle, not `sbx_…`. A git occupy gets a generated kebab
name such as `resonant-silence`. `folder.local` may use the directory name.
`repo@short-sha` is the git pin on the banner, not the handle. Reopen keeps
the same name. Cloud Agents list and detail use that name. JSON / `--json`
may still include `workspaceId`. `workspace show|pause|resume|terminate` accept
the display name.
...
The first interactive `appaloft code` asks whether to bring local skills and, when more
than one root exists, which directory to read. Answers are saved in
`~/.appaloft/agent-prefs.json`. Later launches reuse that choice. `--yes` or a
non-TTY first run saves skills sync off.
...
those two roots are **beyond Railway**. Occupy copies only the saved root, not every
root at once.
...
## Pick Claude, Codex, or Grok in Cloud Agents <a id="agent-code-vendor" />
...
`--claude`, `--codex`, and `--grok` launch that vendor CLI on the remote
Sandbox and copy the laptop credential onto session HOME. `--opencode`,
`--pi`, and `--omp` pick that Cloud Agents harness. `--harness
opencode|pi|omp|claude|codex|grok` is compatibility only. With no alias, `code` uses the
saved Cloud Agents preference, then what is signed in or installed on this
laptop.



Occupy writes that credential onto the session disk (`HOME=/workspace`), not as a
sandbox variable:

- Grok: `~/.grok/auth.json` → session `.grok/auth.json`
- Codex: `~/.codex/auth.json` → session `.codex/auth.json`
...
The session also keeps the public Appaloft skill and a first-party Appaloft MCP
...
After the occupy ends, remove the remote copy of a Codex credential with:

```bash
appaloft sandbox file remove <sandboxId> --path .codex/auth.json
```

Removing the remote file does not revoke the corresponding upstream login session.
If the credential may have been exposed, also revoke that session in the Codex / OpenAI
...
<a id="agent-workspace-preparing-disk" />
If **Preparing disk** hits a temporary Cloud gateway failure (HTTP 502 or 503, including a
Cloudflare bad-gateway or incomplete origin page), the Cloud Agents wait stays up, marks that disk
step as retrying, and keeps retrying disk preparation on the same enrolled Server until a typable
session is bound or the occupy deadline expires. Attach starts only after disk prep succeeds;
staying on the wait panel is not success. The wait panel does not leave to print a folder path or
restore the terminal just because a short retry burst failed. If the deadline expires, the in-flight
disk prep is cancelled, or a session never attaches, the CLI fails closed: restore the terminal
once, print that Cloud is temporarily unreachable / disk preparation did not finish, exit non-zero,
and do not print a folder path. Ctrl-C / quit on the wait panel aborts the in-flight open and does
not look like success. This stay/retry is the same occupy, not a second local folder open. `--omp`
is the OpenCode harness; use `--pi` when that is the agent you opened. The CLI keeps the HTTP status
and does not dump the gateway page.
...
someone else's session. Dirty trees,
...
Default `appaloft code` prefers a live session over dead same-name Profiles.
...
more than one live session. It is distinct from `--control-plane-profile`.
`--new` still occupies the cwd origin, not the current whoami session. A failed
preferred Workspace in partial creation is retained for diagnostics while default
`code` automatically creates an isolated replacement; the user does not need to
rerun with `--new`. Lower-level `workspace open` / `workspace create` continue to
return the original partial-recovery evidence instead of creating a replacement.
An otherwise failed
...
`workspaceId`/`runtimeId`, retryability, and recovery/terminate entrypoints. Lower-level
`workspace open` coordinates against that same partial identity instead of silently creating a
duplicate Sandbox. The default one-command `code` door may retain that identity and create the
