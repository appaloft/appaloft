...
`<name>` is a readable workspace name, not `sbx_…`. Linked folders use the directory
name. A git repo uses `repo@short-sha`. An unlinked open gets a generated kebab
name such as `resonant-silence`. Reopen keeps the same name. Cloud Agents list
and detail use that name. JSON / `--json` may still include `workspaceId`.
`workspace show|pause|resume|terminate` accept the display name.
