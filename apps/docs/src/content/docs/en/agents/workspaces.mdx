...
If **Preparing disk** hits a temporary Cloud gateway failure (HTTP 502 or
503, including a Cloudflare bad-gateway or incomplete origin page), the Cloud
Agents wait stays up, marks that disk step as retrying, and keeps retrying
disk preparation on the same enrolled Server until a typable session is bound
or the occupy deadline expires. Attach starts only after disk prep succeeds;
staying on the wait panel is not success. The wait panel does not leave to
print a folder path or restore the terminal just because a short retry burst
failed. If the deadline expires, the in-flight disk prep is cancelled, or a
session never attaches, the CLI fails closed: restore the terminal once, print
that Cloud is temporarily unreachable / disk preparation did not finish, exit
non-zero, and do not print a folder path. Ctrl-C / quit on the wait panel
aborts the in-flight open and does not look like success. This stay/retry is
the same occupy, not a second local folder open. `--omp` is the OpenCode
harness; use `--pi` when that is the agent you opened. The CLI keeps the HTTP
status and does not dump the gateway page.
