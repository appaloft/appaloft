...
HOME skill copy is fail-soft and time-bounded. `--no-attach` and non-TTY scripts
keep one-line stderr progress (including a first-byte status line before local
database or composition boot) and print the Remote banner.
...
Only directories that contain `SKILL.md` are copied. Existing sandbox files are not
