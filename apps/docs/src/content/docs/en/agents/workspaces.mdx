...
Run this from a clean, pushed Git branch:
...
The successful result and Workspace detail return safe activation and target-selection evidence:

- Project, Repository Binding, and Profile activation items each report `created` or `reused`;
- target class is `managed`, `registered-server`, or `local`; selection source is
  `platform-default`, `saved-policy`, or `explicit`; and a stable reason explains the choice;
- evidence contains no Server id, host, provider handle, capacity probe, or credential. Reopening
  the same Workspace returns the persisted evidence without rerunning placement. A legacy Workspace
  without evidence explicitly reports `legacy-unclassified` rather than guessing its location.
...
Appaloft never silently changes a managed request to a registered Server or the local machine.
