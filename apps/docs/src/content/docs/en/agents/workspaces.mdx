...
`<name>` is a readable workspace name, not `sbx_…`. Linked or current-folder opens
use the directory name. A git repo uses `repo@short-sha`. An unlinked or unnamed
open gets an Appaloft-generated kebab name such as `resonant-silence`. Reopen
keeps the same name. Cloud Agents list and detail use that name. JSON / `--json`
...
`--claude` and `--grok` pick the vendor credential on this laptop and map onto
the OpenCode Cloud Agents runtime. `--codex` launches Codex on the remote
Sandbox and copies `~/.codex/auth.json` onto session HOME. `--opencode`,
`--pi`, and `--omp` pick that Cloud Agents harness. `--harness
opencode|pi|omp|codex` is compatibility only. With no alias, `code` uses the
