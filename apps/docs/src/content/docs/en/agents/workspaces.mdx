...
clone. On a TTY, default `code` opens the Cloud Agents TUI (alt screen)
immediately with the workspace list **collapsed**, so the wait gets the whole
window. The parent first frame is already the centered wait panel (**preparing
the agent** plus Checking login / Preparing skills / Preparing disk), not an
empty two-line splash. Occupy / copy skills / attach stay a left-aligned step
list (Braille spinner, then ✓). Attach replaces that wait in the same fullscreen
pane. Reopening the same folder resumes the last live disk instead of creating
another one. `workspace` opens the same TUI as a navigator. They do not stream
status lines as the primary UX.
