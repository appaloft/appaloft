...
Leaving or disconnecting the TUI detaches only the client; explicit Workspace lifecycle commands
still own pause, resume, and termination. For scripts, CI, a non-TTY environment, or when the
renderer should not load, use:
...
mode needs separate release and terminal acceptance.
