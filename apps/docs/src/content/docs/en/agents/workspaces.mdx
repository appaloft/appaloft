...
copy. Delivery actions keep the current Agent Session attached. Promotion rows with deployment
...
task approve/deliver`, `sandbox promote accept/retry`, and `deployment proof`.
