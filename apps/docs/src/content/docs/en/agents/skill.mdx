...
One command leaves detected Cursor and OpenCode with both the skill and Local MCP:
...
`appaloft setup agent` copies the skill into `~/.cursor/skills/appaloft`, `~/.config/opencode/skills/appaloft`, and `~/.agents/skills/appaloft` for detected hosts, then writes token-free Local MCP. Tokens stay out of `mcp.json` / `opencode.json`.
