...
Web console 应让用户从资源页或访问页创建绑定，并在同一流程中提示所有权检查和 TLS readiness。

CLI 适合自动化绑定，例如在发布脚本里把 hostname 绑定到已存在资源。CLI 输出应包含绑定 id、hostname、当前状态和下一步操作。

HTTP API 应返回绑定状态、所有权检查状态、证书状态和可恢复错误。API 不应把 DNS/TLS 语义藏在普通部署状态里。
